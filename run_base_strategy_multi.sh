...
#
# Enable detailed V4.1.1 research logging:
...
grep -q "DEPLOY_POLICY_VERSION = 'base_v4_4_champion'" "$AGENT_PATH/BaseStrategy.py" || {
  echo "ERROR: BaseStrategy.py is not the frozen V4.4 champion" >&2
  echo "Expected DEPLOY_POLICY_VERSION = 'base_v4_4_champion' in $AGENT_PATH/BaseStrategy.py" >&2
  exit 1
}
grep -q "BASE_CHAMPION_PARENT = 'base_v4_1_1_maker_guard'" "$AGENT_PATH/BaseStrategy.py" || {
  echo "ERROR: BaseStrategy.py is missing the V4.1.1 maker-guard parent marker" >&2
  exit 1
}
...
mm_force_post_only=1 mm_maker_guard_reprice=1 debug_slow_request_ms=250.0 \
...
echo "[BaseStrategy] version=base_v4_4_champion"
