...
if ! grep -q "DEPLOY_POLICY_VERSION = 'base_v4_4_champion'" "$AGENT_PATH/BaseStrategy.py"; then
  echo "ERROR: AdaptiveAgent V3 requires live BaseStrategy V4.4 champion." >&2
  echo "Expected DEPLOY_POLICY_VERSION = 'base_v4_4_champion' in $AGENT_PATH/BaseStrategy.py" >&2
  exit 1
fi
if ! grep -q "BASE_CHAMPION_PARENT = 'base_v4_1_1_maker_guard'" "$AGENT_PATH/BaseStrategy.py"; then
  echo "ERROR: AdaptiveAgent V3 requires BaseStrategy with V4.1.1 maker-guard parent." >&2
  exit 1
fi
...
adaptive_drift_start_requests=1000 adaptive_drift_min_quotes=30 \
adaptive_drift_fill_abs_min=0.005 adaptive_drift_fill_relative=0.40 \
adaptive_drift_spread_ratio=1.30 adaptive_drift_spread_delta_bps=4.0 \
...
echo "[AdaptiveAgent] base_policy=base_v4_4_champion"
