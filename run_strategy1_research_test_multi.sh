...
# Historical 6/12/50 flywheel density scheduling is intentionally NOT
# preflighted in V4.14.5: it is retained only as telemetry/backward-compatible
# helper code and must not own live lane allocation.
...
from research_inventory_liveness import (
    ...
    BOUNDED_LOSS_ESCAPE_VERSION, bounded_loss_escape_applies, bounded_loss_escape_reason,
    counts_against_productive_open_cap, classify_liveness_stage, evaluate_bounded_rescue, fresh_maker_grace_applies,
...
if KAPPA_PRODUCTIVITY_VERSION != "wide_kappa_productivity_v4_14_2":
    raise SystemExit("ERROR: stale research_kappa_productivity.py for V4.14.2")
...
# SN79 launcher for Strategy1_Research V4.14.5 TOTAL_SCORE_FRONTIER P1 (Miner 1).
...
grep -q 'RESEARCH_POLICY_VERSION = "total_score_frontier_v4_14_5"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not total_score_frontier_v4_14_5" >&2
  exit 1
}
...
[[ -f "$AGENT_PATH/research_total_score_frontier.py" ]] || { echo "research_total_score_frontier.py missing" >&2; exit 1; }
...
if TOTAL_SCORE_FRONTIER_VERSION != "total_score_frontier_v4_14_5":
    raise SystemExit("ERROR: stale V4.14.5 total-score helper")
if phase_budget_tuple(PHASE_IGNITION) != (4, 3, 3, 1):
    raise SystemExit("ERROR: V4.14.5 ignition budgets changed")
if phase_budget_tuple(PHASE_SURVIVAL) != (2, 5, 3, 1):
    raise SystemExit("ERROR: V4.14.5 survival budgets changed")
if phase_budget_tuple(PHASE_FRONTIER) != (2, 4, 3, 1):
    raise SystemExit("ERROR: V4.14.5 frontier budgets changed")
if scoring_pivot_indices(41) != (0, 1) or scoring_pivot_indices(80) != (39, 40):
    raise SystemExit("ERROR: V4.14.5 score-pivot math changed")
rows = [
...
if plan.phase != PHASE_IGNITION:
    raise SystemExit("ERROR: V4.14.5 ignition phase failed")
if not by_id[1].total_score_due or classify_execution_lane(by_id[1]) != LANE_COMPLETION:
    raise SystemExit("ERROR: V4.14.5 ONE_AWAY must own completion priority")
if by_id[2].total_score_due or classify_execution_lane(by_id[2]) != LANE_COVERAGE:
    raise SystemExit("ERROR: V4.14.5 qualified book stole completion capacity during ignition")
if classify_execution_lane(by_id[3]) != LANE_COVERAGE:
    raise SystemExit("ERROR: V4.14.5 fresh book must remain coverage")
...
if alloc.used[LANE_COVERAGE] != cov:
    raise SystemExit("ERROR: V4.14.5 IGNITION coverage reserve collapsed under the candidate cap")
print("V4.14.5 TOTAL_SCORE_FRONTIER API OK")
PYV4145TOTALSCORE
...
research_enable_fast_candidate_screen=1 research_candidate_count=11 research_cohort_size=8 research_cohort_exploration_slots=2 research_max_open_books=6 research_max_active_open_books=6 research_max_total_open_books=8 research_max_parked_open_books=4 research_max_total_abs_base=2.0 research_kappa_conversion_pressure_enabled=1 research_kappa_conversion_reserve_slots=3 research_kappa_exploration_slots=2 research_kappa_flywheel_enabled=1 research_kappa_productivity_enabled=1 research_wide_kappa_min_density_observations=3 research_wide_kappa_preferred_density_observations=3 research_wide_kappa_raw_target=0.35 research_core_probe_enabled=0 research_persistent_maker_enabled=1 research_hysteresis_min_price_ticks=3 research_post_only_safety_ticks=2 \
...
if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "V4.14.5 total-score-frontier + V4.14.4 RealNet safety preflight-only PASS"
  exit 0
fi
