...
"""V4.12.15 bounded inventory-liveness policy for Strategy1 Research.
...
INVENTORY_LIVENESS_VERSION = "inventory_liveness_v4_12_15"
...
def evaluate_bounded_rescue(
    ...
    *,
    ...
    if not stage.taker_rescue_armed:
        return InventoryRescueDecision(
    ...
    if taker < stage.taker_floor_bps - 1e-12:
        # The staged rescue window has already been missed. Do not leave this
        # stale position consuming an active acquisition slot while waiting for
        # the wider hard-emergency window; park it and reconsider only on the
        # bounded refresh cadence / material touch move / hard-risk trigger.
        return InventoryRescueDecision(
    ...
    return InventoryRescueDecision(
        True, False, "BOUNDED_RESCUE_GT_WAIT", adverse, taker, wait, advantage,
        stage.taker_floor_bps, stage.hard_floor_bps,
    )
