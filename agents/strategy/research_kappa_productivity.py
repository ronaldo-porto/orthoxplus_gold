...
KAPPA_PRODUCTIVITY_VERSION = "simplified_kappa_productivity_v4_13"
...
class ProductivitySnapshot:
    ...
    def placements_per_rt(self) -> float:
        ...
        return float(max(0, self.maker_quotes)) / float(self.round_trips)
    ...
    def execution_tier(self) -> str:
        # Require fresh V4.13 execution evidence before calling a book PRODUCTIVE.
        # This prevents an upgraded session with old RT history but no quote/fill
        # burden ledger from being promoted to CORE on migration.
        if self.maker_quotes < 6 or self.maker_fills < 2:
            return TIER_UNKNOWN
        if self.round_trips <= 0 and self.maker_quotes < 20:
            return TIER_UNKNOWN
        if (
            self.placements_per_rt > 45.0
            or self.contract_reject_rate > 0.025
            or (self.maker_quotes >= 20 and self.maker_fill_conversion < 0.05)
        ):
...
def priority_for_state(
    ...
    *,
    ...
    tier_mult = 0.55 if snapshot.execution_tier == TIER_INEFFICIENT else 1.0
...
def sanitize_productivity_runtime(raw: Any) -> dict[int, dict[str, int]]:
    ...
    keys = ("maker_quotes", "maker_fills", "contract_rejects", "last_rt_tick")
