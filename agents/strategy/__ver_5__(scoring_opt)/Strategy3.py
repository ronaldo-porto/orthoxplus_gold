...
with:
  ...
  - Simulation-time phased mining: OBSERVE, CALIBRATE, ACTIVE, COOLDOWN,
    and RESUME, with score/risk-gated transitions
...
class Strategy3(DetailedTemplateAgent):
    ...
    Agent params (in addition to DetailedTemplateAgent params):
        ...
        rolling_kappa_lookback_ns (int): Exact Kappa rolling window
            (default 3 sim hours). Independent of PnL lookback.
        rolling_pnl_lookback_ns (int): Exact PnL rolling window
            (default 3 sim hours). Do not assume equal to Kappa.
        rolling_kappa_min_history_ns (int): Min Kappa history span
            (default 1.5 sim hours).
        rolling_min_observations (int): Min non-zero PnL obs for Kappa
            eligibility (default 3).
        score_floor_guard_ratio (float): Defend harder when
            score_to_internal_median < ratio (default 1.05).
        weak_book_score_quantile (float): Books at/below this quantile of own
            book scores are weak (default 0.35).
        left_tail_score_quantile (float): Candidate cutoff for far-left-tail
            books (default 0.20).
        weak_book_size_mult (float): Size multiplier on weak books (default 0.5).
        left_tail_new_risk_enabled (bool): Permit fresh risk on left-tail books
            (default False).
        min_floor_expected_pnl (float): Stronger maintenance FIFO PnL gate
            (default 0.0001).
        expiry_horizon_frac (float): Fraction of Kappa lookback treated as
            near-expiry for strong books (default 0.15).
        top_rank_selectivity_boost (float): Extra EV / alpha selectivity when
            below floor guard (default 0.15). Does not raise volume.
        enable_phase_mining (bool): Enable simulation-time phased mining
            (default True).
        startup_observe_ns (int): OBSERVE duration in simulation ns
            (default 600_000_000_000).
        calibration_max_ns (int): Maximum CALIBRATE duration
            (default 1_800_000_000_000).
        min_calibration_books (int): Distinct safely quoted calibration books
            required before ACTIVE (default 8).
        phase_score_guard_ratio (float): Enter/retain ACTIVE only above this
            internal-median ratio (default 1.05).
        cooldown_ns (int): Minimum COOLDOWN duration (default 600_000_000_000).
        resume_score_guard_ratio (float): Score ratio required to leave
            COOLDOWN through RESUME (default 1.08).
        phase_min_expected_pnl (float): CALIBRATE adjusted expected-PnL floor
            (default 0.0001).
    """
    ...
    def initialize(self) -> None:
        ...
        self._selectivity_alpha_boost = 0.0
        self._selectivity_pnl_boost = 0.0
        self._selectivity_book_cap = 0
        ...
        # Simulation-time phased mining. Never use wall-clock time for phases.
        ...
        self.cooldown_ns = max(
            1, int(getattr(cfg, "cooldown_ns", 600_000_000_000))
        )
        ...
        self._phase_reason = (
            "startup_observation" if self.enable_phase_mining else "phase_mining_disabled"
        )
        self._phase_score_state: dict = {}
        self._phase_score_state_ts: int | None = None
        self._phase_regime: MarketRegime | None = None
        ...
        bt.logging.info(
            ...
            f"kappa_min_hist={self.rolling_kappa_min_history_ns} "
            f"min_obs={self.rolling_min_observations} "
            ...
            f"expiry_frac={self.expiry_horizon_frac} "
            f"selectivity_boost={self.top_rank_selectivity_boost} "
            f"phase_mining={self.enable_phase_mining} "
            f"phase={self._mining_phase} observe_ns={self.startup_observe_ns} "
    ...
    def _reset_pnl_state(self) -> None:
        ...
        self._phase_score_state = {}
        self._phase_score_state_ts = None
        self._phase_regime = None
    ...
    def _apply_top_rank_pressure(self, snap: RollingDiagnosticsSnapshot) -> None:
        """Increase selectivity near/below soft floor — never raise raw volume."""
        pressure = bool(
            self.enable_floor_awareness
            and snap.floor_threshold > 0.0
            and snap.score_to_internal_median < self.score_floor_guard_ratio
        )
        self._top_rank_pressure = pressure
        if pressure:
            self._selectivity_alpha_boost = self.top_rank_selectivity_boost
            self._selectivity_pnl_boost = max(
                0.0, self.min_floor_expected_pnl * self.top_rank_selectivity_boost
            )
            # Prefer fewer cleaner books (cap, never expand).
            self._selectivity_book_cap = max(
                1, int(math.ceil(self.max_mm_books_per_tick * 0.75))
            )
        else:
            self._selectivity_alpha_boost = 0.0
            self._selectivity_pnl_boost = 0.0
    ...
    def _effective_min_expected_alpha(self) -> float:
        return self.min_expected_alpha + float(
            getattr(self, "_selectivity_alpha_boost", 0.0)
        )

    def _effective_min_expected_pnl(self) -> float:
        return self.min_expected_realized_pnl + float(
            getattr(self, "_selectivity_pnl_boost", 0.0)
        )

    def _effective_mm_book_cap(self) -> int:
        cap = int(getattr(self, "_selectivity_book_cap", 0) or 0)
        if cap > 0:
            return min(self.max_mm_books_per_tick, cap)
        return self.max_mm_books_per_tick

    def _passes_expected_pnl_gate(self, expected_realized_pnl: float) -> bool:
        return expected_realized_pnl > self._effective_min_expected_pnl()

    def _book_allows_score_refresh(
        self,
        book_id: int,
        profile: BookProfile,
        expected_pnl: float,
    ) -> bool:
        """Refresh expiring strong history only on clean positive-EV books."""
        if expected_pnl <= max(self.min_floor_expected_pnl, self._effective_min_expected_pnl()):
            return False
        if book_id in self._last_weak_books or book_id in self._last_left_tail_books:
            return False
        if profile.tier not in ("GREEN", "YELLOW"):
            return False
        if float(profile.realized_pnl) < 0.0:
            return False
        mem = self._mem(book_id)
        if mem.loss_streak > 0 or mem.recent_pnl < 0.0:
            return False
        return True

    def _compute_rolling_diagnostics(
        self,
        profiles: list[BookProfile],
    ) -> RollingDiagnosticsSnapshot:
        book_ids = [p.book_id for p in profiles]
        if not book_ids and self.realized_pnl_history:
            book_ids = []
            for books in self.realized_pnl_history.values():
                book_ids.extend(books.keys())
        return compute_rolling_diagnostics(
    ...
    def _maintenance_floor_gate(
        ...
        """No maintenance trades for volume; require clean positive expected PnL."""
        if not self.enable_floor_awareness:
            return True
        # Activity impact is disabled on validators — never farm volume.
        mem = self._mem(book_id)
        ...
        if snap is not None:
            diag = snap.books.get(book_id)
            if diag is not None and diag.rolling_pnl < 0.0:
                return False
            # Near-expiry strong books: only refresh if this book is clean.
            if book_id in snap.expiring_strong_books:
                pass  # still require EV gate below
            elif snap.expiring_strong_books and book_id not in snap.eligible_books:
                # Do not replace expiring quality history with cold/low-quality books.
                return False
        ...
        min_pnl = max(self.min_floor_expected_pnl, self._effective_min_expected_pnl())
        if est.expected_realized_pnl < min_pnl:
            return False
        if book_id in getattr(self, "_last_expiring_strong_books", set()):
            return self._book_allows_score_refresh(
                book_id, profile, est.expected_realized_pnl
            )
        return True
    ...
    def _evaluate_score_floor(
    ...
    ) -> dict[str, float | int | bool | set[int] | dict[int, float] | list[int]]:
        """Rolling Kappa/PnL diagnostics + soft-floor proxy (local median stand-in)."""
        del regime  # retained for call-site compatibility
        snap = self._compute_rolling_diagnostics(profiles)
        self._apply_top_rank_pressure(snap)
        ...
        return {
            ...
            "internal_book_score_median": snap.floor_threshold,
    ...
    def _estimate_internal_score_state(
        self,
        state: MarketSimulationStateUpdate,
        selection: BookSelection,
    ) -> dict:
        """Cache the lightweight local score/floor estimate once per simulation tick."""
        if self._phase_score_state_ts == state.timestamp and self._phase_score_state:
            return self._phase_score_state

        floor_state = self._evaluate_score_floor(
            selection.profiles, self._phase_regime
        )
        clean_books = 0
        observed_books = 0
        negative_pnl_books = 0
        snap = self._last_rolling_snap
        for profile in selection.profiles:
            diag = snap.books.get(profile.book_id) if snap else None
            obs = diag.rolling_pnl_obs if diag is not None else profile.pnl_obs_count
            rolling_pnl = diag.rolling_pnl if diag is not None else profile.realized_pnl
            if obs > 0 or profile.raw_kappa is not None:
                observed_books += 1
            if rolling_pnl < 0.0:
                negative_pnl_books += 1
            if (
                profile.tier in ("GREEN", "YELLOW")
                and profile.book_id not in floor_state["weak_books"]
                and profile.book_id not in floor_state["left_tail_books"]
                and rolling_pnl >= 0.0
            ):
                clean_books += 1

        floor_state["clean_books_count"] = clean_books
        floor_state["observed_books_count"] = observed_books
        floor_state["negative_pnl_books_count"] = negative_pnl_books
        floor_state["calibrated_books_count"] = len(self._phase_calibrated_books)
        floor_state["eligible_books_count"] = len(floor_state.get("eligible_books", set()))
        floor_state["expiring_strong_count"] = len(
            floor_state.get("expiring_strong_books", set())
        )
        self._phase_score_state = floor_state
        self._phase_score_state_ts = state.timestamp
        return floor_state

    def _phase_elapsed(self, state: MarketSimulationStateUpdate) -> int:
        """Elapsed phase time in simulation nanoseconds, never wall-clock time."""
    ...
    @staticmethod
    def _phase_score_is_healthy(score_state: dict, ratio: float) -> bool:
        # Prefer rolling ratio score_to_internal_median; fall back to proxy > 0.
        score_ratio = float(score_state.get("score_to_internal_median", 0.0))
        median = float(score_state.get("floor_threshold", 0.0))
        if median <= 0.0 or score_ratio <= 0.0:
            score = float(score_state.get("trading_score_proxy",
                                          score_state.get("estimated_trading_score", 0.0)))
            return score > 0.0
    ...
    def _update_mining_phase(
        ...
        """Advance the mining FSM using simulation time and local score/risk state."""
        self._phase_regime = regime
        score_state = self._estimate_internal_score_state(state, selection)
        elapsed = self._phase_elapsed(state)
        if not self.enable_phase_mining:
            self._set_mining_phase(
                "ACTIVE", state.timestamp, "phase_mining_disabled"
            )
            return score_state

        normal_regime = regime.mode != "STRESSED"
        active_score_ok = self._phase_score_is_healthy(
            score_state, self.phase_score_guard_ratio
        )
        resume_score_ok = self._phase_score_is_healthy(
            score_state, self.resume_score_guard_ratio
        )
        ...
        if self._mining_phase == "OBSERVE":
            ...
            if not normal_regime:
                self._set_mining_phase(
                    "COOLDOWN", state.timestamp, "stressed_regime"
                )
            elif not active_score_ok:
                self._set_mining_phase(
            ...
            if not normal_regime or not resume_score_ok:
                self._set_mining_phase(
    ...
    def _phase_allows_new_risk(
        ...
        """Gate fresh exposure; inventory repair is handled outside this helper."""
        if not self.enable_phase_mining or self._mining_phase == "ACTIVE":
            # Still apply top-rank pressure alpha floor in ACTIVE.
            return expected_alpha >= self._effective_min_expected_alpha()
        if self._mining_phase not in ("CALIBRATE", "RESUME"):
            return False
        if inventory.band != "FLAT":
            return False
        if self._phase_regime is None or self._phase_regime.mode == "STRESSED":
            return False
        if profile.tier not in ("GREEN", "YELLOW"):
            return False
        snap = self._last_rolling_snap
        rolling_pnl = float(profile.realized_pnl)
        if snap is not None and book_id in snap.books:
            rolling_pnl = snap.books[book_id].rolling_pnl
        if rolling_pnl < 0.0:
            return False
        if book_id in self._last_weak_books or book_id in self._last_left_tail_books:
            return False
        ...
        if mem.loss_streak > 0 or mem.recent_pnl < 0.0:
            return False
        # CALIBRATE: only produce minimum high-quality observations.
        if self._mining_phase == "CALIBRATE":
            if len(self._phase_calibrated_books) >= self.min_calibration_books:
                return False
            if snap is not None and book_id in snap.expiring_strong_books:
                # Refresh only if clean positive-EV (checked by caller EV gate).
                pass
            elif snap is not None and snap.expiring_strong_books:
                # Prefer refreshing expiring strong books over cold new risk.
                if book_id not in snap.eligible_books:
                    return False
        alpha_floor = self._effective_min_expected_alpha()
        if self._mining_phase == "RESUME":
            alpha_floor *= self.resume_score_guard_ratio
        elif self._mining_phase == "CALIBRATE":
            alpha_floor = max(alpha_floor, self.min_expected_alpha + 0.05)
        return expected_alpha >= alpha_floor
    ...
    def _place_skewed_quotes(
        ...
        pnl_floor = self.min_expected_realized_pnl
        if (
            self.enable_phase_mining
            and self._mining_phase in ("CALIBRATE", "RESUME")
        ):
            pnl_floor = max(pnl_floor, self.phase_min_expected_pnl)
        if adj_pnl <= pnl_floor:
            ...
            if self.log_predict_pnl and self.verbose_log:
                bt.logging.info(
                    ...
                    f"(min={pnl_floor}) bid={bid_px} ask={ask_px}"
                )
            return 0
        if (
        ...
        if placed > 0 and self.enable_phase_mining and self._mining_phase == "CALIBRATE":
    ...
    def build_mm_strategy_instructions(
        ...
        stats = {
            ...
            "phase_elapsed_ns": self._phase_elapsed(state),
            "phase_reason": self._phase_reason,
            "internal_score_proxy": 0.0,
        }
        ...
        if self.enable_floor_awareness or self.enable_phase_mining:
            floor_state = self._estimate_internal_score_state(state, selection)
            ...
            stats["trading_score_proxy"] = round(
                float(floor_state.get("trading_score_proxy", stats["estimated_trading_score"])),
                6,
            )
            stats["soft_floor_score_proxy"] = round(
                float(
                    floor_state.get(
                        "soft_floor_score_proxy",
                        stats["estimated_soft_floor_score"],
                    )
                ),
                6,
            )
            stats["soft_floor_proxy"] = stats["soft_floor_score_proxy"]
            ...
            stats["score_to_median"] = stats["estimated_score_to_median"]
            ...
            stats["internal_score_proxy"] = stats["trading_score_proxy"]
        ...
        for book_id, book in state.books.items():
            ...
            # OBSERVE/COOLDOWN never add fresh risk: cancel live orders
            # and repair every non-flat inventory, including positions below the
            # normal management threshold.
            if (
                self.enable_phase_mining
                ...
                if inventory.band != "FLAT":
                    if profile is not None:
                        phase_archetype = self.classify_book_archetype(profile, regime)
                        phase_params = self.merge_regime_and_archetype_params(
                            regime_params, phase_archetype
                        )
                    else:
                        phase_archetype = "STRESSED"
                        phase_params = regime_params
                    urgency = 12.0 + self._inventory_util(inventory)
                    manage_queue.append(
                        (
                            urgency,
                            book_id,
            ...
            # Left-tail books are repair-only by default. This runs before all
            # fresh quote/maintenance paths and also repairs small inventories
            # below the normal management threshold.
            if (
                ...
                if n_cancel:
                    ...
                    stats["instructions"] += n_cancel
                if inventory.band != "FLAT":
                    urgency = 11.0 + self._inventory_util(inventory)
                    manage_queue.append(
            ...
            if inventory.band != "FLAT":
                if (
                    ...
                    if n_cancel:
                        stats["cancelled_risk_orders"] += 1
                        stats["instructions"] += n_cancel
                    urgency = 8.0 + self._inventory_util(inventory)
                    manage_queue.append(
                        (urgency, book_id, book, inventory, book_params, archetype),
                    )
                    continue
            ...
            if expected_alpha < self._effective_min_expected_alpha():
                stats["skipped_low_alpha"] += 1
                continue
            if not self._phase_allows_new_risk(
                book_id, profile, inventory, expected_alpha
            ):
                n_cancel = self._cancel_open_orders(response, book_id)
                if n_cancel:
                    stats["cancelled_risk_orders"] += 1
            ...
            # Near-expiry refresh: only clean positive-EV books may replace history.
            if (
                self.enable_floor_awareness
                and book_id in getattr(self, "_last_expiring_strong_books", set())
            ):
                # Rank boost so quality refresh is preferred over cold books.
                pass
            elif (
                self.enable_floor_awareness
                and getattr(self, "_last_expiring_strong_books", set())
                and book_id not in getattr(self, "_last_eligible_books", set())
                and self._top_rank_pressure
            ):
                stats["skipped_low_alpha"] += 1
                continue
        ...
        mm_cap = self._effective_mm_book_cap()
        for item in mm_candidates[:mm_cap]:
            (
                _rank,
                _ea,
                book_id,
            ...
            if self.enable_phase_mining and self._mining_phase == "CALIBRATE":
                quote_size = min(
                    self.mm_base_size * 0.25,
                    self.maintenance_order_size,
                )
            elif self.enable_phase_mining and self._mining_phase == "RESUME":
                quote_size = self.mm_base_size * self.weak_book_size_mult
            if book_id in weak_books:
                quote_size *= self.weak_book_size_mult
        ...
        if (
            ...
            for book_id in selection.alpha_books:
                ...
                if ea < self._effective_min_expected_alpha():
            ...
            for ea, book_id, pred, profile, archetype in alpha_candidates[
                ...
                if book_id in weak_books:
                    alpha_size = self._round_order_size(
                        alpha_size * self.weak_book_size_mult,
                        state.config.volumeDecimals,
                    )
    ...
    def _log_mm_strategy(self, stats: dict, regime: MarketRegime) -> None:
        ...
        if self.enable_floor_awareness or self.enable_phase_mining:
            bt.logging.info(
                ...
                f"weak_books={stats.get('weak_books_count', 0)} "
                f"left_tail_books={stats.get('left_tail_books_count', 0)} "
    ...
    def respond(self, state: MarketSimulationStateUpdate) -> FinanceAgentResponse:
        ...
        self._update_mining_phase(state, selection, regime)
