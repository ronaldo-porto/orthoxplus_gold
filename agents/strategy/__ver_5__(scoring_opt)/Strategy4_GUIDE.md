...
8. **July 2026 soft-floor awareness** — local `0.79*Kappa + 0.21*PnL` book scores, soft-floor taper proxy, earlier REDUCE_ONLY/LIQUIDATE on weak/left-tail books, floor-aware UCB rewards, and side EV gates that avoid weak-book symmetric quoting.
...
### Soft floor / relative rank (July 2026 scoring)

| Parameter | Default | Meaning |
|---|---:|---|
| `enable_floor_awareness` | true | Local soft-floor + weak/left-tail defense |
| `score_floor_guard_ratio` | 1.05 | Defend when proxy < ratio * local median |
| `weak_book_score_quantile` | 0.35 | Weak-book cutoff among own books |
| `weak_book_size_mult` | 0.50 | Size cut on weak books |
| `floor_percentile` / `floor_softness` | 50 / 0.5 | Mirror validator `apply_reward_floor` |
| `left_tail_markout_penalty` | 1.75 | Stronger UCB penalty on left-tail losses |
| `weak_book_markout_bonus` | 1.15 | Reward improving weak-book markouts |
| `inventory_risk_reward_penalty` | 0.35 | Penalize inventory-worsening fills without PnL |

Logs `[FLOOR_SCORE]` with `trading_proxy`, `soft_floor_proxy`, `score_to_median`, weak/left-tail counts, and risk-mode counts.
