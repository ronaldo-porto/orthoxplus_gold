...
class Strategy4(Strategy1):
    ...
    def _classify_weak_and_left_tail_books(
        ...
        for book_id, score in book_scores.items():
            if score <= weak_cut or (
                floor_threshold > 0.0 and score < floor_threshold
            ):
                weak.add(book_id)
            if score <= left_cut:
    ...
    def _evaluate_score_floor(
        ...
        # Smoothed change in the soft-floor-adjusted proxy; feeds UCB rewards.
        raw_delta = soft_floor_score - self._last_soft_floor_proxy
    ...
    def _floor_aware_policy_reward(
        ...
        """UCB reward: PnL + floor-score improvement; no volume-only credit."""
        reward = markout_bps
        mem = self._mem(item.book_id)
        # recent_pnl is FIFO realized PnL maintained by Strategy1. It is
        # normalized and clipped so it cannot swamp delayed markout evidence.
        realized_pnl_signal = self._clip(
            mem.recent_pnl / max(self._floor_pnl_scale, 1e-9), -1.0, 1.0
        )
        ...
        # Reward actions taken while the soft-floor-adjusted score is improving;
        # penalize actions that coincide with a deteriorating floor proxy.
        if self.enable_floor_awareness:
            reward += self._clip(
                self._soft_floor_proxy_delta * 50.0, -1.0, 1.0
            )
    ...
    def _phase_score_is_healthy(self, ratio: float) -> bool:
        score = self._last_trading_proxy
        median = self._last_floor_threshold
        # Before an internal median exists, require only a positive proxy so
        # sparse early history does not block calibration forever.
        if median <= 0.0:
            return score > 0.0
    ...
    def build_mm_strategy_instructions(
        ...
        if self.enable_floor_awareness or self.enable_phase_mining:
            ...
            internal_median = float(floor_state["floor_threshold"])
