...
A window is deteriorating when fast EWMAs diverge from slow EWMAs on
fill-hazard, actionable fill, markout, spread/regime, maker PnL, or dust
rate. DRIFT requires persistent evidence: multiple deteriorating windows
plus a minimum sample count. A single bad observation is not enough.
...
class DriftTracker:
    ...
    def observe(self, obs: DriftObservation) -> None:
        ...
        if obs.spread_bps is not None:
            ...
            updated = True
        if obs.maker_pnl is not None:
            self.maker_pnl.update(obs.maker_pnl, cfg.fast_alpha, cfg.slow_alpha)
            updated = True
