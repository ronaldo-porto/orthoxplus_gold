...
class Strategy3(DetailedTemplateAgent):
    ...
    def _estimate_book_trading_score(
        ...
        """Local book proxy using score inputs plus execution/risk quality."""
        kappa_score = self._normalize_kappa_score(profile.raw_kappa, mem)
        profile_pnl_score = self._normalize_pnl_score(float(profile.realized_pnl))
        memory_pnl_score = self._normalize_pnl_score(float(mem.recent_pnl))
        pnl_score = 0.70 * profile_pnl_score + 0.30 * memory_pnl_score
        score = (
            self.score_kappa_weight * kappa_score
            + self.score_pnl_weight * pnl_score
        )

        # Fill/win quality separates books with similar Kappa/PnL while keeping
        # the validator's 0.79/0.21 score blend as the primary component.
        fill_score = mem.fill_rate if mem.quote_count > 0 else 0.5
        outcomes = mem.win_count + mem.loss_count
        win_score = mem.win_rate if outcomes > 0 else 0.5
        score *= 0.75 + 0.12 * fill_score + 0.13 * win_score
        ...
        if mem.loss_streak >= self.toxic_loss_streak:
            score *= 0.55
        elif mem.loss_streak > 0:
            score *= max(0.65, 1.0 - 0.08 * mem.loss_streak)
        ...
        if regime is not None:
            ...
            score *= archetype_mult
            if self.is_toxic_book(profile.book_id, profile, archetype):
                score *= 0.65
    ...
    def _compute_floor_book_scores(
        ...
        for profile in profiles:
            ...
            self._sync_kappa_factor(mem, profile)
            scores[profile.book_id] = self._estimate_book_trading_score(
                profile, mem, regime
            )
        return scores
    ...
    def _classify_weak_and_left_tail_books(
        ...
        internal_median = self._percentile(values, 50.0)
        left_cut = self._percentile(
            values, self.left_tail_score_quantile * 100.0
        )
        profile_by_id = {profile.book_id: profile for profile in profiles}
        weak: set[int] = set()
        left_tail: set[int] = set()
        for book_id, score in book_scores.items():
            # Below either configured weak quantile or own-book median.
            if score <= weak_cut or score < internal_median:
                weak.add(book_id)
            profile = profile_by_id.get(book_id)
            mem = self._mem(book_id)
            negative_with_streak = (
                profile is not None
                and (profile.realized_pnl < 0.0 or mem.recent_pnl < 0.0)
                and mem.loss_streak > 0
            )
            far_below_median = (
                score <= left_cut
            ...
            if negative_with_streak or far_below_median:
    ...
    def _evaluate_score_floor(
        ...
        book_scores = self._compute_floor_book_scores(profiles, regime)
        trading_score = self._estimate_agent_trading_score(book_scores)
        floor_threshold = self._soft_floor_threshold(book_scores)
        internal_median = self._percentile(list(book_scores.values()), 50.0)
        ...
        return {
            ...
            "internal_book_score_median": internal_median,
    ...
    def _estimate_internal_score_state(
        ...
        for profile in selection.profiles:
            if profile.pnl_obs_count > 0 or profile.raw_kappa is not None:
                observed_books += 1
            if profile.realized_pnl < 0.0:
    ...
    def _phase_score_is_healthy(score_state: dict, ratio: float) -> bool:
        score = float(score_state.get("estimated_trading_score", 0.0))
        median = float(score_state.get("floor_threshold", 0.0))
        # Before an internal median exists, require a positive proxy rather than
        # blocking calibration solely because the history is still sparse.
        if median <= 0.0:
            return score > 0.0
        return score >= ratio * median
    ...
    def _phase_allows_new_risk(
        ...
        if mem.loss_streak > 0 or mem.recent_pnl < 0.0:
            return False
        alpha_floor = self.min_expected_alpha
    ...
    def build_mm_strategy_instructions(
        ...
        if self.enable_floor_awareness or self.enable_phase_mining:
            ...
            stats["trading_score_proxy"] = stats["estimated_trading_score"]
            stats["soft_floor_proxy"] = stats["estimated_soft_floor_score"]
            stats["score_to_median"] = stats["estimated_score_to_median"]
            internal_median = float(
                floor_state.get("internal_book_score_median", 0.0)
            )
            stats["score_to_internal_median_ratio"] = round(
                float(floor_state["estimated_trading_score"])
                / max(internal_median, 1e-12),
                6,
            ) if internal_median > 0.0 else 0.0
            stats["weak_books_count"] = len(floor_state["weak_books"])
            stats["left_tail_books_count"] = len(floor_state["left_tail_books"])
            stats["internal_score_proxy"] = stats["estimated_trading_score"]
    ...
    def _log_mm_strategy(self, stats: dict, regime: MarketRegime) -> None:
        ...
        if self.enable_floor_awareness or self.enable_phase_mining:
            bt.logging.info(
                ...
                f"trading_score_proxy={stats.get('estimated_trading_score', 0.0)} "
                f"soft_floor_proxy={stats.get('estimated_soft_floor_score', 0.0)} "
                f"score_to_median={stats.get('estimated_score_to_median', 0.0)} "
                f"score_to_median_ratio="
                f"{stats.get('score_to_internal_median_ratio', 0.0)} "
                f"weak_books_count={stats.get('weak_books_count', 0)} "
                f"left_tail_books_count={stats.get('left_tail_books_count', 0)} "
