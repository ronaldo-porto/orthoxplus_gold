...
@dataclass
class BookMemory:
    """Per-book trading memory — drives skip/size and fill-model calibration."""
...
class KappaMMStrategyAgent(DetailedTemplateAgent):
    ...
    def initialize(self) -> None:
        ...
        self.w_micro = float(getattr(cfg, "w_micro", 0.8))
    ...
    def _maintenance_allowed(
        ...
        if archetype == "MM_BOOK":
            return True
        if profile.tier in ("GREEN", "YELLOW"):
            return True
        return False
    ...
    def estimate_fill_probability(
        ...
        if book_id is not None:
            mem_fill = self._mem(book_id).fill_rate
            blend = 0.15
            p_buy = (1.0 - blend) * p_buy + blend * mem_fill
            p_sell = (1.0 - blend) * p_sell + blend * mem_fill
