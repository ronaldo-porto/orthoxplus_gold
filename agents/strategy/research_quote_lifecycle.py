...
from research_markout import MidHistory, MARKOUT_HORIZONS_MS as _HISTORY_HORIZONS
...
MARKOUT_HORIZONS_MS = _HISTORY_HORIZONS
NS_PER_MS = 1_000_000
...
class QuoteLifecycleStore:
    ...
    def record_mid(self, book: int, timestamp_ns: int | float | None, mid: float | None) -> None:
        self.history.record(book, timestamp_ns, mid)

    def record_book_mids(self, books: Mapping[Any, Any] | None, timestamp_ns: int | float | None) -> int:
        return self.history.record_books(books, timestamp_ns)
    ...
    def evaluate(
        ...
        *,
        now_ts: int,
        mids: Mapping[int, float | None] | None = None,
        history: MidHistory | None = None,
    ) -> list[MarkoutResult]:
        """Emit due markouts from timestamped history. Does not block."""
        due: list[MarkoutResult] = []
        kept: deque[PendingMarkout] = deque()
        now = int(now_ts)
        source = history if history is not None else self.history
        snapshot = dict(mids or {})
        for item in self.pending:
            age_ns = now - int(item.fill_ts)
            if age_ns < 0:
                kept.append(item)
                continue
            target = int(item.fill_ts) + ms_to_ns(item.horizon_ms)
            ready = now >= target
            missing = age_ns >= self.missing_after_ns
            hit = source.nearest_future_mid(item.book, target)
            future_ts = None
            future_mid = None
            if hit is not None:
                future_ts, future_mid = hit
            elif ready:
                snap = snapshot.get(int(item.book))
                if snap is None:
                    snap = snapshot.get(item.book)
                if snap is not None:
                    try:
                        px = float(snap)
