...
VALIDATOR_HISTORY_ALIGNMENT_VERSION = "validator_history_alignment_v4_14_1"


def _shift_scoring_timestamp(ts: Any, *, old_ts: int, new_ts: int) -> int | None:
    """Rebase one validator scoring timestamp across a simulation restart."""
    try:
        return int(new_ts) - (int(old_ts) - int(ts))
    except (TypeError, ValueError):
        return None


def rebase_observation_timestamps(
    raw: Mapping[Any, Any] | None, *, old_ts: int, new_ts: int, lookback_ns: int
) -> dict[int, list[int]]:
    """Mirror validator trade.shift_simulation_histories for Kappa observation timestamps.

    Shifted timestamps may legitimately be negative because the new simulation
    starts at timestamp 0 while retained lookback history lives before that 0.
    """
    cutoff = int(new_ts) - max(0, int(lookback_ns or 0))
    out: dict[int, list[int]] = {}
    for raw_book, raw_rows in dict(raw or {}).items():
        try:
            bid = int(raw_book)
        except (TypeError, ValueError):
            continue
        shifted: list[int] = []
        for raw_value in raw_rows or ():
            value = _shift_scoring_timestamp(raw_value, old_ts=old_ts, new_ts=new_ts)
            if value is None or value < cutoff or value > int(new_ts):
                continue
            shifted.append(value)
        if shifted:
            out[bid] = sorted(set(shifted))
    return out


def rebase_realized_pnl_events(
    raw: Mapping[Any, Any] | None, *, old_ts: int, new_ts: int, lookback_ns: int
) -> dict[int, list[tuple[int, float]]]:
    """Rebase restart-safe per-book realized-PnL events to the new sim clock."""
    cutoff = int(new_ts) - max(0, int(lookback_ns or 0))
    ...
    for raw_book, raw_rows in dict(raw or {}).items():
        ...
        for item in raw_rows or ():
            if not isinstance(item, (list, tuple)) or len(item) < 2:
                continue
            shifted = _shift_scoring_timestamp(item[0], old_ts=old_ts, new_ts=new_ts)
            try:
                pnl = float(item[1])
            except (TypeError, ValueError):
                continue
            if shifted is None or shifted < cutoff or shifted > int(new_ts) or not math.isfinite(pnl):
                continue
            if abs(pnl) <= 1e-15:
                continue
            rows.append((shifted, pnl))
        if rows:
...
def rebase_sparse_pnl_history(
    raw: Mapping[Any, Mapping[Any, Any]] | None, *, old_ts: int, new_ts: int, lookback_ns: int
) -> dict[int, dict[int, float]]:
    """Rebase miner-side sparse FIFO PnL buckets across a validator sim restart."""
    cutoff = int(new_ts) - max(0, int(lookback_ns or 0))
    out: dict[int, dict[int, float]] = {}
    for raw_ts, books in dict(raw or {}).items():
        shifted = _shift_scoring_timestamp(raw_ts, old_ts=old_ts, new_ts=new_ts)
        if shifted is None or shifted < cutoff or shifted > int(new_ts):
            continue
        clean: dict[int, float] = {}
        for raw_book, raw_pnl in dict(books or {}).items():
            try:
                clean[int(raw_book)] = float(raw_pnl)
            except (TypeError, ValueError):
                continue
        out[shifted] = clean
    return out
