...
# SN79 testnet launcher for Strategy1_Research V4.11 Aggressive Performance Research (Miner 1).
# Default PM2 name: sn79-m1 | Default Axon port: 8091
set -euo pipefail
# VERSION ARCHIVE: V4.11 launcher. Runtime still uses live agents/strategy/*.py.
...
grep -q 'RESEARCH_POLICY_VERSION = "hybrid_score_taker_v4_11"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not hybrid_score_taker_v4_11" >&2
  exit 1
}
...
research_enable_adaptive_ttl=1 research_enable_dust_escape=0 research_ttl_min_ms=250 research_ttl_max_ms=1500 research_quiet_ttl_ms=1000 \
research_enable_fast_candidate_screen=1 research_candidate_count=12 research_cohort_size=10 research_cohort_exploration_slots=1 \
...
research_positive_ev_min_order_override=1 research_positive_ev_min_safe_fraction=0.35 research_positive_ev_min_exit_fraction=0.45 research_positive_ev_min_trading_ev=0.05 \
research_quote_tighten_mult=0.85 research_quote_width_floor_mult=0.80 research_score_qualified_pnl_floor=0.0 research_score_qualified_kappa_floor=0.0 research_p95_target_ms=120 \
...
echo "[Strategy1_Research] version=hybrid_score_taker_v4_11"
