...
# V4.12.15 deployment compatibility checks.
...
# V4.12.15 inventory-liveness contract. Keep score-critical QUALIFIED/ONE_AWAY
# protected, permit bounded rescue only for TWO_AWAY/UNCOVERED, and park rather
# than dump positions that have already exceeded the absolute -12 bps floor.
PYTHONPATH="agents/strategy${PYTHONPATH:+:$PYTHONPATH}" python - <<'PYV41215LIVENESS'
...
if INVENTORY_LIVENESS_VERSION != "inventory_liveness_v4_12_15":
    raise SystemExit("ERROR: stale research_inventory_liveness.py for V4.12.15")
...
if not stage(0, failed=50, age=100).protected or not stage(1, failed=50, age=100).protected:
    raise SystemExit("ERROR: V4.12.15 QUALIFIED/ONE_AWAY protection failed")
if not stage(2, failed=3).maker_rescue_armed or not stage(3, age=8).maker_rescue_armed:
    raise SystemExit("ERROR: V4.12.15 bounded Maker rescue arming failed")
soft = rescue(stage(2, failed=8, age=16), -7.0, -10.0)
if not soft.authorized or soft.allowed_loss_floor_bps != -8.0:
    raise SystemExit("ERROR: V4.12.15 soft bounded rescue contract failed")
missed = rescue(stage(2, failed=8, age=16), -9.0, -20.0)
if missed.authorized or not missed.park or missed.reason != "LOSS_BEYOND_CURRENT_RESCUE_FLOOR":
    raise SystemExit("ERROR: V4.12.15 missed soft-window parking contract failed")
hard_stage = stage(3, failed=12, age=24, state="EMERGENCY", stop=True)
hard = rescue(hard_stage, -11.0, -20.0, state="EMERGENCY", stop=True)
if not hard.authorized or hard.allowed_loss_floor_bps != -12.0:
    raise SystemExit("ERROR: V4.12.15 emergency bounded rescue contract failed")
beyond = rescue(hard_stage, -13.0, -30.0, state="EMERGENCY", stop=True)
if beyond.authorized or not beyond.park:
    raise SystemExit("ERROR: V4.12.15 hard-floor parking contract failed")
one = rescue(stage(1, failed=50, age=500, state="EMERGENCY", stop=True), -5.0, -20.0, state="EMERGENCY", stop=True)
if one.authorized or one.park:
    raise SystemExit("ERROR: V4.12.15 ONE_AWAY loss subsidy leaked")
...
if due or reason != "PARKED_COOLDOWN":
    raise SystemExit("ERROR: V4.12.15 parked cooldown contract failed")
due, reason = parked_refresh_due(
    current_tick=120, last_refresh_tick=100, current_mid=100.01, last_mid=100.0,
    refresh_interval_ticks=20, material_touch_move_bps=8.0,
)
if not due or reason != "INTERVAL":
    raise SystemExit("ERROR: V4.12.15 parked refresh interval failed")
print("V4.12.15 inventory-liveness API OK")
PYV41215LIVENESS

# V4.12.16 Kappa conversion: under parked/total-cap pressure, fresh coverage
# yields to existing progress; score-ready ONE_AWAY work leads and receives a
# longer but sub-publish Maker TTL. No Taker or loss floor is widened.
PYTHONPATH="agents/strategy${PYTHONPATH:+:$PYTHONPATH}" python - <<'PYV41216KAPPA'
from research_execution_lanes import LaneBook, apply_kappa_conversion_pressure_gate, completion_sort_key
from research_quote_hysteresis import ONE_AWAY_CONVERSION_TTL_VERSION, one_away_stale_completion_ttl

rows = [
    LaneBook(book_id=1, observations_remaining=1, economics_ok=True, score_pnl_ready=True),
    LaneBook(book_id=2, observations_remaining=3, economics_ok=True, is_uncovered=True),
]
gated, suppressed, productive, reason = apply_kappa_conversion_pressure_gate(
    rows, parked_open_books=6, max_parked_open_books=6,
    total_open_books=9, max_total_open_books=12, reserve_total_slots=3,
)
if reason != "PARKED_CAP" or suppressed != {2} or productive != 1:
    raise SystemExit("ERROR: V4.12.16 Kappa pressure gate failed")
if not gated[0].entry_feasible or gated[1].entry_feasible:
    raise SystemExit("ERROR: V4.12.16 completion capacity reservation failed")
ttl, ttl_reason, used = one_away_stale_completion_ttl(
    chosen_ttl_ms=None, ttl_reason="STALE", completion_candidate=True,
    completion_samples=2, completion_target=3, trading_ev=0.05,
    market_regime="QUIET", min_ttl_ms=250.0, stale_ttl_ms=900.0,
)
if ONE_AWAY_CONVERSION_TTL_VERSION != "one_away_conversion_ttl_v4_12_16" or not used or ttl != 900.0:
    raise SystemExit("ERROR: V4.12.16 ONE_AWAY conversion TTL failed")
print("V4.12.16 Kappa-conversion API OK")
PYV41216KAPPA
# Internal runner: run_miner_multi.sh
# SN79 pre-deploy launcher for Strategy1_Research V4.12.16 Kappa Conversion (Miner 1).
...
grep -q 'RESEARCH_POLICY_VERSION = "kappa_conversion_v4_12_16_predeploy"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not kappa_conversion_v4_12_16_predeploy" >&2
  exit 1
}
...
research_enable_fast_candidate_screen=1 research_candidate_count=10 research_cohort_size=8 research_cohort_exploration_slots=1 research_max_open_books=6 research_max_active_open_books=6 research_max_total_open_books=12 research_max_parked_open_books=6 research_max_total_abs_base=3.0 research_kappa_conversion_pressure_enabled=1 research_kappa_conversion_reserve_slots=3 \
...
echo "[Strategy1_Research] version=kappa_conversion_v4_12_16_predeploy"
