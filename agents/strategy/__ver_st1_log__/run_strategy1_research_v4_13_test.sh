...
# V4.13 Simplified Kappa Productivity Engine deployment contract.
...
if KAPPA_PRODUCTIVITY_VERSION != "simplified_kappa_productivity_v4_13":
    raise SystemExit("ERROR: stale research_kappa_productivity.py for V4.13")
if scheduler_phase(40) != "BOOTSTRAP" or scheduler_phase(41) != "BALANCED" or scheduler_phase(80) != "DENSITY":
    raise SystemExit("ERROR: V4.13 breadth/density phase thresholds failed")
...
if not book115.core_candidate or kappa_state(observations=3, core=True) != STATE_CORE:
    raise SystemExit("ERROR: V4.13 productive CORE recycling failed")
if book98.execution_tier != TIER_INEFFICIENT or book98.core_candidate:
    raise SystemExit("ERROR: V4.13 inefficient-book demotion failed")
if priority_for_state(book115, phase="DENSITY") <= priority_for_state(book98, phase="DENSITY"):
    raise SystemExit("ERROR: V4.13 Kappa-productivity ranking failed")
print("V4.13 simplified Kappa productivity API OK")
PYV413PRODUCTIVITY
...
grep -q 'RESEARCH_POLICY_VERSION = "simplified_kappa_productivity_v4_13"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not simplified_kappa_productivity_v4_13" >&2
  exit 1
}
...
echo "[Strategy1_Research] version=simplified_kappa_productivity_v4_13"
echo "[Strategy1_Research] log_dir=$RESEARCH_DIR"

if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "V4.13 launcher preflight-only PASS"
  exit 0
fi
