...
class Strategy1_Research(Strategy1_Debug):
    RESEARCH_POLICY_VERSION = "simplified_kappa_productivity_v4_13"
    ...
    def _research_productivity_row(self, book_id: int) -> dict[str, int]:
        ...
        return table.setdefault(
            int(book_id),
            {"maker_quotes": 0, "maker_fills": 0, "contract_rejects": 0, "last_rt_tick": 0},
        )
    ...
    def _research_fast_screen(self, state) -> ScreenResult:
        ...
        for row in lane_rows:
            ...
            density_due = bool(
                ...
                and (int(row.book_id) in core_ids or obs < int(density_target))
            )
