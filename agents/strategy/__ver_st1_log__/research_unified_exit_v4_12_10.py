...
"""V4.12.10 unified hybrid Maker/Taker exit economics.

The existing protective -2 bps path remains unchanged. V4.12.10 adds one
strict stale-direct bridge for the already-authorized positive-EV SN79 path.
The bridge compares *incremental* exit EV (where the entry fee is sunk) while
still enforcing a hard total round-trip loss ceiling. This prevents the final
unified layer from silently vetoing a valid stale-capital release solely
because the already-paid entry fee is counted again as a switching cost.
"""
...
def bounded_stale_direct_bridge(
    *,
    legacy_direct_authorized: bool,
    positive_ev_authorized: bool,
    sn79_take: bool,
    legacy_taker_ev_bps: float,
    legacy_wait_ev_bps: float,
    actual_roundtrip_taker_net_bps: float,
    p_maker_fill: float,
    maker_fill_evidence: bool,
    failed_exit_count: int,
    inventory_age: float,
    min_failed_exits: int = 8,
    min_age_ticks: float = 16.0,
    max_maker_fill: float = 0.08,
    ev_advantage_bps: float = 0.50,
    roundtrip_loss_floor_bps: float = -12.0,
) -> bool:
    """Strict bridge from legacy positive-EV/SN79 authority into unified exit.

    This is deliberately *not* a generic negative-EV Taker path.  It requires:
    - an already-valid direct positive-EV authority;
    - SN79 utility agreement;
    - real Maker-fill evidence and very low fill probability;
    - both stale age and repeated failed exits;
    - non-negative incremental Taker EV that beats waiting; and
    - total round-trip PnL no worse than a hard -12 bps ceiling.

    The hard clamp means config can tighten this ceiling but cannot widen it.
    """
    hard_floor = max(-12.0, min(0.0, _finite(roundtrip_loss_floor_bps, -12.0)))
    taker_ev = _finite(legacy_taker_ev_bps, default=-1e9)
    wait_ev = _finite(legacy_wait_ev_bps, default=1e9)
    actual = _finite(actual_roundtrip_taker_net_bps, default=-1e9)
    fill = _clip01(p_maker_fill)
    failures = max(0, int(failed_exit_count))
    age = max(0.0, _finite(inventory_age))
    return bool(
        legacy_direct_authorized
        and positive_ev_authorized
        and sn79_take
        and maker_fill_evidence
        and failures >= max(1, int(min_failed_exits))
        and age + 1e-12 >= max(1.0, _finite(min_age_ticks, 16.0))
        and fill <= _clip01(max_maker_fill) + 1e-12
        and taker_ev >= -1e-12
        and taker_ev > wait_ev + max(0.0, _finite(ev_advantage_bps, 0.50)) + 1e-12
        and actual >= hard_floor - 1e-12
    )
