...
class Strategy1_Research(Strategy1_Debug):
    RESEARCH_POLICY_VERSION = "hybrid_score_taker_v4_11_2"
    ...
    def initialize(self) -> None:
        ...
        self.research_candidate_count = clamp_candidate_count(
            getattr(cfg, "research_candidate_count", DEFAULT_CANDIDATE_COUNT)
        )
        ...
        self.research_cohort_size = max(
            8, min(12, int(getattr(cfg, "research_cohort_size", 10)))
        )
        ...
        self.research_positive_ev_min_order_override = self._as_bool(
            getattr(cfg, "research_positive_ev_min_order_override", True)
        )
        ...
        self.RESEARCH_SPEED_VERSION = "response_speed_v1"
        ...
        self._enqueue({
            ...
            "candidate_count": int(getattr(self, "research_candidate_count", 12)),
            "cohort_size": int(getattr(self, "research_cohort_size", 10)),
            "cohort_exploration_slots": int(getattr(self, "research_cohort_exploration_slots", 1)),
            "positive_ev_min_order_override": bool(getattr(self, "research_positive_ev_min_order_override", True)),
    ...
    def _research_note_realized_observation(self, book_id: int, timestamp=None) -> None:
        ...
        if ts is not None:
            try:
                ...
                rows = self._research_persisted_observation_timestamps.setdefault(bid, [])
        ...
        if getattr(self, "research_enable_min_order_admission", True):
            admission = admit_minimum_order(
                ...
                enable_positive_ev_override=bool(
                    getattr(self, "research_positive_ev_min_order_override", True)
                ),
