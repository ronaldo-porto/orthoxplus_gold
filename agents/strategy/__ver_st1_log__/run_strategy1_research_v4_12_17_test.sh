...
# V4.12.17 deployment compatibility checks.
...
# V4.12.17 Kappa Flywheel Foundation deployment contract.
# Preserve QUALIFIED/ONE_AWAY loss protection, make the -8 -> -12 bps rescue
# transition event-driven, treat parking as a classification rather than a
# six-slot blocker, keep one exploration slot under true total-headroom
# pressure, and keep velocity-stale ONE_AWAY Maker TTL short.
PYTHONPATH="agents/strategy${PYTHONPATH:+:$PYTHONPATH}" python - <<'PYV41217FLYWHEEL'
...
if INVENTORY_LIVENESS_VERSION != "inventory_liveness_v4_12_17":
    raise SystemExit("ERROR: stale research_inventory_liveness.py for V4.12.17")
if KAPPA_FLYWHEEL_VERSION != "kappa_flywheel_v4_12_17":
    raise SystemExit("ERROR: stale research_kappa_flywheel.py for V4.12.17")
if ONE_AWAY_CONVERSION_TTL_VERSION != "one_away_velocity_stale_ttl_v4_12_17":
    raise SystemExit("ERROR: stale research_quote_hysteresis.py for V4.12.17")
...
# Score-critical positions remain protected from the liveness loss subsidy.
if not stage(0, failed=50, age=100).protected or not stage(1, failed=50, age=100).protected:
    raise SystemExit("ERROR: V4.12.17 QUALIFIED/ONE_AWAY protection failed")

# Soft bounded rescue remains valid.
soft = rescue(stage(2, failed=8, age=16), -7.0, -10.0)
if not soft.authorized or soft.allowed_loss_floor_bps != -8.0:
    raise SystemExit("ERROR: V4.12.17 soft bounded rescue contract failed")

# Critical V4.12.17 correction: crossing below -8 while still >= -12 is the
# emergency event *now*, even before age/failed-exit hard-window counters mature.
price_hard = rescue(stage(2, failed=3, age=5), -9.1, -10.0)
if (
    not price_hard.authorized
    or price_hard.park
    or price_hard.allowed_loss_floor_bps != -12.0
    or price_hard.reason != "PRICE_HARD_WINDOW_RESCUE"
):
    raise SystemExit("ERROR: V4.12.17 event-driven -8/-12 hard rescue failed")

# Never dump below the absolute -12 bps ceiling.
beyond = rescue(stage(2, failed=20, age=30, state="EMERGENCY", stop=True), -12.1, -30.0, state="EMERGENCY", stop=True)
if beyond.authorized or not beyond.park or beyond.reason != "LOSS_BEYOND_HARD_FLOOR":
    raise SystemExit("ERROR: V4.12.17 hard-floor parking contract failed")

# Park labels alone are no longer a scarce capacity resource.
rows = [
    LaneBook(book_id=1, observations_remaining=1, economics_ok=True),
    LaneBook(book_id=2, observations_remaining=3, is_uncovered=True, economics_ok=True),
]
gated, suppressed, productive, reason = apply_kappa_conversion_pressure_gate(
    rows, parked_open_books=12, max_parked_open_books=6,
    total_open_books=2, max_total_open_books=12, reserve_total_slots=3,
    exploration_slots=1, enabled=True,
)
if reason != "NO_PRESSURE" or suppressed or not all(row.entry_feasible for row in gated):
    raise SystemExit("ERROR: V4.12.17 parked-classification liveness contract failed")

# Under true total-headroom pressure, completion leads but one fresh exploration
# slot remains fail-open so an unfillable completion backlog cannot freeze breadth.
rows = [
    LaneBook(book_id=1, observations_remaining=1, economics_ok=True),
...
if reason != "TOTAL_HEADROOM" or productive != 2 or suppressed != {11}:
    raise SystemExit("ERROR: V4.12.17 total-headroom pressure gate failed")
if not by_id[10].entry_feasible or by_id[11].entry_feasible:
    raise SystemExit("ERROR: V4.12.17 exploration fail-open contract failed")

# Qualified low-density/core-maintenance work must re-enter the completion lane.
density = LaneBook(
    book_id=42, observations_remaining=0, score_qualified=True,
...
if classify_execution_lane(density) != LANE_COMPLETION:
    raise SystemExit("ERROR: V4.12.17 qualified-density completion lane failed")

# Breadth-to-density phase boundaries match SN79 padded-median objectives.
if flywheel_phase(40) != PHASE_BOOTSTRAP or flywheel_phase(41) != PHASE_BREADTH or flywheel_phase(80) != PHASE_DENSITY:
    raise SystemExit("ERROR: V4.12.17 flywheel phase boundary failed")
if (phase_density_target(PHASE_BOOTSTRAP), phase_density_target(PHASE_BREADTH), phase_density_target(PHASE_DENSITY)) != (6, 12, 50):
    raise SystemExit("ERROR: V4.12.17 density target contract failed")

# Rolling realized-PnL evidence must survive restart serialization; never infer
# score-PnL readiness from a reset EMA default.
events = {}
...
if stats.nonzero_count != 2 or abs(stats.realized_sum - 0.07) > 1e-12:
    raise SystemExit("ERROR: V4.12.17 rolling realized-PnL restart contract failed")

# Velocity STALE is an adverse-selection safety reason, so the ONE_AWAY override
# stays at the short 250 ms rescue rather than the old 900 ms predeploy value.
...
if not used or ttl != 250.0 or ttl_reason != "ONE_AWAY_VELOCITY_STALE_SHORT":
    raise SystemExit("ERROR: V4.12.17 velocity-stale ONE_AWAY TTL contract failed")

# Parked refresh remains bounded rather than requoted every response.
due, reason = parked_refresh_due(
    current_tick=110, last_refresh_tick=100, current_mid=100.01, last_mid=100.0,
    refresh_interval_ticks=20, material_touch_move_bps=8.0,
)
if due or reason != "PARKED_COOLDOWN":
    raise SystemExit("ERROR: V4.12.17 parked cooldown contract failed")

print("V4.12.17 Kappa Flywheel Foundation API OK")
PYV41217FLYWHEEL
# Internal runner: run_miner_multi.sh
# SN79 launcher for Strategy1_Research V4.12.17 Kappa Flywheel Foundation (Miner 1).
...
grep -q 'RESEARCH_POLICY_VERSION = "kappa_flywheel_v4_12_17"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not kappa_flywheel_v4_12_17" >&2
  exit 1
}
...
research_enable_fast_candidate_screen=1 research_candidate_count=10 research_cohort_size=8 research_cohort_exploration_slots=1 research_max_open_books=6 research_max_active_open_books=6 research_max_total_open_books=12 research_max_parked_open_books=6 research_max_total_abs_base=3.0 research_kappa_conversion_pressure_enabled=1 research_kappa_conversion_reserve_slots=3 research_kappa_exploration_slots=1 research_kappa_flywheel_enabled=1 \
...
echo "[Strategy1_Research] version=kappa_flywheel_v4_12_17"
echo "[Strategy1_Research] log_dir=$RESEARCH_DIR"

if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "V4.12.17 launcher preflight-only PASS"
  exit 0
fi
