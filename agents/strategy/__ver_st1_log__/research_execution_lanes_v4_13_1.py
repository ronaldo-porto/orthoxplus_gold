...
def completion_sort_key(book: LaneBook) -> tuple:
    ...
    # Recycling bootstrap competes with ONE_AWAY rather than waiting behind
    ...
    *,
    ...
    for lane in (LANE_REALIZATION, LANE_COMPLETION, LANE_COVERAGE):
        ...
        if lane == LANE_COMPLETION and take > 0:
            # V4.13.1 reserves one completion slot for a qualified book that
            # produced its first clean fresh RT. This breaks the CORE chicken-
            # and-egg loop without changing lane budgets.
            bridge = next(
                (row for row in pools[lane] if bool(getattr(row, "recycling_candidate", False))),
                None,
            )
            if bridge is not None and all(int(row.book_id) != int(bridge.book_id) for row in chosen_rows):
                chosen_rows = [bridge] + [
                    row for row in chosen_rows
                    if int(row.book_id) != int(bridge.book_id)
                ][: max(0, take - 1)]
