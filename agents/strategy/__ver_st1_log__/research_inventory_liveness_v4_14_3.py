...
BOUNDED_LOSS_ESCAPE_VERSION = "two_stage_bounded_loss_escape_v4_14_3"
...
def bounded_loss_escape_reason(
    *,
    taker_net_bps: float,
    ...
    soft_maker_failed_exits: int = 4,
    soft_maker_min_age_ticks: float = 8.0,
) -> str:
    """Return the V4.14.3 two-stage bounded-loss decision.

    HARD_ESCAPE protects against the old multi-hour parked tail. SOFT_MAKER_HOLD
    prevents a young profitable Maker exit from being converted into an
    unnecessary negative Kappa observation merely because Taker crossed -8 bps.
    ...
    trigger = max(hard, min(soft, _finite(hard_trigger_bps, -18.0)))
    ...
    if drawdown + 1e-12 < max(0.0, _finite(min_drawdown_bps, 2.0)):
        return "NO_DRAWDOWN"

    # Once the loss reaches the hard stage, capital recycling outranks Maker
    # optimism. This preserves V4.14's tail-risk fix.
    if taker <= trigger + 1e-12:
        return "HARD_ESCAPE"

    # In the soft -8..-18 zone, keep a genuinely profitable Maker exit while
    # the lifecycle is still young. Existing failed-exit/age knobs bound the hold.
    maker = _finite(maker_net_bps, default=-1e9) if maker_net_bps is not None else -1e9
    maker_floor = max(0.0, _finite(maker_positive_floor_bps, 1.0))
    failures = max(0, int(failed_exit_count))
    if (
        maker >= maker_floor - 1e-12
        and failures < max(1, int(soft_maker_failed_exits))
        and age + 1e-12 < max(1.0, _finite(soft_maker_min_age_ticks, 8.0))
    ):
        return "SOFT_MAKER_HOLD"
    return "SOFT_ESCAPE"
...
def bounded_loss_escape_applies(
    *,
    ...
    reason = bounded_loss_escape_reason(
        taker_net_bps=taker_net_bps,
        peak_taker_net_bps=peak_taker_net_bps,
        inventory_age=inventory_age,
        maker_net_bps=maker_net_bps,
        failed_exit_count=failed_exit_count,
        enabled=enabled,
        min_age_ticks=min_age_ticks,
        soft_floor_bps=soft_floor_bps,
        hard_trigger_bps=hard_trigger_bps,
        hard_floor_bps=hard_floor_bps,
        min_drawdown_bps=min_drawdown_bps,
        maker_positive_floor_bps=maker_positive_floor_bps,
        soft_maker_failed_exits=soft_maker_failed_exits,
        soft_maker_min_age_ticks=soft_maker_min_age_ticks,
    )
    return reason in {"SOFT_ESCAPE", "HARD_ESCAPE"}


def counts_against_productive_open_cap(
    *, has_inventory: bool, is_liveness_parked: bool, is_dust: bool
) -> bool:
    """Dust stays risk-accounted but must not consume scarce productive slots."""
    return bool(has_inventory and not is_liveness_parked and not is_dust)
