...
# V4.12.12 deployment compatibility checks.
...
# V4.12.12 reject-aware post-only retry guard.
PYTHONPATH="agents/strategy${PYTHONPATH:+:$PYTHONPATH}" python - <<'PYV41212CONTRACT'
...
if CONTRACT_GUARD_VERSION != "post_only_contract_guard_v4_12_12":
    raise SystemExit("ERROR: stale research_contract_guard.py for V4.12.12")
s = register_contract_reject(None, current_tick=100)
if not guard_should_skip(s, current_tick=101):
    raise SystemExit("ERROR: V4.12.12 immediate retry cooldown failed")
if guard_should_skip(s, current_tick=102):
    raise SystemExit("ERROR: V4.12.12 cooldown did not release")
p = guarded_post_only_price(
    side="sell", original_price=309.03, best_bid=309.02, best_ask=309.40,
    tick_size=0.01, reject_streak=1,
)
if p is None or abs(p - 309.41) > 1e-12:
    raise SystemExit("ERROR: V4.12.12 SELL safe reprice failed")
p = guarded_post_only_price(
    side="buy", original_price=309.39, best_bid=309.02, best_ask=309.40,
    tick_size=0.01, reject_streak=1,
)
if p is None or abs(p - 309.01) > 1e-12:
    raise SystemExit("ERROR: V4.12.12 BUY safe reprice failed")
print("V4.12.12 post-only contract guard API OK")
PYV41212CONTRACT
# Internal runner: run_miner_multi.sh
# SN79 testnet launcher for Strategy1_Research V4.12.12 Post-Only Reject Guard Research (Miner 1).
...
grep -q 'RESEARCH_POLICY_VERSION = "post_only_reject_guard_v4_12_12"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not post_only_reject_guard_v4_12_12" >&2
  exit 1
}
...
echo "[Strategy1_Research] version=post_only_reject_guard_v4_12_12"
