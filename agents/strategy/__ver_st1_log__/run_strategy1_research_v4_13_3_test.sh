...
# V4.13.3 focused CORE_PROBE lane-consistency deployment contract.
...
from research_execution_lanes import (
    LaneBook, LaneBudgets, LANE_COMPLETION, execution_completion_candidate,
    select_lane_candidates,
)
from research_inventory_liveness import (
    FRESH_MAKER_GRACE_VERSION, classify_liveness_stage, evaluate_bounded_rescue,
    fresh_maker_grace_applies,
)

if KAPPA_PRODUCTIVITY_VERSION != "simplified_kappa_productivity_v4_13_3":
    raise SystemExit("ERROR: stale research_kappa_productivity.py for V4.13.3")
...
if alloc.by_lane[LANE_COMPLETION] != [83]:
    raise SystemExit("ERROR: V4.13.3 CORE_PROBE slot reservation failed")
if not execution_completion_candidate(
    inventory_flat=True,
    core_probe_candidate=True,
    legacy_completion_candidate=False,
):
    raise SystemExit("ERROR: V4.13.3 CORE_PROBE execution lane identity failed")
print("V4.13.3 CORE_PROBE lane consistency + inherited Maker grace API OK")
PYV413PRODUCTIVITY
...
grep -q 'RESEARCH_POLICY_VERSION = "simplified_kappa_productivity_v4_13_3"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not simplified_kappa_productivity_v4_13_3" >&2
  exit 1
}
...
echo "[Strategy1_Research] version=simplified_kappa_productivity_v4_13_3"
echo "[Strategy1_Research] log_dir=$RESEARCH_DIR"

if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "V4.13.3 launcher preflight-only PASS"
  exit 0
fi
