...
class Strategy1_Research(Strategy1_Debug):
    ...
    def initialize(self) -> None:
        ...
        # V4.14.2: parked count is telemetry / realization pressure only. Fresh
        # COVERAGE remains possible whenever the authoritative total-open and
        # aggregate-BASE caps still have real headroom.
        ...
        self.research_bounded_loss_escape_hard_trigger_bps = max(
            self.research_bounded_loss_escape_floor_bps,
            min(
                float(getattr(self, "research_liveness_soft_taker_floor_bps", -8.0)),
                float(getattr(cfg, "research_bounded_loss_escape_hard_trigger_bps", -18.0)),
            ),
        )
    ...
    def _research_execution_lane_budgets(self):
        if bool(getattr(self, "research_kappa_productivity_enabled", True)):
            ...
            if phase == PRODUCTIVITY_PHASE_BALANCED:
                return normalize_lane_budgets(
                    coverage_slots=3, completion_slots=4, realization_slots=3,
                    shared_overflow_slots=1,
                )
            return normalize_lane_budgets(
    ...
    def _research_fast_screen(self, state) -> ScreenResult:
        ...
        for book_id, book in (getattr(state, "books", None) or {}).items():
            ...
            if has_inv:
                ...
                if is_dust:
                    dust_nonflat += 1
                if counts_against_productive_open_cap(
                    has_inventory=True,
                    is_liveness_parked=is_liveness_parked,
                    is_dust=is_dust,
                ):
                    active_nonflat += 1
            else:
        ...
        for row in lane_rows:
            ...
            raw_kappa = getattr(row, "raw_kappa", None)
            wide_density_due = wide_kappa_density_due(
                observations=obs,
                raw_kappa=raw_kappa,
                kappa_eligible=bool(getattr(row, "kappa_eligible", False)),
                execution_tier=str(getattr(row, "kappa_productivity_tier", "UNKNOWN") or "UNKNOWN"),
                min_density_observations=int(getattr(
                    self, "research_wide_kappa_min_density_observations", 6
                )),
                preferred_density_observations=int(getattr(
                    self, "research_wide_kappa_preferred_density_observations", 8
                )),
                raw_kappa_target=float(getattr(self, "research_wide_kappa_raw_target", 0.35)),
            )
            density_due = bool(
        ...
        self._research_inventory_lane_diag = {
            ...
            "productive_active_nonflat_inventory": int(active_nonflat),
