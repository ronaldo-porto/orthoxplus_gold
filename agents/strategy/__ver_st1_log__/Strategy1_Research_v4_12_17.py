...
class Strategy1_Research(Strategy1_Debug):
    RESEARCH_POLICY_VERSION = "kappa_flywheel_v4_12_17"
    ...
    RESEARCH_LANES_VERSION = "execution_lanes_v6_kappa_flywheel"
    ...
    def initialize(self) -> None:
        ...
        self.research_parked_refresh_ticks = max(
            10, min(30, int(getattr(cfg, "research_parked_refresh_ticks", 20)))
        )
    ...
    def _research_fast_screen(self, state) -> ScreenResult:
        ...
        for book_id, book in (getattr(state, "books", None) or {}).items():
            ...
            pnl_authority_complete = bool(
                int(samples) <= 0 or int(rolling_econ.nonzero_count) >= int(samples)
            )
            entry_feasible = True
            ...
            flywheel_priority = (
                ...
                + 0.21 * (1.0 if pnl_rank_value >= 0.0 else -1.0)
        ...
        fly_phase = flywheel_phase(len(score_qualified_ids))
        density_target = phase_density_target(fly_phase)
        core_limit = phase_core_limit(fly_phase)
        qualified_pool = [
            row for row in lane_rows
            if bool(getattr(row, "score_qualified", False))
            and bool(getattr(row, "score_pnl_ready", False))
    ...
    def _research_apply_unified_exit(
        ...
        *,
        ...
        if liveness_rescue is not None and unified.action == UNIFIED_KEEP_MAKER:
            if bool(liveness_rescue.authorized):
                ...
                try:
                    ...
                    pass
            elif bool(liveness_touch_maker_blocked):
            ...
            elif bool(liveness_rescue.park):
                liveness_parked_now = self._research_park_inventory(
                    int(book_id), inventory=inventory, book=book,
                    score_state=str(liveness_stage.score_state),
                    taker_net_bps=float(taker_net),
            ...
            elif liveness_stage.taker_rescue_armed:
                self._research_inventory_rescue_blocked = int(
                    getattr(self, "_research_inventory_rescue_blocked", 0) or 0
                ) + 1
                try:
    ...
    def _research_manage_realization(
        ...
        if newly_parked_liveness:
            # Do not churn one more Maker order in the same response that parked
            # the position. Any already-accepted order is left alone; the next
            # reconsideration is interval/touch/risk driven.
            try:
                self._emit(
                    "PARKED_HOLD", force=True, tick=getattr(self, "_tick", None),
                    book=int(book_id), reason="NEWLY_PARKED_NO_REQUOTE",
                    selected_action=decision.selected_action,
                    inventory=float(inventory.net_base),
                )
            except Exception:
    ...
    def _console_allowed(self, r: dict[str, Any]) -> bool:
        ...
        if typ in {"ERROR", "RUN_SUMMARY", "RESEARCH_CONFIG", "DEBUG_CONFIG", "POSITION", "POSITION_GUARD", "ACTIONABLE_FILL", "REGIME", "SCORE_REGIME", "STALE_RESCUE", "RESCUE_ARMED", "RESCUE_MAKER", "RESCUE_TAKER", "RESCUE_BLOCKED", "PARK_POSITION", "UNPARK_POSITION", "PARK_REFRESH", "PARKED_HOLD", "REALIZATION", "EXIT_URGENCY", "LADDER", "TAKER_DECISION", "EXIT_QTY", "INVENTORY_STATE", "ENTRY_SIZE", "QUOTE", "EXEC_PROB", "FILL", "MARKOUT", "FILL_CAL", "RANK", "KAPPA", "SCHED", "LANES", "CANCEL_DECISION", "TTL", "RESPOND_TIMING", "VOLUME_CAP", "HYBRID", "HYBRID_SUMMARY", "SESSION_TRANSITION"}:
            return True
        if typ == "ORDER_LIFECYCLE":
            phase = str(r.get("phase", "")).upper()
