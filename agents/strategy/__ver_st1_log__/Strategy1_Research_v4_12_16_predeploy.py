...
class Strategy1_Research(Strategy1_Debug):
    RESEARCH_POLICY_VERSION = "kappa_conversion_v4_12_16_predeploy"
    ...
    RESEARCH_LANES_VERSION = "execution_lanes_v5_kappa_pressure"
    ...
    def initialize(self) -> None:
        ...
        # V4.12.16: when parked/total risk capacity is scarce, reserve the
        # remaining slots for books already one or two round trips from score.
        ...
        self.research_one_away_stale_ttl_ms = max(
            ...
            min(990.0, float(getattr(cfg, "research_one_away_stale_ttl_ms", 900.0))),
        )
        ...
        self._enqueue({
            ...
            "one_away_stale_ttl_ms": float(getattr(self, "research_one_away_stale_ttl_ms", 900.0)),
    ...
    def _research_park_inventory(
        ...
        *,
        ...
        if existing is None:
            ...
            if len(table) >= max(0, cap):
                try:
                    self._emit(
                        "RESCUE_BLOCKED", force=True, tick=now, book=bid,
                        reason="PARK_CAP", parked_open_books=len(table),
                        max_parked_open_books=cap, score_state=score_state,
    ...
    def _research_apply_unified_exit(
        ...
        *,
        ...
        if liveness_maker_active:
            ...
            try:
                self._emit(
                    ...
                    wait_ev_bps=float(wait_ev), **liveness_stage.as_log(),
                )
            except Exception:
                pass
        peaks = getattr(self, "_research_peak_taker_net_bps", None)
        ...
        bridge = False
        # A v4.12.16 park decision is authoritative for this response. The
        ...
        if getattr(self, "research_enable_adaptive_ttl", False):
            ...
            if chosen is None:
                chosen, ttl_reason, stale_override = one_away_stale_completion_ttl(
                    ...
                    stale_ttl_ms=float(getattr(self, "research_one_away_stale_ttl_ms", 900.0)),
                )
                chosen_ttl_ms = chosen
            if chosen is None:
