...
# V4.13.2 focused Maker-Grace + CORE_PROBE deployment contract.
...
from research_execution_lanes import LaneBook, LaneBudgets, LANE_COMPLETION, select_lane_candidates
from research_inventory_liveness import (
    FRESH_MAKER_GRACE_VERSION, classify_liveness_stage, evaluate_bounded_rescue,
    fresh_maker_grace_applies,
)

if KAPPA_PRODUCTIVITY_VERSION != "simplified_kappa_productivity_v4_13_2":
    raise SystemExit("ERROR: stale research_kappa_productivity.py for V4.13.2")
...
if alloc.by_lane[LANE_COMPLETION] != [83]:
    raise SystemExit("ERROR: V4.13.2 CORE_PROBE slot reservation failed")
print("V4.13.2 Maker grace + CORE_PROBE API OK")
PYV413PRODUCTIVITY
...
grep -q 'RESEARCH_POLICY_VERSION = "simplified_kappa_productivity_v4_13_2"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not simplified_kappa_productivity_v4_13_2" >&2
  exit 1
}
...
echo "[Strategy1_Research] version=simplified_kappa_productivity_v4_13_2"
echo "[Strategy1_Research] log_dir=$RESEARCH_DIR"

if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "V4.13.2 launcher preflight-only PASS"
  exit 0
fi
