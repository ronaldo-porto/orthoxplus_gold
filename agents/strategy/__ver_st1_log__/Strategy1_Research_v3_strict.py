...
"""Strategy1 research V3 Strict: execution-quality, dust-scheduler, and sparse-tier fixes.
...
16. Sub-minimum exact inventory is parked outside the finite inventory-management pool, remains
    fully accounted for, is quarantined from new MM/maintenance risk, and emits transition/
    heartbeat telemetry instead of consuming a management slot every tick.
...
class Strategy1_Research(Strategy1_Debug):
    ...
    def initialize(self) -> None:
        ...
        # --------------------------------------------------------------
        # V3 Strict execution-quality fixes, all derived from the completed
        ...
        self._enqueue({
            ...
            "policy_version": "deadlock_fix_v3_strict",
    ...
    def _refresh_dust_state(
        ...
        *,
        ...
        V3 Strict does not synthesize an exchange-illegal exact close and does
    ...
    def classify_book_archetype(
        ...
        # archetype condition; otherwise one regime bit poisons all books.
            ...
            # V3 Strict bridge: a book that already earned YELLOW history but has
    ...
    def _inventory_needs_management(self, inventory: InventorySnapshot) -> bool:
        ...
        # V3 Strict: parked dust is exchange-nonactionable and must NOT
        # consume max_managed_books_per_tick. It stays quarantined from fresh
        # MM/maintenance through is_toxic_book().
        if (
            self.research_dust_park_enabled
    ...
    def _allows_aggressive_close(
        ...
        if (
            ...
            # V3 Strict deliberately removes age-only / close-score-only market
    ...
    def _place_skewed_quotes(
        ...
        # Parent Strategy1 slices candidates before quote-level gates. V3 Strict
    ...
    def _emit_book_decision(
        ...
        if record.get("dust_quarantine") and reason == "TOXIC_BOOK":
