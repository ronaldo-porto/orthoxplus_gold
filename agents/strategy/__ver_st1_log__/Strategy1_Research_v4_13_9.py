...
class Strategy1_Research(Strategy1_Debug):
    RESEARCH_POLICY_VERSION = "simplified_kappa_productivity_v4_13_9"
    ...
    def initialize(self) -> None:
        ...
        self.research_max_total_open_books = max(
            ...
            min(12, int(getattr(cfg, "research_max_total_open_books", 12))),
        )
        # V4.12.17: this is a telemetry/warning threshold only. PARKED is a
        # classification, not a scarce slot resource. Real risk is constrained
        # below by total open books and aggregate BASE exposure.
        self.research_max_parked_open_books = max(
            0,
            min(
                self.research_max_total_open_books,
                int(getattr(cfg, "research_max_parked_open_books", 6)),
            ),
        )
        ...
        self.research_positive_maker_veto_max_failed_exits = max(
            1, int(getattr(cfg, "research_positive_maker_veto_max_failed_exits", 3))
        )
        ...
        self._enqueue({
            ...
            "positive_maker_veto_max_failed_exits": int(getattr(self, "research_positive_maker_veto_max_failed_exits", 3)),
    ...
    def _console_allowed(self, r: dict[str, Any]) -> bool:
        ...
        if bool(getattr(self, "research_compact_console", True)):
            ...
            if typ in {
                ...
                "RUN_SUMMARY", "STALE_RESCUE", "RESCUE_TAKER", "PARK_POSITION",
        ...
        if typ in {"ERROR", "RUN_SUMMARY", "RESEARCH_CONFIG", "DEBUG_CONFIG", "POSITION", "POSITION_GUARD", "ACTIONABLE_FILL", "REGIME", "SCORE_REGIME", "STALE_RESCUE", "RESCUE_ARMED", "RESCUE_MAKER", "RESCUE_TAKER", "RESCUE_BLOCKED", "PARK_POSITION", "PARK_PROTECTED", "PARK_LIVENESS", "UNPARK_POSITION", "PARK_REFRESH", "PARKED_HOLD", "PROTECTED_REFRESH_BLOCKED", "PROTECTED_REFRESH_SUBMIT", "PARK_REFRESH_SUBMIT", "PARK_CANCEL", "PARK_CANCEL_BLOCKED", "REALIZATION", "EXIT_URGENCY", "LADDER", "TAKER_DECISION", "EXIT_QTY", "INVENTORY_STATE", "ENTRY_SIZE", "QUOTE", "EXEC_PROB", "FILL", "MARKOUT", "FILL_CAL", "RANK", "KAPPA", "SCHED", "LANES", "CANCEL_DECISION", "TTL", "RESPOND_TIMING", "VOLUME_CAP", "HYBRID", "HYBRID_SUMMARY", "SESSION_TRANSITION"}:
            return True
        if typ == "ORDER_LIFECYCLE":
            phase = str(r.get("phase", "")).upper()
