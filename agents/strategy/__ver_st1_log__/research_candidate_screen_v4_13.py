...
"""Research V4.3 Phase 1.9: cheap two-stage candidate screening.

128 books → cheap screen → always keep inventory / one-away Kappa /
risk-critical / dust-critical books → fill remaining slots by cheap score →
full Strategy1 prediction on the selected set (default 16–24).

Unchanged top-of-book features may be reused. Callers keep a full-universe
predict fallback when the screen is off or fails.
"""
...
def book_touch_fingerprint(book: Any) -> tuple[Any, ...] | None:
    ...
    try:
        bid = bids[0]
        ask = asks[0]
        # V4.13 hot-path fingerprint: top-of-book only.  Touch cache invalidation
        # must not traverse/size the event stream or deeper levels on all 128 books.
        return (
