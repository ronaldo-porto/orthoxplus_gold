...
class LaneBook:
    ...
    # V4.12.16: raw observation completion is not sufficient for the validator
    # score. Prefer ONE_AWAY books whose realized economics are already at or
    # above the score floor, then the closest negative book that can recover.
...
def apply_kappa_conversion_pressure_gate(
    ...
    *,
    ...
    """Reserve scarce risk capacity for existing Kappa progress.

    When parked inventory is full, or total-position headroom is nearly gone,
    opening another zero-observation book has lower score conversion value than
    completing a safe ONE_AWAY/TWO_AWAY book. This gate suppresses only flat
    fresh-coverage entries. Existing inventory, risk handling, incomplete books,
    and qualified deadline refreshes are never blocked.
    """
    rows = list(books)
    productive = [
        row for row in rows
    ...
    parked = max(0, int(parked_open_books or 0))
    parked_cap = max(0, int(max_parked_open_books or 0))
    ...
    parked_full = parked_cap > 0 and parked >= parked_cap
    headroom_tight = total_cap > 0 and headroom <= reserve
    if not parked_full and not headroom_tight:
        return rows, set(), productive_count, "NO_PRESSURE"

    reason = "PARKED_CAP" if parked_full else "TOTAL_HEADROOM"
    suppressed: set[int] = set()
    out: list[LaneBook] = []
    for row in rows:
        ...
        if block:
            ...
            out.append(row)
    return out, suppressed, productive_count, reason
...
def completion_sort_key(book: LaneBook) -> tuple:
    """V4.12.8 earliest-deadline / fewest-RT-to-score ordering.

    A ONE_AWAY book and a qualified refresh both need one additional round trip
    to create/preserve one qualified book, so they share the same completion
    cost.  Critical rolling deadlines lead within that cost class.  TWO_AWAY
    books follow because they require two completions.  This prevents broad
    maintenance refresh from crowding out score progress while still protecting
    books that are genuinely close to rolling expiry.
    """
    remaining = max(0, int(book.observations_remaining or 0))
    deadline = max(
    ...
    # Within equal cost, a critical deadline leads.  When deadlines are equally
    # non-critical, creating a new qualified book (ONE_AWAY) leads an early
    # refresh so we keep growing breadth instead of churning the same books.
    type_rank = 0 if remaining == 1 else (1 if refresh else 2)
...
def coverage_sort_key(book: LaneBook) -> tuple:
    """Prefer productive coverage, not coverage at any price.

    A positive-EV/economically acceptable uncovered book should lead the lane.
    Negative-EV uncovered books are kept only as low-priority fallback candidates
    so breadth pressure cannot outrank basic economics.
    """
    ...
    # Productive known-EV opportunities lead. A cold uncovered book is an
    # exploration candidate, not a negative-EV book, so it remains ahead of
    # known non-positive candidates and cannot be permanently starved.
