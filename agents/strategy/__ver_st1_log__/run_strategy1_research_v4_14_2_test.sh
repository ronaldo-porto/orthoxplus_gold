...
grep -q 'RESEARCH_POLICY_VERSION = "wide_kappa_wave_v4_14_2"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not wide_kappa_wave_v4_14_2" >&2
  exit 1
}
...
echo "[Strategy1_Research] version=wide_kappa_wave_v4_14_2"
echo "[Strategy1_Research] log_dir=$RESEARCH_DIR"

if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "V4.14.2 validator-aligned Wide-Kappa Wave launcher preflight-only PASS"
  exit 0
fi
