...
45. V4.12.12 breaks repeated post-only CONTRACT_VIOLATION loops. A real reject
    installs a bounded book/side cooldown; the next retry is repriced away from the
    live opposite touch by 1-3 ticks, and the guard clears on acceptance. Normal
    Maker pricing, ONE_AWAY completion, and Taker authority are otherwise unchanged.
...
from research_contract_guard import (
    ...
    ContractRejectState,
    DECAY_TICKS as CONTRACT_GUARD_DECAY_TICKS,
    guard_is_active,
    guard_should_skip,
...
class Strategy1_Research(Strategy1_Debug):
    ...
    def initialize(self) -> None:
        ...
        # V4.12.12: reject-aware post-only hygiene. This state is populated only
    ...
    def _research_apply_contract_reject_guard(self, response, state) -> dict[str, int]:
        """Suppress immediate retries and safely reprice later post-only retries.
        ...
        one policy. Market/Taker instructions are never touched.
        """
        ...
        # Drop expired state first. A guard must not become permanent if a book
        # simply stops producing orders after the original reject.
        for key, guard in list(self._research_contract_reject_state.items()):
            if not guard_is_active(guard, current_tick=now):
