...
if KAPPA_PRODUCTIVITY_VERSION != "simplified_kappa_productivity_v4_13_8":
...
grep -q 'RESEARCH_POLICY_VERSION = "simplified_kappa_productivity_v4_13_8"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not simplified_kappa_productivity_v4_13_8" >&2
  exit 1
}
...
echo "[Strategy1_Research] version=simplified_kappa_productivity_v4_13_8"
echo "[Strategy1_Research] log_dir=$RESEARCH_DIR"

if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "V4.13.8 launcher preflight-only PASS"
  exit 0
fi
