...
class LaneBook:
    ...
    # V4.12.17 Kappa flywheel state. ``recent_realized_pnl`` is retained as a
...
def apply_kappa_conversion_pressure_gate(
    ...
    *,
    ...
    keep_ids = {
        int(row.book_id)
        for row in sorted(fresh, key=coverage_sort_key)[:keep_n]
    }
    suppressed: set[int] = set()
    out: list[LaneBook] = []
    for row in rows:
...
def classify_execution_lane(book: LaneBook) -> str:
    ...
    if bool(getattr(book, "density_due", False)) and bool(book.economics_ok):
        return LANE_COMPLETION
...
def completion_sort_key(book: LaneBook) -> tuple:
    ...
    type_rank = 0 if remaining == 1 else (1 if refresh else (2 if remaining == 2 else 3))
    score_pnl_rank = 0 if bool(getattr(book, "score_pnl_ready", True)) else 1
    recent_pnl = _finite(getattr(book, "recent_realized_pnl", 0.0))
    flywheel = _finite(getattr(book, "flywheel_priority", 0.0))
    ...
    return (
        ...
        -flywheel,
        -recent_pnl,
        maker_fee,  # more-negative maker fee/rebate wins equal-EV ties
