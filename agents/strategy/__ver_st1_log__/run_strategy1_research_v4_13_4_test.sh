...
# V4.13.4 authoritative productivity-lane deployment contract.
...
from research_execution_lanes import (
    LaneBook, LaneBudgets, LANE_COMPLETION, LANE_COVERAGE, LANE_REALIZATION,
    authoritative_execution_lane, execution_completion_candidate, select_lane_candidates,
)
from research_inventory_liveness import (
...
if KAPPA_PRODUCTIVITY_VERSION != "simplified_kappa_productivity_v4_13_4":
    raise SystemExit("ERROR: stale research_kappa_productivity.py for V4.13.4")
...
print("V4.13.4 authoritative CORE/RECYCLING/PROBE lane API + inherited Maker grace OK")
PYV413PRODUCTIVITY
...
grep -q 'RESEARCH_POLICY_VERSION = "simplified_kappa_productivity_v4_13_4"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not simplified_kappa_productivity_v4_13_4" >&2
  exit 1
}
...
echo "[Strategy1_Research] version=simplified_kappa_productivity_v4_13_4"
echo "[Strategy1_Research] log_dir=$RESEARCH_DIR"

if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "V4.13.4 launcher preflight-only PASS"
  exit 0
fi
