...
class Strategy1_Research(Strategy1_Debug):
    RESEARCH_POLICY_VERSION = "pending_reprice_guard_v4_12_13"
    ...
    def _research_apply_contract_reject_guard(self, response, state) -> dict[str, int]:
        ...
        for instruction in instructions:
            ...
            if not bids or not asks or price is None:
                # Fail closed while a real reject guard is active; blindly
                # retrying without a live touch recreates the observed loop.
                skips += 1
                self._research_contract_guard_skips += 1
                self._emit(
                    "CONTRACT_REJECT_GUARD",
