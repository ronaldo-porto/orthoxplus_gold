...
class Strategy1_Research(Strategy1_Debug):
    RESEARCH_POLICY_VERSION = "simple_performance_core_v4_12_1"
    ...
    RESEARCH_KAPPA_SCHEDULER_VERSION = "kappa_completion_v4_rolling"
    ...
    RESEARCH_LANES_VERSION = "execution_lanes_v3_hard_cap"
    RESEARCH_SCORE_ACQUISITION_VERSION = "score_acquisition_v2_sticky_cohort"
    ...
    def initialize(self) -> None:
        ...
        self.RESEARCH_ENTRY_SIZE_VERSION = "entry_size_v1"
        self.RESEARCH_MIN_ORDER_ADMISSION_VERSION = "min_order_admission_v2_positive_ev"
        ...
        self.research_aggressive_positive_ev_max_maker_fill = max(
            0.0, min(1.0, float(getattr(cfg, "research_aggressive_positive_ev_max_maker_fill", 0.05)))
        )
        ...
        self.RESEARCH_SPEED_VERSION = "response_speed_v2_cached_kappa"
        ...
        self._enqueue({
            ...
            "aggressive_positive_ev_max_maker_fill": float(getattr(self, "research_aggressive_positive_ev_max_maker_fill", 0.05)),
    ...
    def _research_fast_screen(self, state) -> ScreenResult:
        ...
        for book_id, book in (getattr(state, "books", None) or {}).items():
            ...
            refresh_urgency = float(getattr(expiry, "expiry_urgency", 0.0) or 0.0)
    ...
    def _compute_local_kappa(self, state):
        """Cache miner-side validator Kappa between realizations.

        Full kappa_3() over realized_pnl_history is unnecessary on every request.
        A new own realization invalidates immediately; otherwise refresh at a
        short cadence so rolling-window expiry remains bounded and deterministic.
        """
        tick = int(getattr(self, "_tick", 0) or 0)
        cadence = max(1, int(getattr(self, "research_local_kappa_refresh_ticks", 10) or 10))
        bucket = tick // cadence
        generation = int(getattr(self, "_research_realized_generation", 0) or 0)
        cached = getattr(self, "_research_local_kappa_cache_value", None)
        if (
            cached is not None
            and int(getattr(self, "_research_local_kappa_cache_generation", -1)) == generation
            and int(getattr(self, "_research_local_kappa_cache_bucket", -1)) == bucket
        ):
            return cached
        value = super()._compute_local_kappa(state)
        self._research_local_kappa_cache_value = value
        self._research_local_kappa_cache_generation = generation
        self._research_local_kappa_cache_bucket = bucket
        return value
    ...
    def _research_refresh_rolling_kappa_cache(self) -> None:
        ...
        if key == getattr(self, "_research_kappa_roll_cache_key", None):
            return
        live = rolling_observation_timestamps(
            history, now=now, lookback_ns=lookback,
        )
        ...
        # Keep only rolling-window evidence in memory; lifetime counters are
        # intentionally not consulted by the scheduler or completion authority.
        self._research_persisted_observation_timestamps = {
        ...
        self._research_kappa_roll_cache_key = key
    ...
    def _global_book_rank(self, expected_alpha: float, mem) -> float:
        ...
        try:
            ...
            if expiry.qualified and expiry.expiry_urgency > 0.0:
                ...
                rank += float(self.research_kappa_expiry_rank_bonus) * float(expiry.expiry_urgency)
        except Exception:
            pass
        return rank
    ...
    def _research_evaluate_realization(
        ...
        return evaluate_realization(
            ...
            aggressive_positive_ev_max_maker_fill=float(
                getattr(self, "research_aggressive_positive_ev_max_maker_fill", 0.05)
            ),
    ...
    def _research_manage_realization(
        ...
        if econ is not None:
            try:
                self._emit(
                    ...
                    wait_ev=(
                        getattr(getattr(decision, "maker_taker_ev", None), "expected_maker_exit_value", None)
                    ),
                    taker_ev=(
                        getattr(getattr(decision, "maker_taker_ev", None), "expected_taker_exit_value", None)
                    ),
                    aggressive_maker_ev=(
                        getattr(getattr(decision, "maker_taker_ev", None), "expected_maker_exit_value", decision.maker_exit_ev)
                    ),
                    **econ.as_log(),
                    **(
        ...
        if decision.selected_action == ACTION_TAKER:
            ...
            econ = getattr(decision, "taker_economics", None)
            catastrophic = bool(getattr(econ, "catastrophic", False)) or str(
    ...
    def _console_allowed(self, r: dict[str, Any]) -> bool:
        ...
        if bool(getattr(self, "research_compact_console", True)):
            ...
            if typ == "ENTRY_SIZE":
                ...
                return trigger in {"ONE_AWAY_EXACT_MIN", "OPEN_BOOK_CAP"} or (allow and sampled)
            if typ == "SKIP":
                reason = str(r.get("reason", r.get("raw_reason", ""))).upper()
                # NO_PREDICTION dominated >90% of V4.12 console lines and carries
                # little operational value. Keep only the explicitly filtered book.
                if reason == "NO_PREDICTION":
                    return self.research_book_id >= 0
                return sampled
        ...
        if typ in {"ERROR", "RUN_SUMMARY", "RESEARCH_CONFIG", "DEBUG_CONFIG", "POSITION", "POSITION_GUARD", "ACTIONABLE_FILL", "REGIME", "SCORE_REGIME", "REALIZATION", "EXIT_URGENCY", "LADDER", "TAKER_DECISION", "EXIT_QTY", "INVENTORY_STATE", "ENTRY_SIZE", "QUOTE", "EXEC_PROB", "FILL", "MARKOUT", "FILL_CAL", "RANK", "KAPPA", "SCHED", "LANES", "CANCEL_DECISION", "TTL", "RESPOND_TIMING", "VOLUME_CAP", "HYBRID", "HYBRID_SUMMARY", "SESSION_TRANSITION"}:
            return True
        if typ == "ORDER_LIFECYCLE":
            phase = str(r.get("phase", "")).upper()
