...
from research_contract_guard import (
    ...
    ContractRejectState,
    HARD_LIFETIME_TICKS as CONTRACT_GUARD_HARD_LIFETIME_TICKS,
    guard_is_active,
    guard_should_skip,
...
class Strategy1_Research(Strategy1_Debug):
    ...
    RESEARCH_ONE_AWAY_TTL_VERSION = ONE_AWAY_STALE_TTL_VERSION
    RESEARCH_ONE_AWAY_CONVERSION_TTL_VERSION = ONE_AWAY_CONVERSION_TTL_VERSION
    RESEARCH_CONTRACT_GUARD_VERSION = CONTRACT_GUARD_VERSION
    EXIT_CONTRACT_GUARD_PERSISTENCE_VERSION = "sticky_exit_contract_guard_v4_13_9"
    RESEARCH_INVENTORY_LIVENESS_VERSION = INVENTORY_LIVENESS_VERSION
    ...
    RESEARCH_LANES_VERSION = "execution_lanes_v7_inventory_decoupled"
    ...
    def initialize(self) -> None:
        ...
        # V4.13.6: retain the latest expensive Score-EV result briefly so
        # the cheap 128-book screen can stop known NEGATIVE_EV books from
        # consuming scarce completion slots. Unknown EV remains fail-open.
        ...
        self.RESEARCH_ENTRY_SIZE_VERSION = "entry_size_v4_12_11_one_away_completion"
        self.RESEARCH_MIN_ORDER_ADMISSION_VERSION = "min_order_admission_v4_one_away_completion"
        ...
        self.research_hysteresis_min_price_ticks = max(
            0.25, float(getattr(cfg, "research_hysteresis_min_price_ticks", 3.0))
        )
        self.research_persistent_maker_enabled = self._as_bool(
            getattr(cfg, "research_persistent_maker_enabled", True)
        )
        ...
        # V4.12.10 final blocker fix: hard total round-trip ceiling for the
        # strict stale-direct bridge. Config may tighten but never widen below -12 bps.
        self.research_unified_stale_bridge_roundtrip_floor_bps = max(
            -12.0, min(0.0, float(getattr(
                cfg, "research_unified_stale_bridge_roundtrip_floor_bps", -12.0
            )))
        )
        ...
        # open-position and aggregate-base cap. Keep the old name as a backwards-
        # compatible alias for the active cap used by older telemetry/tests.
        self.research_max_active_open_books = max(
            2, min(8, int(getattr(
                cfg, "research_max_active_open_books",
                getattr(cfg, "research_max_open_books", 6),
            )))
        )
        self.research_max_total_open_books = max(
            self.research_max_active_open_books,
            min(8, int(getattr(cfg, "research_max_total_open_books", 8))),
        )
        # V4.14.0: parked count is now also a *coverage acquisition* pressure
        # threshold. It does not force-close inventory, but once reached we stop
        # opening fresh COVERAGE books until realization/completion frees capital.
        self.research_max_parked_open_books = max(
            0,
            min(
                self.research_max_total_open_books,
        ...
        self.research_kappa_exploration_slots = max(
            1, min(2, int(getattr(cfg, "research_kappa_exploration_slots", 1)))
        )
        ...
        self.research_max_open_books = self.research_max_active_open_books
        _default_abs_cap = (
            float(self.research_max_total_open_books)
            * max(float(getattr(self, "mm_base_size", 0.25) or 0.25), 1e-9)
        )
        self.research_max_total_abs_base = max(
            float(self.research_max_active_open_books)
            * max(float(getattr(self, "mm_base_size", 0.25) or 0.25), 1e-9),
            min(
                _default_abs_cap,
        ...
        self.research_liveness_maker_failed_exits = max(
            1, int(getattr(cfg, "research_liveness_maker_failed_exits", 3))
        )
        self.research_liveness_maker_min_age_ticks = max(
            1.0, float(getattr(cfg, "research_liveness_maker_min_age_ticks", 8.0))
        )
        self.research_liveness_taker_failed_exits = max(
            self.research_liveness_maker_failed_exits,
            int(getattr(cfg, "research_liveness_taker_failed_exits", 8)),
        )
        self.research_liveness_taker_min_age_ticks = max(
            self.research_liveness_maker_min_age_ticks,
            float(getattr(cfg, "research_liveness_taker_min_age_ticks", 16.0)),
        )
        self.research_liveness_hard_failed_exits = max(
            self.research_liveness_taker_failed_exits,
            int(getattr(cfg, "research_liveness_hard_failed_exits", 12)),
        )
        self.research_liveness_hard_min_age_ticks = max(
            self.research_liveness_taker_min_age_ticks,
            float(getattr(cfg, "research_liveness_hard_min_age_ticks", 24.0)),
        )
        # V4.12.18: protected Kappa states may PARK without receiving loss subsidy.
        # These thresholds only arm capacity release when the closest legal Maker
        # touch cannot satisfy the existing Kappa loss floor.
        self.research_protected_park_failed_exits = max(
            1, int(getattr(cfg, "research_protected_park_failed_exits", 4))
        )
        self.research_protected_park_min_age_ticks = max(
            1.0, float(getattr(cfg, "research_protected_park_min_age_ticks", 8.0))
        )
        self.research_liveness_maker_floor_bps = max(
            -8.0, min(0.0, float(getattr(cfg, "research_liveness_maker_floor_bps", -4.0)))
        )
        self.research_liveness_soft_taker_floor_bps = max(
            -8.0, min(0.0, float(getattr(cfg, "research_liveness_soft_taker_floor_bps", -8.0)))
        )
        self.research_liveness_hard_taker_floor_bps = max(
            -12.0,
            min(
                self.research_liveness_soft_taker_floor_bps,
                float(getattr(cfg, "research_liveness_hard_taker_floor_bps", -12.0)),
            ),
        )
        self.research_liveness_min_ev_advantage_bps = max(
            0.0, float(getattr(cfg, "research_liveness_min_ev_advantage_bps", 0.50))
        )
        self.research_liveness_adverse_markout_bps = max(
            0.0, float(getattr(cfg, "research_liveness_adverse_markout_bps", 1.0))
        )
        self.research_liveness_adverse_risk_floor = max(
            0.0, min(1.0, float(getattr(cfg, "research_liveness_adverse_risk_floor", 0.25)))
        )
        # V4.13.2 fresh-position Maker grace. Only suppress the event-driven
        # PRICE_HARD_WINDOW_RESCUE; all real hard-risk/stop/emergency authority
        # and the absolute -12 bps floor remain unchanged.
        self.research_fresh_maker_grace_enabled = self._as_bool(
            getattr(cfg, "research_fresh_maker_grace_enabled", True)
        )
        self.research_fresh_maker_grace_ticks = max(
            1.0, min(3.0, float(getattr(cfg, "research_fresh_maker_grace_ticks", 3.0)))
        )
        # V4.13.5 exit-authority ordering.  A meaningful executable Maker
        # profit dominates an ordinary negative Taker rescue for the first
        # few failed exits. True hard-risk/stop/emergency authority is unchanged.
        self.research_positive_maker_veto_enabled = self._as_bool(
            getattr(cfg, "research_positive_maker_veto_enabled", True)
        )
        self.research_positive_maker_veto_floor_bps = max(
            0.0, float(getattr(cfg, "research_positive_maker_veto_floor_bps", 1.0))
        )
        self.research_positive_maker_veto_max_failed_exits = max(
            1, int(getattr(cfg, "research_positive_maker_veto_max_failed_exits", 4))
        )
        # V4.14.0 long-run recycling: accept a controlled loss while it is still
        # small instead of protecting Kappa observations until inventory becomes
        # a multi-hour parked tail. This authority is intentionally tiny and uses
        # the already-tracked peak Taker value; it adds no new runtime state.
        self.research_bounded_loss_escape_enabled = self._as_bool(
            getattr(cfg, "research_bounded_loss_escape_enabled", True)
        )
        self.research_bounded_loss_escape_min_age_ticks = max(
            1.0, float(getattr(cfg, "research_bounded_loss_escape_min_age_ticks", 2.0))
        )
        self.research_bounded_loss_escape_floor_bps = max(
            -40.0, min(-12.0, float(getattr(cfg, "research_bounded_loss_escape_floor_bps", -25.0)))
        )
        self.research_bounded_loss_escape_drawdown_bps = max(
            0.0, float(getattr(cfg, "research_bounded_loss_escape_drawdown_bps", 2.0))
        )
        self.research_parked_refresh_ticks = max(
            10, min(30, int(getattr(cfg, "research_parked_refresh_ticks", 25)))
        )
        ...
        self.research_one_away_exact_min_safe_fraction = max(
            0.15, min(1.0, float(getattr(cfg, "research_one_away_exact_min_safe_fraction", 0.15)))
        )
        self.research_one_away_exact_min_exit_fraction = max(
            0.20, min(1.0, float(getattr(cfg, "research_one_away_exact_min_exit_fraction", 0.20)))
        )
        ...
        # V4.13.8: profitable Maker realization may remain live across publish
        # cycles. Existing same-side exits are held to preserve queue priority
        # unless touch moves materially; this affects exits only, never entries.
        self.research_profitable_exit_persistence_enabled = self._as_bool(
            getattr(cfg, "research_profitable_exit_persistence_enabled", True)
        )
        self.research_profitable_exit_ttl_ms = max(
            1000.0, min(5000.0, float(getattr(cfg, "research_profitable_exit_ttl_ms", 3000.0)))
        )
        self.research_profitable_exit_min_net_bps = max(
            -0.25, float(getattr(cfg, "research_profitable_exit_min_net_bps", 0.0))
        )
        self.research_profitable_exit_reprice_ticks = max(
            1.0, min(10.0, float(getattr(cfg, "research_profitable_exit_reprice_ticks", 3.0)))
        )
        ...
        self.research_one_away_max_touch_bps = max(
            0.5, min(20.0, float(getattr(cfg, "research_one_away_max_touch_bps", 1.5)))
        )
        ...
        # V4.12.15 full-size stale inventory parking. Unlike dust parking this
        # registry does not pretend the position is flat; it only removes stale
        # books from the active acquisition/realization pool between refreshes.
        self._research_parked_inventory: dict[int, dict[str, Any]] = {}
        self._research_inventory_rescue_armed = 0
        self._research_inventory_rescue_maker = 0
        self._research_inventory_rescue_taker = 0
        self._research_inventory_rescue_blocked = 0
        self._research_inventory_parks = 0
        self._research_inventory_unparks = 0
        ...
        # by real simulator CONTRACT_VIOLATION notices and is keyed by
        # (book_id, side). Normal Maker pricing is untouched until a reject occurs.
        self._research_contract_reject_state: dict[tuple[int, str], ContractRejectState] = {}
        self._research_contract_rejects = 0
        self._research_contract_guard_skips = 0
        self._research_contract_guard_reprices = 0
        ...
        self._enqueue({
            ...
            "liveness_maker_floor_bps": float(getattr(self, "research_liveness_maker_floor_bps", -4.0)),
            "liveness_soft_taker_floor_bps": float(getattr(self, "research_liveness_soft_taker_floor_bps", -8.0)),
            "liveness_hard_taker_floor_bps": float(getattr(self, "research_liveness_hard_taker_floor_bps", -12.0)),
            "fresh_maker_grace_enabled": int(bool(getattr(self, "research_fresh_maker_grace_enabled", True))),
            "fresh_maker_grace_ticks": float(getattr(self, "research_fresh_maker_grace_ticks", 3.0)),
            "positive_maker_veto_version": self.RESEARCH_POSITIVE_MAKER_VETO_VERSION,
            ...
            "bounded_loss_escape_enabled": int(bool(getattr(self, "research_bounded_loss_escape_enabled", True))),
            "bounded_loss_escape_min_age_ticks": float(getattr(self, "research_bounded_loss_escape_min_age_ticks", 2.0)),
            "bounded_loss_escape_floor_bps": float(getattr(self, "research_bounded_loss_escape_floor_bps", -25.0)),
            ...
            "one_away_exact_min_exit_fraction": float(getattr(self, "research_one_away_exact_min_exit_fraction", 0.20)),
            ...
            "unified_stale_bridge_roundtrip_floor_bps": float(
                getattr(self, "research_unified_stale_bridge_roundtrip_floor_bps", -12.0)
            ),
            ...
            "profitable_exit_persistence_enabled": int(bool(getattr(self, "research_profitable_exit_persistence_enabled", True))),
            "profitable_exit_ttl_ms": float(getattr(self, "research_profitable_exit_ttl_ms", 3000.0)),
            "profitable_exit_min_net_bps": float(getattr(self, "research_profitable_exit_min_net_bps", 0.0)),
            "profitable_exit_reprice_ticks": float(getattr(self, "research_profitable_exit_reprice_ticks", 3.0)),
            ...
            "one_away_max_touch_bps": float(getattr(self, "research_one_away_max_touch_bps", 1.5)),
            "one_away_stale_ttl_ms": float(getattr(self, "research_one_away_stale_ttl_ms", 250.0)),
            ...
            "density_priority_enabled": int(bool(self.research_density_priority_enabled)),
    ...
    def _research_restore_realized_pnl_events(self, raw_session) -> None:
        raw = raw_session if isinstance(raw_session, dict) else {}
        self._research_realized_pnl_events_by_book = sanitize_realized_pnl_events(
            raw.get("rolling_realized_pnl_events")
        )

    def _research_note_realized_pnl_event(
        self, book_id: int, *, timestamp=None, realized_pnl: float
    ) -> None:
        ts = timestamp
        if ts is None:
            ts = getattr(self, "_research_last_sim_ts", None)
        if ts is None:
            ts = int(getattr(self, "_tick", 0) or 0)
        try:
            its = int(ts)
        except (TypeError, ValueError):
            its = int(getattr(self, "_tick", 0) or 0)
        now = getattr(self, "_research_last_sim_ts", None)
        if now is None:
            now = its
        self._research_realized_pnl_events_by_book = note_realized_pnl_event(
    ...
    def _research_rolling_book_economics(self, book_id: int):
        now = getattr(self, "_research_last_sim_ts", None)
        return rolling_book_economics(
            getattr(self, "_research_realized_pnl_events_by_book", {}),
            int(book_id),
    ...
    def _research_productivity_snapshot(
        ...
        *,
        ...
        return ProductivitySnapshot(
            ...
            realized_pnl=float(getattr(rolling_econ, "realized_sum", 0.0) or 0.0),
            positive_count=max(0, int(getattr(rolling_econ, "positive_count", 0) or 0)),
            negative_count=max(0, int(getattr(rolling_econ, "negative_count", 0) or 0)),
            maker_fee_bps=float(maker_fee_bps),
            fill_rate_hint=max(0.0, min(1.0, float(fill_rate_hint or 0.0))),
            raw_kappa=raw_kappa,
    ...
    def _research_save_session(self, force: bool = False) -> None:
        ...
        try:
            ...
            payload["rolling_realized_pnl_events"] = {
                str(book): [[int(ts), float(pnl)] for ts, pnl in rows]
                for book, rows in sorted(
                    (getattr(self, "_research_realized_pnl_events_by_book", {}) or {}).items()
                )
                if rows
            }
    ...
    def _research_compute_cheap_tob(self, book) -> tuple[float | None, float | None, int]:
        ...
        # V4.13: full event parsing is Stage-2 work only.  The all-book scan is
        # intentionally TOB-only to cut LazyBooks/event hot-path latency.
        return spread_bps, imb, 0
    ...
    def _research_execution_lane_budgets(self):
        if bool(getattr(self, "research_kappa_productivity_enabled", True)):
            phase = str(
                getattr(self, "_research_productivity_phase", PRODUCTIVITY_PHASE_BOOTSTRAP)
                or PRODUCTIVITY_PHASE_BOOTSTRAP
            ).upper()
            # Realization is a separate safety reservation.  The remaining deep
            # candidate capacity follows the V4.13 breadth -> density schedule.
            if phase == PRODUCTIVITY_PHASE_DENSITY:
                return normalize_lane_budgets(
                    coverage_slots=1, completion_slots=6, realization_slots=3,
                    shared_overflow_slots=1,
                )
            if phase == PRODUCTIVITY_PHASE_BALANCED:
                return normalize_lane_budgets(
                    coverage_slots=2, completion_slots=5, realization_slots=3,
                    shared_overflow_slots=1,
                )
            return normalize_lane_budgets(
                coverage_slots=4, completion_slots=3, realization_slots=3,
                shared_overflow_slots=1,
            )
        if bool(getattr(self, "research_kappa_flywheel_enabled", True)):
            phase = str(getattr(self, "_research_flywheel_phase", PHASE_BOOTSTRAP) or PHASE_BOOTSTRAP).upper()
            if phase == PHASE_DENSITY:
                return normalize_lane_budgets(coverage_slots=2, completion_slots=5, realization_slots=3, shared_overflow_slots=1)
            if phase == PHASE_BREADTH:
                return normalize_lane_budgets(coverage_slots=3, completion_slots=4, realization_slots=3, shared_overflow_slots=1)
            return normalize_lane_budgets(coverage_slots=4, completion_slots=3, realization_slots=3, shared_overflow_slots=1)
        return normalize_lane_budgets(
    ...
    def _research_is_parked_inventory(self, book_id: int) -> bool:
        return int(book_id) in (getattr(self, "_research_parked_inventory", {}) or {})

    def _research_clear_parked_inventory(self, book_id: int, *, reason: str) -> bool:
        table = getattr(self, "_research_parked_inventory", None)
        if not isinstance(table, dict):
            return False
        row = table.pop(int(book_id), None)
        if row is None:
            return False
        self._research_inventory_unparks = int(
            getattr(self, "_research_inventory_unparks", 0) or 0
        ) + 1
        try:
            self._emit(
                "UNPARK_POSITION", force=True, tick=getattr(self, "_tick", None),
                book=int(book_id), reason=str(reason or "UNPARK"),
                parked_age_ticks=max(
                    0,
                    int(getattr(self, "_tick", 0) or 0)
                    - int(row.get("first_tick", getattr(self, "_tick", 0)) or 0),
                ),
                score_state=row.get("score_state"),
                park_state=row.get("park_state"),
                loss_rescue_eligible=row.get("loss_rescue_eligible"),
                park_eligible=row.get("park_eligible"),
                rescue_floor_bps=row.get("rescue_floor_bps"),
                protected_floor_bps=row.get("protected_floor_bps"),
                net_base=row.get("net_base"),
            )
        except Exception:
            pass
        return True

    def _research_park_inventory(
        self,
        book_id: int,
        *,
        inventory,
        ...
        score_state: str,
        taker_net_bps: float,
        rescue_floor_bps: float,
        reason: str,
        park_state: str = "PARKED_LIVENESS",
        protected_floor_bps: float | None = None,
        loss_rescue_eligible: bool = True,
        park_eligible: bool = True,
    ) -> bool:
        table = getattr(self, "_research_parked_inventory", None)
        if not isinstance(table, dict):
            table = {}
            self._research_parked_inventory = table
        bid = int(book_id)
        now = int(getattr(self, "_tick", 0) or 0)
        mid = self._research_book_mid(book)
        existing = table.get(bid)
        if existing is None:
            # V4.12.17: PARKED is a state classification, not a scarce slot.
            # Never refuse to park book #7 merely because six other stale books
            # are already parked. Total open-position and aggregate BASE caps in
            # the screen remain the hard risk authority. Keep the old parked cap
            # only as a telemetry warning threshold.
            cap = max(0, int(getattr(self, "research_max_parked_open_books", 6)))
            parked_warning = bool(cap > 0 and len(table) >= cap)
            row = {
                "first_tick": now,
                "last_refresh_tick": now,
                "last_mid": mid,
                "score_state": str(score_state),
                "park_state": str(park_state or "PARKED_LIVENESS"),
                "loss_rescue_eligible": int(bool(loss_rescue_eligible)),
                "park_eligible": int(bool(park_eligible)),
                "rescue_floor_bps": float(rescue_floor_bps),
                "protected_floor_bps": float(
                    rescue_floor_bps if protected_floor_bps is None else protected_floor_bps
                ),
                "taker_net_bps_at_park": float(taker_net_bps),
                "net_base": float(getattr(inventory, "net_base", 0.0) or 0.0),
                "reason": str(reason or "PARK"),
            }
            table[bid] = row
            self._research_inventory_parks = int(
                getattr(self, "_research_inventory_parks", 0) or 0
            ) + 1
            try:
                self._emit(
                    "PARK_POSITION", force=True, tick=now, book=bid,
                    reason=str(reason or "PARK"), score_state=str(score_state),
                    park_state=str(park_state or "PARKED_LIVENESS"),
                    loss_rescue_eligible=int(bool(loss_rescue_eligible)),
                    park_eligible=int(bool(park_eligible)),
                    taker_net_bps=float(taker_net_bps),
                    rescue_floor_bps=float(rescue_floor_bps),
                    protected_floor_bps=float(
                        rescue_floor_bps if protected_floor_bps is None else protected_floor_bps
                    ),
                    ...
                    parked_open_books=len(table),
                    max_parked_open_books=max(0, int(getattr(self, "research_max_parked_open_books", 6))),
                    parked_warning_threshold_exceeded=int(parked_warning),
                )
            except Exception:
                pass
            try:
                self._emit(
                    "PARK_PROTECTED" if str(park_state).upper() == "PARKED_PROTECTED" else "PARK_LIVENESS",
                    force=True, tick=now, book=bid, reason=str(reason or "PARK"),
                    score_state=str(score_state), park_state=str(park_state),
                    protected_floor_bps=float(
                        rescue_floor_bps if protected_floor_bps is None else protected_floor_bps
                    ),
                    taker_net_bps=float(taker_net_bps),
                ...
                pass
            return True
        existing["score_state"] = str(score_state)
        existing["park_state"] = str(park_state or existing.get("park_state") or "PARKED_LIVENESS")
        existing["loss_rescue_eligible"] = int(bool(loss_rescue_eligible))
        existing["park_eligible"] = int(bool(park_eligible))
        existing["rescue_floor_bps"] = float(rescue_floor_bps)
        existing["protected_floor_bps"] = float(
            rescue_floor_bps if protected_floor_bps is None else protected_floor_bps
        )
        existing["net_base"] = float(getattr(inventory, "net_base", 0.0) or 0.0)
        if mid is not None and existing.get("last_mid") is None:
            existing["last_mid"] = mid
        return True

    def _research_park_refresh_due(self, book_id: int, book, *, hard_risk: bool) -> tuple[bool, str]:
        row = (getattr(self, "_research_parked_inventory", {}) or {}).get(int(book_id))
        if not isinstance(row, dict):
            return True, "NOT_PARKED"
        return parked_refresh_due(
            current_tick=int(getattr(self, "_tick", 0) or 0),
            last_refresh_tick=row.get("last_refresh_tick"),
            current_mid=self._research_book_mid(book),
            last_mid=row.get("last_mid"),
            refresh_interval_ticks=int(getattr(self, "research_parked_refresh_ticks", 20) or 20),
            material_touch_move_bps=float(getattr(self, "research_parked_touch_move_bps", 8.0) or 8.0),
            hard_risk=bool(hard_risk),
        )

    def _research_note_park_refresh(self, book_id: int, book, *, reason: str) -> None:
        row = (getattr(self, "_research_parked_inventory", {}) or {}).get(int(book_id))
        if not isinstance(row, dict):
            return
        row["last_refresh_tick"] = int(getattr(self, "_tick", 0) or 0)
        mid = self._research_book_mid(book)
        if mid is not None:
            row["last_mid"] = mid
        row["last_refresh_reason"] = str(reason or "REFRESH")
        try:
            self._emit(
                "PARK_REFRESH", force=True, tick=getattr(self, "_tick", None),
                book=int(book_id), reason=str(reason or "REFRESH"),
                parked_age_ticks=max(
                    0,
                    int(getattr(self, "_tick", 0) or 0)
                    - int(row.get("first_tick", getattr(self, "_tick", 0)) or 0),
                ),
                mid=mid, score_state=row.get("score_state"),
            )
        except Exception:
            pass

    def _research_cancel_resting_for_park(self, response, book_id: int) -> int:
        """Cancel accepted resting orders when a book becomes PARKED.

        Parking is a capacity state, but leaving an older loss-making Maker exit
        live would let it realize below the newly protected floor (the V4.12.17
        Book26 failure). Cancel once on park; later refreshes submit only if the
        current touch satisfies the stored floor.
        """
        account = (getattr(self, "accounts", {}) or {}).get(int(book_id))
        orders = getattr(account, "orders", None) or [] if account is not None else []
        order_ids = [getattr(order, "id", None) for order in orders if getattr(order, "id", None) is not None]
        if not order_ids:
            return 0
        if self._count_book_instructions(response, int(book_id)) >= self.max_instructions_per_book:
            try:
                self._emit(
                    "PARK_CANCEL_BLOCKED", force=True, tick=getattr(self, "_tick", None),
                    book=int(book_id), reason="INSTRUCTION_CAP", resting_orders=len(order_ids),
                )
            except Exception:
                pass
            return 0
        try:
            ...
            self._emit(
                "PARK_CANCEL", force=True, tick=getattr(self, "_tick", None),
                book=int(book_id), resting_orders=len(order_ids),
            )
            return 1
        except Exception:
            return 0

    def _research_parked_touch_exit(self, book_id: int, book, inventory) -> tuple[float | None, float | None]:
        """Return closest legal post-only exit and round-trip net bps at that touch."""
        try:
            bid = float(book.bids[0].price)
            ask = float(book.asks[0].price)
            ...
            return None, None
        if bid <= 0.0 or ask <= 0.0 or ask < bid or entry <= 0.0:
            return None, None
        cfg = getattr(getattr(self, "_research_volume_cap_state", None), "config", None)
        price_dec = int(getattr(cfg, "priceDecimals", 2) or 2)
        tick = 10.0 ** (-max(0, price_dec))
        ...
        net = unified_completion_net_bps(
            entry_price=entry, exit_price=float(px), long_position=long_pos,
            entry_fee_bps=maker_fee, exit_fee_bps=maker_fee,
        )
        return float(px), float(net)

    def _research_liveness_stage(
        self,
        *,
        ...
        required = required_observation_count(
            kappa_min_observations=getattr(self.config, "kappa_min_observations", None),
            research_target=getattr(self, "research_kappa_required_observations", None),
        )
        return classify_liveness_stage(
            ...
            inventory_state=str(inventory_state or "NORMAL"),
            stop_loss_hit=bool(stop_loss_hit),
            maker_failed_exits=int(getattr(self, "research_liveness_maker_failed_exits", 3)),
            maker_min_age_ticks=float(getattr(self, "research_liveness_maker_min_age_ticks", 8.0)),
            taker_failed_exits=int(getattr(self, "research_liveness_taker_failed_exits", 8)),
            taker_min_age_ticks=float(getattr(self, "research_liveness_taker_min_age_ticks", 16.0)),
            hard_failed_exits=int(getattr(self, "research_liveness_hard_failed_exits", 12)),
            hard_min_age_ticks=float(getattr(self, "research_liveness_hard_min_age_ticks", 24.0)),
            protected_park_failed_exits=int(getattr(self, "research_protected_park_failed_exits", 4)),
            protected_park_min_age_ticks=float(getattr(self, "research_protected_park_min_age_ticks", 8.0)),
            maker_floor_bps=float(getattr(self, "research_liveness_maker_floor_bps", -4.0)),
            soft_taker_floor_bps=float(getattr(self, "research_liveness_soft_taker_floor_bps", -8.0)),
    ...
    def _research_fast_screen(self, state) -> ScreenResult:
        ...
        for book_id, book in (getattr(state, "books", None) or {}).items():
            ...
            has_inv = qty > flat
            is_liveness_parked = bool(has_inv and bid in parked_inventory)
            if has_inv:
                actual_nonflat += 1
                total_abs_base += float(qty)
                if is_liveness_parked:
                    parked_nonflat += 1
                else:
                    active_nonflat += 1
            else:
                if bid in parked_inventory:
                    self._research_clear_parked_inventory(bid, reason="FLAT_SCREEN")
                    parked_inventory = getattr(self, "_research_parked_inventory", {}) or {}
                if bid in open_position_keys:
            ...
            rolling_econ = self._research_rolling_book_economics(bid)
            pnl_conf = pnl_confidence(int(samples), int(rolling_econ.nonzero_count))
            pnl_conf_mult = float(pnl_confidence_multiplier(pnl_conf))
            pnl_authority_complete = bool(pnl_conf == "FULL")
            entry_feasible = True
            ...
            if spread_bps is not None and spread_bps >= toxic_spread:
                hard = True
            park_refresh_due_now = True
            park_refresh_reason = "NOT_PARKED"
            if is_liveness_parked:
                # Do not let a wide/toxic spread itself defeat parking by forcing
                # a refresh every tick. Hard refresh is reserved for genuine
                # inventory-cap pressure; adverse/toxic books otherwise use the
                # bounded interval/touch-move cadence and are re-evaluated there.
                park_hard_risk = bool((qty / max_inv) >= close_thr)
                park_refresh_due_now, park_refresh_reason = self._research_park_refresh_due(
                    bid, book, hard_risk=park_hard_risk
                )
                # A parked position remains real inventory and counts against the
                # total risk cap, but between bounded refreshes it must not consume
                # REALIZATION candidate capacity. entry_feasible=False also keeps it
                # out of COVERAGE/COMPLETION when represented as lane-flat.
                if not park_refresh_due_now:
                    entry_feasible = False
            lane_has_inv = bool(has_inv and (not is_liveness_parked or park_refresh_due_now))
            ...
            if not maker_ev_known:
                ...
                if cached_ev is not None:
                    try:
                        cached_tick = int(cached_ev.get("tick", -10**9))
                        cached_value = float(cached_ev.get("trading_ev", 0.0))
                        cache_age = int(getattr(self, "_tick", 0) or 0) - cached_tick
                        if (
                            0 <= cache_age <= int(getattr(self, "research_completion_ev_cache_ticks", 20))
            ...
            completion_ev_ok = bool(
                (not maker_ev_known)
                or maker_ev > float(getattr(self, "research_score_ev_min_trading", 0.0))
            )
            ...
            recent_ema = float(getattr(mem, "recent_pnl", 0.0) or 0.0) if mem is not None else 0.0
            rolling_pnl = float(rolling_econ.realized_sum)
            # For a restored legacy snapshot with Kappa observations but no PnL
            # event evidence, fail closed on PnL-ready ranking instead of treating
            # the default EMA value 0 as validator-positive.
            pnl_rank_value = rolling_pnl if pnl_authority_complete else float("-inf")
            age = 0.0
            try:
            ...
            if has_inv or is_dust:
                ...
                urgency = exit_urgency(
                    ...
                    recent_realized_pnl=(rolling_pnl if pnl_authority_complete else recent_ema),
            ...
            rows.append(
                ScreenBook(
                    book_id=bid,
                    has_inventory=lane_has_inv,
                    is_dust=bool(is_dust and lane_has_inv),
            ...
            lane_rows.append(
                LaneBook(
                    ...
                    cheap_score=cheap,
                    maker_ev=maker_ev,
                    maker_ev_known=bool(maker_ev_known),
                    completion_ev_known=bool(maker_ev_known),
                    completion_ev_ok=bool(completion_ev_ok),
                    economics_ok=(
                    ...
                    score_pnl_ready=bool(pnl_authority_complete and rolling_pnl >= 0.0),
                    recent_realized_pnl=(rolling_pnl if rolling_econ.nonzero_count > 0 else recent_ema),
                    rolling_observation_count=int(samples),
                    rolling_positive_count=int(rolling_econ.positive_count),
        ...
        # V4.13 simplified Kappa productivity scheduler.  Qualification is not
        # a finish line: productive qualified books remain density candidates,
        # while order-hungry books are demoted by execution efficiency.
        productivity_phase = productivity_scheduler_phase(len(kappa_eligible_ids))
        if productivity_phase == PRODUCTIVITY_PHASE_DENSITY:
            compat_phase = PHASE_DENSITY
            density_target = 50
            core_limit = 48
        elif productivity_phase == PRODUCTIVITY_PHASE_BALANCED:
            compat_phase = PHASE_BREADTH
            density_target = 12
            core_limit = 24
        else:
            compat_phase = PHASE_BOOTSTRAP
            density_target = 6
            core_limit = 8

        # Strict CORE is earned by observed execution productivity.  Restored or
        # newly-qualified books without enough runtime evidence remain QUALIFIED
        # and can still receive density work until evidence is learned.
        strict_core = [
            snap for snap in productivity_snapshots.values()
            if bool(snap.core_candidate)
        ]
        strict_core.sort(key=lambda snap: (-float(snap.score), int(snap.book_id)))
        core_ids = {int(snap.book_id) for snap in strict_core[:core_limit]}

        # V4.13.1 CORE bootstrap bridge: reserve one density recycle candidate
        # after its first clean fresh RT. This is deliberately one slot only;
        # full CORE still requires three fresh RTs.
        bridge_candidates = [
            snap for snap in productivity_snapshots.values()
            if bool(snap.recycling_candidate) and int(snap.book_id) not in core_ids
        ]
        bridge_candidates.sort(key=lambda snap: (-float(snap.score), int(snap.book_id)))
        recycling_ids = {int(bridge_candidates[0].book_id)} if bridge_candidates else set()

        # V4.13.2 CORE_PROBE: one qualified Kappa-eligible book with positive
        # Maker economics, safe acquisition state, and UNKNOWN fresh execution
        # evidence gets one minimum-size Maker cycle. Historical Kappa/PnL are
        # used only as vetoes when they are actually available; no book is
        # promoted to RECYCLING/CORE without fresh realized evidence.
        probe_candidates: list[LaneBook] = []
        for row in lane_rows if bool(getattr(self, "research_core_probe_enabled", True)) else []:
            bid = int(row.book_id)
            snap = productivity_snapshots.get(bid)
            if snap is None:
                continue
            if not core_probe_eligible(
                snap,
                kappa_eligible=bool(getattr(row, "kappa_eligible", False)),
                maker_ev=float(getattr(row, "maker_ev", 0.0) or 0.0),
                maker_ev_known=bool(getattr(row, "maker_ev_known", False)),
                flat_and_safe=bool(
                    not bool(getattr(row, "has_inventory", False))
                    and not bool(getattr(row, "is_dust", False))
                    and not bool(getattr(row, "is_hard_risk", False))
                ),
                entry_feasible=bool(getattr(row, "entry_feasible", True)),
                economics_ok=bool(getattr(row, "economics_ok", True)),
                pnl_confidence=str(getattr(row, "pnl_confidence", "UNKNOWN") or "UNKNOWN"),
                recent_realized_pnl=float(getattr(row, "recent_realized_pnl", 0.0) or 0.0),
                raw_kappa=getattr(row, "raw_kappa", None),
            ):
                continue
            probe_candidates.append(row)
        probe_candidates.sort(
            key=lambda row: (
                -float(getattr(row, "raw_kappa", 0.0) or 0.0),
                -float(getattr(row, "recent_realized_pnl", 0.0) or 0.0),
                -float(getattr(row, "maker_ev", 0.0) or 0.0),
                -float(getattr(row, "kappa_productivity_score", 0.0) or 0.0),
                int(row.book_id),
            )
        )
        core_probe_ids = {int(probe_candidates[0].book_id)} if probe_candidates else set()

        updated_lane_rows: list[LaneBook] = []
        for row in lane_rows:
            snap = productivity_snapshots.get(int(row.book_id))
            priority = float(getattr(row, "kappa_productivity_score", 0.0) or 0.0)
            if snap is not None:
                priority = kappa_productivity_priority(
                    snap,
                    phase=productivity_phase,
                    required_observations=self._research_required_observation_count(),
                )
            obs = int(getattr(row, "rolling_observation_count", 0) or 0)
            probe_failed = bool(snap is not None and getattr(snap, "fresh_probe_failed", False))
            density_due = bool(
                bool(getattr(row, "kappa_eligible", False))
                and not probe_failed
                and (
                    int(row.book_id) in core_ids
                    or int(row.book_id) in recycling_ids
                    or int(row.book_id) in core_probe_ids
                    or obs < int(density_target)
                )
            )
            updated_lane_rows.append(
                replace(
                    row,
                    density_due=density_due,
                    core_candidate=bool(int(row.book_id) in core_ids),
                    recycling_candidate=bool(int(row.book_id) in recycling_ids),
                    core_probe_candidate=bool(int(row.book_id) in core_probe_ids),
                    kappa_productivity_score=float(priority),
                    kappa_productivity_state=str(
                        productivity_kappa_state(
                            observations=obs, core=bool(int(row.book_id) in core_ids)
                        )
                    ),
                )
            )
        lane_rows = updated_lane_rows
        self._research_productivity_phase = str(productivity_phase)
        self._research_productivity_core_ids = set(core_ids)
        self._research_productivity_recycling_ids = set(recycling_ids)
        self._research_productivity_core_probe_ids = set(core_probe_ids)
        # Preserve old Flywheel fields as compatibility telemetry/utilities only.
        self._research_flywheel_phase = str(compat_phase)
        self._research_flywheel_core_ids = set(core_ids)
        self._research_flywheel_kappa_eligible_ids = set(kappa_eligible_ids)
        self._research_flywheel_density_target = int(density_target)
        self._research_lane_budgets = self._research_execution_lane_budgets()
        ...
        lane_rows, pressure_suppressed_ids, pressure_productive, pressure_reason = (
            apply_kappa_conversion_pressure_gate(
                ...
                exploration_slots=int(getattr(
                    self, "research_kappa_exploration_slots", 1
                )),
        ...
        # V4.14.0: once the parked target is reached, stop opening fresh
        # COVERAGE positions. Existing inventory and KAPPA_COMPLETION/CORE work
        # remain executable so capital is recycled before breadth expands again.
        parked_cap = max(0, int(getattr(self, "research_max_parked_open_books", 4) or 0))
        parked_recycle_suppressed = set()
        if parked_cap > 0 and int(parked_nonflat) >= parked_cap:
            parked_rows = []
            for row in lane_rows:
                if (
                    bool(getattr(row, "entry_feasible", True))
                    and not bool(getattr(row, "has_inventory", False))
                    and not bool(getattr(row, "is_dust", False))
                    and not bool(getattr(row, "is_hard_risk", False))
                    and classify_execution_lane(row) == EXEC_LANE_COVERAGE
                ):
                    parked_recycle_suppressed.add(int(row.book_id))
                    parked_rows.append(replace(row, entry_feasible=False))
                else:
                    parked_rows.append(row)
            lane_rows = parked_rows
            if parked_recycle_suppressed and str(pressure_reason) == "NO_PRESSURE":
                pressure_reason = "PARKED_RECYCLE"
        pressure_suppressed = set(pressure_suppressed_ids) | parked_recycle_suppressed
        ...
        active_cap_saturated = active_nonflat >= max_active_open
        total_cap_saturated = actual_nonflat >= max_total_open
        exposure_cap_saturated = total_abs_base + 1e-12 >= max_total_abs_base
        ...
        try:
            ...
            self._emit(
                ...
                score_deficit=max(0, int(getattr(self, "research_score_target_books", 88)) - len(kappa_eligible_ids)),
                ...
                flywheel_density_target=int(getattr(self, "_research_flywheel_density_target", 6) or 6),
        ...
        self._research_inventory_lane_diag = {
            ...
            "parked_nonflat_inventory": int(parked_nonflat),
            "total_abs_base_inventory": float(total_abs_base),
            "max_open_books": int(getattr(self, "research_max_active_open_books", 6)),
            "max_active_open_books": int(getattr(self, "research_max_active_open_books", 6)),
            "max_total_open_books": int(getattr(self, "research_max_total_open_books", 12)),
            "max_total_abs_base": float(getattr(self, "research_max_total_abs_base", 3.0)),
            "open_cap_saturated": int(open_cap_saturated),
            "active_cap_saturated": int(active_cap_saturated),
            "total_cap_saturated": int(total_cap_saturated),
        ...
        if not self._research_lanes_on():
            ...
            allocation = select_lane_candidates(
                lane_rows,
                self._research_lane_budgets_for_screen(lane_rows),
                max_candidates=cap,
            )
    ...
    def _research_lane_budgets_for_screen(self, lane_rows=None):
        return density_priority_budgets(
            lane_rows or [],
            getattr(self, "_research_lane_budgets", None),
            enabled=bool(getattr(self, "research_density_priority_enabled", True)),
            min_candidates=int(getattr(self, "research_density_priority_min_candidates", 1)),
            aggressive_coverage=bool(getattr(self, "research_enable_aggressive_coverage", True)),
        )
    ...
    def _research_score_ev_for_book(
        ...
        try:
            ...
            if int(roll.nonzero_count) > 0:
                recent_pnl = float(roll.realized_sum)
        except Exception:
            recent_pnl = None
    ...
    def _global_book_rank(self, expected_alpha: float, mem) -> float:
        ...
        try:
            ...
            if math.isfinite(trading_ev):
                self._research_completion_ev_cache[int(book_id)] = {
                    "tick": int(getattr(self, "_tick", 0) or 0),
                    "trading_ev": float(trading_ev),
                }
        except (TypeError, ValueError):
            pass
        try:
    ...
    def skewed_quote_prices(
        ...
        safety_ticks = max(1, int(getattr(self, "research_post_only_safety_ticks", 2) or 2))
        post_only_gap = safety_ticks * tick_size
        bid_px = min(reservation - half_spread, ask - post_only_gap)
        ask_px = max(reservation + half_spread, bid + post_only_gap)
        ...
        if bool(getattr(self, "_research_completion_quiet_tight_context", False)):
            ...
            max_touch_bps = float(
                getattr(self, "research_one_away_max_touch_bps", 1.5) or 1.5
            )
            ...
            # Never sit on the fragile one-tick crossing boundary.  The small
            # latency-aware gap is the Book98 contract-reject correction.
            bid_px = min(bid_px, round(ask - post_only_gap, price_dec))
            ask_px = max(ask_px, round(bid + post_only_gap, price_dec))
        ...
        # Final post-only safety clamp after inventory competitiveness/skew.
        bid_px = min(float(bid_px), round(ask - post_only_gap, price_dec))
        ask_px = max(float(ask_px), round(bid + post_only_gap, price_dec))
    ...
    def _research_apply_unified_exit(
        ...
        *,
        ...
        bounded_loss_escape = bounded_loss_escape_applies(
            taker_net_bps=float(taker_net),
    ...
    def _research_place_maker_exit(
        ...
        # V4.13.9: reuse the existing contract-reject guard for profitable
        # inventory exits.  A repaired/accepted exit keeps the guard until the
        # position lifecycle ends, so persistence cannot immediately recreate
        # the same rejected post-only price.  This is intentionally O(1): no
        # new cache/state machine, only the existing (book, side) guard.
        side_token = "sell" if long_pos else "buy"
        exit_guard = self._research_contract_reject_state.get((int(book_id), side_token))
        if exit_guard is not None and guard_is_active(
            exit_guard, current_tick=int(getattr(self, "_tick", 0) or 0)
        ):
            guarded_close_px = guarded_post_only_price(
                side=side_token, original_price=close_px, best_bid=bid, best_ask=ask,
                tick_size=tick, reject_streak=exit_guard.streak,
            )
            if guarded_close_px is not None:
                guarded_close_px = round(float(guarded_close_px), price_dec)
                if abs(guarded_close_px - close_px) > max(1e-12, tick * 0.25):
                    self._emit(
                        "CONTRACT_REJECT_GUARD", tick=int(getattr(self, "_tick", 0) or 0),
                        book=int(book_id), side=side_token, action="EXIT_PRICE_CLAMP",
                        old_price=float(close_px), new_price=float(guarded_close_px),
                        reject_streak=exit_guard.streak,
                        guard_version=self.EXIT_CONTRACT_GUARD_PERSISTENCE_VERSION,
                    )
                close_px = guarded_close_px

        # V4.13.8 profitable-exit persistence: if an accepted same-side exit is
        # already near the desired touch and carries enough size, preserve its
        # queue position instead of cancel/replacing it every publish cycle.
        persistence = bool(getattr(self, "research_profitable_exit_persistence_enabled", True))
        if persistence and maker_net_bps is not None:
            account = self.accounts[book_id]
            for order in (getattr(account, "orders", None) or []):
                try:
                    order_side = "buy" if int(getattr(order, "side", -1)) == 0 else "sell"
                    if order_side != side_token:
                        continue
                    old_px = getattr(order, "price", None)
                    if old_px is None:
                        old_px = getattr(order, "limit_price", None)
                    if hold_existing_profitable_maker_exit(
                        existing_price=old_px, desired_price=close_px, tick_size=tick,
                        existing_qty=float(getattr(order, "quantity", 0.0) or 0.0),
                        desired_qty=float(qty), maker_net_bps=maker_net_bps,
                        enabled=persistence,
                        min_net_bps=float(getattr(self, "research_profitable_exit_min_net_bps", 0.0)),
                        reprice_ticks=float(getattr(self, "research_profitable_exit_reprice_ticks", 3.0)),
                    ):
                        try:
                            self._emit(
                                "PROFITABLE_EXIT_HOLD", force=True, tick=getattr(self, "_tick", None),
                                book=int(book_id), side=side_token, old_price=float(old_px),
                                desired_price=float(close_px), maker_net_bps=float(maker_net_bps),
                                quantity=float(qty),
                            )
                        except Exception:
                            pass
        ...
        base_ttl_ms = float(sim_delta_ms(0, int(expiry_period)) or 500.0)
        persistent_ttl_ms, persistent_exit = profitable_maker_exit_ttl_ms(
            baseline_ttl_ms=base_ttl_ms, maker_net_bps=maker_net_bps,
            market_regime=regime,
            enabled=bool(getattr(self, "research_profitable_exit_persistence_enabled", True)),
            min_net_bps=float(getattr(self, "research_profitable_exit_min_net_bps", 0.0)),
            persistent_ttl_ms=float(getattr(self, "research_profitable_exit_ttl_ms", 3000.0)),
        )
        if persistent_exit:
            expiry_period = max(expiry_period, int(ms_to_ns(persistent_ttl_ms)))
            try:
                self._emit(
                    "PROFITABLE_EXIT_PERSIST", force=True, tick=getattr(self, "_tick", None),
                    book=int(book_id), side=("sell" if long_pos else "buy"),
                    maker_net_bps=float(maker_net_bps), chosen_ttl_ms=float(persistent_ttl_ms),
                    price=float(close_px), quantity=float(qty),
                )
            except Exception:
                pass
        if close_dir == OrderDirection.SELL and account.base_balance.free >= qty:
    ...
    def _research_manage_realization(
        ...
        if getattr(inventory, "band", None) == "FLAT":
            self._research_clear_parked_inventory(int(book_id), reason="FLAT_MANAGE")
            return 0
        was_parked_liveness = self._research_is_parked_inventory(int(book_id))
        if was_parked_liveness:
            # Parked books reach this function only on their bounded refresh,
            # a material touch move, or a hard-risk override from fast screening.
            self._research_note_park_refresh(int(book_id), book, reason="SELECTED_REFRESH")
        placed = 0
        ...
        is_parked_liveness = self._research_is_parked_inventory(int(book_id))
        newly_parked_liveness = bool(is_parked_liveness and not was_parked_liveness)
        parked_row = (getattr(self, "_research_parked_inventory", {}) or {}).get(int(book_id))
        ...
        exit_maker_net_bps = (
            getattr(unified, "maker_net_bps", None) if unified is not None else None
        )
        if newly_parked_liveness:
            # Cancel older resting exits immediately. Leaving one live can realize
            # a loss far below the new park floor before the next bounded refresh.
            placed += self._research_cancel_resting_for_park(response, int(book_id))
            try:
                self._emit(
                    "PARKED_HOLD", force=True, tick=getattr(self, "_tick", None),
                    book=int(book_id), reason="NEWLY_PARKED_CANCEL_AND_HOLD",
                    selected_action=decision.selected_action,
                    inventory=float(inventory.net_base),
                    park_state=(parked_row or {}).get("park_state"),
                    protected_floor_bps=(parked_row or {}).get("protected_floor_bps"),
                )
            except Exception:
                pass
            return placed

        # V4.12.18 parked refresh invariant. A PARKED position may submit a Maker
        # exit only when the *current closest legal post-only touch* itself satisfies
        # the stored state-specific floor. Never submit a far-away breakeven order
        # and never let a parked Maker silently realize below the park floor.
        if was_parked_liveness and is_parked_liveness and isinstance(parked_row, dict):
            touch_px, touch_net = self._research_parked_touch_exit(int(book_id), book, inventory)
            floor = float(parked_row.get(
                "protected_floor_bps", parked_row.get("rescue_floor_bps", 0.0)
            ) or 0.0)
            park_state = str(parked_row.get("park_state") or "PARKED_LIVENESS")
            if touch_px is None or touch_net is None or float(touch_net) < floor - 1e-12:
                event = "PROTECTED_REFRESH_BLOCKED" if park_state == "PARKED_PROTECTED" else "PARKED_HOLD"
                try:
                    self._emit(
                        event, force=True, tick=getattr(self, "_tick", None),
                        book=int(book_id), reason="PARK_FLOOR_BLOCK", park_state=park_state,
                        protected_floor_bps=floor, candidate_touch_exit_bps=touch_net,
                        candidate_touch_price=touch_px, inventory=float(inventory.net_base),
                    )
                except Exception:
                    pass
                return placed
            self._research_clear_parked_inventory(int(book_id), reason="UNPARK_EXECUTABLE")
            unified_px = float(touch_px)
            exit_maker_net_bps = float(touch_net)
            try:
                self._emit(
                    "PROTECTED_REFRESH_SUBMIT" if park_state == "PARKED_PROTECTED" else "PARK_REFRESH_SUBMIT",
                    force=True, tick=getattr(self, "_tick", None), book=int(book_id),
                    park_state=park_state, protected_floor_bps=floor,
                    candidate_touch_exit_bps=float(touch_net), candidate_touch_price=float(touch_px),
                )
            except Exception:
                pass
        ...
        n = self._research_place_maker_exit(
            ...
            close_price=unified_px, maker_net_bps=exit_maker_net_bps,
        )
        if n:
            placed += n
            self._research_note_exit_attempt(
    ...
    @staticmethod
    def _research_notice_side(event) -> str | None:
        raw = getattr(event, "side", None)
        if raw is None:
            raw = getattr(event, "direction", None)
        token = str(getattr(raw, "name", raw)).upper()
        if token in {"0", "BUY", "BID", "ORDERDIRECTION.BUY"}:
            return "buy"
        if token in {"1", "SELL", "ASK", "ORDERDIRECTION.SELL"}:
            return "sell"
        return None

    def onOrderRejected(self, event) -> None:
        """Install a bounded retry guard after a real post-only contract reject."""
        super().onOrderRejected(event)
        reason = str(getattr(event, "message", "") or "").upper()
        if reason != "CONTRACT_VIOLATION":
            return
        book_id = getattr(event, "bookId", None)
        side = self._research_notice_side(event)
        try:
            book_id = int(book_id)
        except (TypeError, ValueError):
            return
        ...
        key = (book_id, side)
        now = int(getattr(self, "_tick", 0) or 0)
        previous = self._research_contract_reject_state.get(key)
        ...
        self._emit(
            ...
            book=book_id,
            side=side,
            action="REGISTER_REJECT",
            reason=reason,
            reject_streak=state.streak,
            first_reject_tick=state.first_reject_tick,
            blocked_until_tick=state.blocked_until_tick,
            hard_lifetime_ticks=CONTRACT_GUARD_HARD_LIFETIME_TICKS,
            guard_version=self.RESEARCH_CONTRACT_GUARD_VERSION,
        )

    def onOrderAccepted(self, event) -> None:
        """Clear a reject guard as soon as a limit order on that book/side is accepted."""
        super().onOrderAccepted(event)
        etype = str(getattr(event, "type", "") or "").upper()
        # Do not let a market-order acceptance clear a Maker/post-only guard.
        if etype and not ("RDPOL" in etype or "PLACE_ORDER_LIMIT" in etype or "LIMIT" in etype):
            return
        book_id = getattr(event, "bookId", None)
        side = self._research_notice_side(event)
        try:
        ...
        if side is None:
            return
        key = (book_id, side)
        old = self._research_contract_reject_state.get(key)
        if old is None:
            return

        # V4.13.9: if this accepted Maker order is still the exit side of a
        # live position, retain the already-learned guard.  It is cleared by
        # FLAT/CROSS or the existing hard lifetime, so there is no new state.
        try:
            net_qty = float(self._position_tracker_snapshot(book_id).net_qty)
        except Exception:
            net_qty = 0.0
        is_exit_side = (net_qty > 1e-12 and side == "sell") or (
            net_qty < -1e-12 and side == "buy"
        )
        if is_exit_side:
            self._emit(
                "CONTRACT_REJECT_GUARD",
                tick=int(getattr(self, "_tick", 0) or 0) + 1,
                book=book_id, side=side, action="ACCEPT_RETAIN_EXIT_GUARD",
                reject_streak=old.streak, first_reject_tick=old.first_reject_tick,
                guard_version=self.EXIT_CONTRACT_GUARD_PERSISTENCE_VERSION,
            )
            return

        self._research_contract_reject_state.pop(key, None)
        self._research_contract_guard_accept_clears += 1
        self._emit(
            "CONTRACT_REJECT_GUARD",
            tick=int(getattr(self, "_tick", 0) or 0) + 1,
            book=book_id,
            side=side,
            action="ACCEPT_CLEAR",
            reject_streak=old.streak,
            first_reject_tick=old.first_reject_tick,
            guard_version=self.RESEARCH_CONTRACT_GUARD_VERSION,
        )

    def _research_clear_contract_guards_for_book(self, book_id: int, *, reason: str) -> int:
        """Clear both side guards when the inventory lifecycle makes them obsolete."""
        try:
            bid = int(book_id)
        except (TypeError, ValueError):
            return 0
        cleared = 0
        now = int(getattr(self, "_tick", 0) or 0)
        for key, old in list(self._research_contract_reject_state.items()):
            if int(key[0]) != bid:
                continue
            self._research_contract_reject_state.pop(key, None)
            cleared += 1
            self._research_contract_guard_lifecycle_clears += 1
            self._emit(
                "CONTRACT_REJECT_GUARD",
                tick=now,
                book=bid,
                side=key[1],
                action="LIFECYCLE_CLEAR",
                reason=str(reason or "LIFECYCLE"),
                reject_streak=old.streak,
                first_reject_tick=old.first_reject_tick,
                guard_version=self.RESEARCH_CONTRACT_GUARD_VERSION,
            )
    ...
    def _research_apply_contract_reject_guard(self, response, state) -> dict[str, int]:
        """Hold a reject pending until a fresh-touch post-only retry is possible.

        This runs after the inherited builder has assembled instructions, so it
        covers maintenance pairs, ONE_AWAY quotes and inventory Maker exits with
        one policy. Market/Taker instructions are never touched. NO_TOUCH does
        not clear or decay the pending state; only hard lifetime, accepted limit
        order, or an inventory lifecycle clear can remove it.
        """
        if not self._research_contract_reject_state:
            return {"skips": 0, "reprices": 0, "active": 0}

        now = int(getattr(self, "_tick", 0) or 0)
        # Hard safety expiry is separate from retry liveness. V4.12.12 used a
        # 32-tick last-reject decay, which was shorter than observed 33-40 tick
        # no-touch gaps and therefore reset the streak before safe repricing.
        for key, guard in list(self._research_contract_reject_state.items()):
            if guard_is_active(guard, current_tick=now):
                continue
            self._research_contract_reject_state.pop(key, None)
            self._research_contract_guard_hard_expiry_clears += 1
            self._emit(
                "CONTRACT_REJECT_GUARD",
                tick=now,
                book=key[0],
                side=key[1],
                action="HARD_EXPIRE_CLEAR",
                reject_streak=guard.streak,
                first_reject_tick=guard.first_reject_tick,
                hard_lifetime_ticks=CONTRACT_GUARD_HARD_LIFETIME_TICKS,
                guard_version=self.RESEARCH_CONTRACT_GUARD_VERSION,
            )

        instructions = list(getattr(response, "instructions", None) or [])
        if not instructions or not self._research_contract_reject_state:
            return {"skips": 0, "reprices": 0, "active": len(self._research_contract_reject_state)}
        ...
        for instruction in instructions:
            ...
            side = self._research_instruction_side(instruction)
            key = (book_id, side)
            guard = self._research_contract_reject_state.get(key)
            if guard is None:
                kept.append(instruction)
                continue

            if guard_should_skip(guard, current_tick=now):
                skips += 1
                self._research_contract_guard_skips += 1
                self._emit(
                    "CONTRACT_REJECT_GUARD",
                    tick=now,
                    book=book_id,
                    ...
                    reject_streak=guard.streak,
                    blocked_until_tick=guard.blocked_until_tick,
                    guard_version=self.RESEARCH_CONTRACT_GUARD_VERSION,
                )
                continue

            # V4.12.14: use the same authoritative state.books mapping as the
            # parent quote builder. With lazy_load=1 this is LazyBooks (Mapping),
            # not dict; V4.12.13's isinstance(books, dict) check made every live
            # retry look touch-less and disconnected the reprice path.
            book = resolve_book_from_state_mapping(books, book_id)
            bids = getattr(book, "bids", None) if book is not None else None
            asks = getattr(book, "asks", None) if book is not None else None
            price = self._get(instruction, "price", "limitPrice", "limit_price")
            if book is None:
                no_touch_reason = "BOOK_NOT_FOUND"
            elif not bids:
                no_touch_reason = "EMPTY_BIDS"
            elif not asks:
                no_touch_reason = "EMPTY_ASKS"
            elif price is None:
                no_touch_reason = "PRICE_MISSING"
            else:
                no_touch_reason = ""
            if no_touch_reason:
                # Fail closed only when the authoritative current state truly
                # lacks L1; do not invent prices or fall back to telemetry.
                skips += 1
                self._research_contract_guard_skips += 1
                self._emit(
                    "CONTRACT_REJECT_GUARD",
                    ...
                    book=book_id,
                    side=side,
                    action="NO_TOUCH_SKIP",
                    no_touch_reason=no_touch_reason,
                    books_type=type(books).__name__,
                    reject_streak=guard.streak,
                    first_reject_tick=guard.first_reject_tick,
                    pending_age_ticks=max(0, now - int(guard.first_reject_tick)),
                    hard_lifetime_ticks=CONTRACT_GUARD_HARD_LIFETIME_TICKS,
                    pending_reprice=1,
                    guard_version=self.RESEARCH_CONTRACT_GUARD_VERSION,
                )
            ...
            new_price = guarded_post_only_price(
                side=side,
            ...
            if abs(new_price - old_price) > max(1e-12, tick_size * 0.25):
                try:
                    object.__setattr__(instruction, "price", new_price)
                except Exception:
                    try:
                        setattr(instruction, "price", new_price)
                    except Exception:
                        skips += 1
                        self._research_contract_guard_skips += 1
                        continue
                reprices += 1
                self._research_contract_guard_reprices += 1
                self._emit(
                    "CONTRACT_REJECT_GUARD",
                    tick=now,
                    book=book_id,
                    side=side,
                    action="REPRICE_RETRY",
                    reject_streak=guard.streak,
                    first_reject_tick=guard.first_reject_tick,
                    pending_age_ticks=max(0, now - int(guard.first_reject_tick)),
                    old_price=old_price,
    ...
    def onTrade(self, event, validator: str | None = None) -> None:
        ...
        if abs(realized_delta) > 1e-12:
            ...
            self._research_note_realized_pnl_event(
                int(book_id),
        ...
        if transition in {"FLAT", "CROSS"}:
            # The rejected Maker exit and any parking state belong to the old
            # inventory lifecycle. Once flat/crossed both are obsolete.
            self._research_clear_contract_guards_for_book(
                int(book_id), reason=transition
            )
            self._research_clear_parked_inventory(int(book_id), reason=transition)
    ...
    def dynamic_order_size(
        ...
        productive_qualified_core = bool(
            book_id is not None
            and int(book_id) in (
                set(getattr(self, "_research_productivity_core_ids", set()) or set())
                | set(getattr(self, "_research_productivity_recycling_ids", set()) or set())
            )
        )
        # V4.12.15 dual concurrency gate. Parked stale inventory releases active
        # acquisition capacity, but it still counts against total positions and a
        # hard aggregate absolute-base exposure cap. Planned opens in this same
        # response are included so a quote burst cannot jump past either limit.
        if book_id is not None and abs(float(inventory.net_base)) <= self._execution_flat_epsilon():
            ...
            max_total = int(getattr(self, "research_max_total_open_books", 12) or 12)
            max_abs_base = float(getattr(self, "research_max_total_abs_base", 3.0) or 3.0)
            base_active = int(diag.get("active_nonflat_inventory", diag.get("actual_nonflat_inventory", 0)) or 0)
            base_total = int(diag.get("actual_nonflat_inventory", 0) or 0)
            total_abs_base = float(diag.get("total_abs_base_inventory", 0.0) or 0.0)
            ...
            planned_count = len(planned_books)
            nominal_open_size = max(
                float(min_size),
                min(
            ...
            adding_new = int(book_id) not in planned_books
            projected_active = base_active + planned_count + (1 if adding_new else 0)
            projected_total = base_total + planned_count + (1 if adding_new else 0)
            projected_abs_base = total_abs_base + (planned_count + (1 if adding_new else 0)) * nominal_open_size
            trigger = None
            if projected_active > max_active:
                trigger = "ACTIVE_OPEN_BOOK_CAP"
            elif projected_total > max_total:
                trigger = "TOTAL_OPEN_BOOK_CAP"
            elif projected_abs_base > max_abs_base + 1e-12:
                trigger = "TOTAL_ABS_BASE_CAP"
            if trigger is not None:
                try:
                    self._emit(
                        ...
                        active_open_books=base_active,
                        total_open_books=base_total,
                        planned_open_books=planned_count,
                        projected_active_open_books=projected_active,
                        projected_total_open_books=projected_total,
                        total_abs_base=total_abs_base,
                        projected_total_abs_base=projected_abs_base,
                        max_active_open_books=max_active,
                        max_total_open_books=max_total,
                        max_total_abs_base=max_abs_base,
                    )
                except Exception:
                    pass
                return 0.0
        ...
        if getattr(self, "research_enable_min_order_admission", True):
            admission = admit_minimum_order(
                ...
                one_away_min_safe_fraction=float(
                    getattr(self, "research_one_away_exact_min_safe_fraction", 0.15)
                ),
                one_away_min_exit_fraction=float(
                    getattr(self, "research_one_away_exact_min_exit_fraction", 0.20)
                ),
        ...
        try:
            ...
            for side, new_px in (("buy", new_buy), ("sell", new_sell)):
                ...
                decision = should_replace_quote(
                    ...
                    hard_safety=hard,
                    persistent_maker=bool(getattr(self, "research_persistent_maker_enabled", True)),
                )
                try:
                    self._emit(
                        "CANCEL_DECISION",
    ...
    def _place_skewed_quotes(
        ...
        # Completion behavior (attempt/success accounting, relaxed fill policy,
        ...
        if getattr(self, "research_enable_adaptive_ttl", False):
            ...
            if chosen is None:
                chosen, ttl_reason, stale_override = one_away_stale_completion_ttl(
                    chosen_ttl_ms=chosen,
            ...
            if stale_override:
                try:
                    self._emit(
                        ...
                        tick=getattr(self, "_tick", None),
                        book=int(book_id),
                        samples=int(completion_samples),
                        target=int(self.research_kappa_completion_target),
                        trading_ev=completion_ev,
                        chosen_ttl_ms=float(chosen),
                        reason=str(ttl_reason),
                    )
                except Exception:
                    pass
    ...
    def build_mm_strategy_instructions(
        ...
        try:
            ...
            if isinstance(stats, dict):
                ...
                stats["research_contract_guard_skips"] = self._research_contract_guard_skips
                stats["research_contract_guard_reprices"] = self._research_contract_guard_reprices
                stats["research_contract_guard_accept_clears"] = self._research_contract_guard_accept_clears
                stats["research_contract_guard_lifecycle_clears"] = self._research_contract_guard_lifecycle_clears
                stats["research_contract_guard_hard_expiry_clears"] = self._research_contract_guard_hard_expiry_clears
                stats["research_contract_guard_hard_lifetime_ticks"] = CONTRACT_GUARD_HARD_LIFETIME_TICKS
                stats["research_contract_guard_active"] = int(contract_guard.get("active", 0))
                ...
                stats["research_inventory_liveness_version"] = self.RESEARCH_INVENTORY_LIVENESS_VERSION
                stats["research_parked_inventory"] = len(getattr(self, "_research_parked_inventory", {}) or {})
                stats["research_inventory_rescue_armed"] = int(getattr(self, "_research_inventory_rescue_armed", 0) or 0)
                stats["research_inventory_rescue_maker"] = int(getattr(self, "_research_inventory_rescue_maker", 0) or 0)
                stats["research_inventory_rescue_taker"] = int(getattr(self, "_research_inventory_rescue_taker", 0) or 0)
                stats["research_inventory_rescue_blocked"] = int(getattr(self, "_research_inventory_rescue_blocked", 0) or 0)
                stats["research_inventory_parks"] = int(getattr(self, "_research_inventory_parks", 0) or 0)
                stats["research_inventory_unparks"] = int(getattr(self, "_research_inventory_unparks", 0) or 0)
                _lane_diag = getattr(self, "_research_inventory_lane_diag", {}) or {}
                stats["research_active_open_books"] = int(_lane_diag.get("active_nonflat_inventory", 0) or 0)
                stats["research_total_open_books"] = int(_lane_diag.get("actual_nonflat_inventory", 0) or 0)
                stats["research_total_abs_base"] = float(_lane_diag.get("total_abs_base_inventory", 0.0) or 0.0)
                stats["research_max_active_open_books"] = int(getattr(self, "research_max_active_open_books", 6) or 6)
                stats["research_max_total_open_books"] = int(getattr(self, "research_max_total_open_books", 12) or 12)
                stats["research_max_total_abs_base"] = float(getattr(self, "research_max_total_abs_base", 3.0) or 3.0)
    ...
    def _research_emit_body(self, event_type: str, force: bool = False, **payload: Any) -> None:
        ...
        if event_type == "RUN_SUMMARY":
            ...
            payload.setdefault("research_inventory_liveness_version", self.RESEARCH_INVENTORY_LIVENESS_VERSION)
            payload.setdefault("research_parked_inventory_positions", len(getattr(self, "_research_parked_inventory", {}) or {}))
            payload.setdefault("research_inventory_rescue_armed", getattr(self, "_research_inventory_rescue_armed", 0))
            payload.setdefault("research_inventory_rescue_maker", getattr(self, "_research_inventory_rescue_maker", 0))
            payload.setdefault("research_inventory_rescue_taker", getattr(self, "_research_inventory_rescue_taker", 0))
            payload.setdefault("research_inventory_rescue_blocked", getattr(self, "_research_inventory_rescue_blocked", 0))
            payload.setdefault("research_inventory_parks", getattr(self, "_research_inventory_parks", 0))
            payload.setdefault("research_inventory_unparks", getattr(self, "_research_inventory_unparks", 0))
            _lane_diag = getattr(self, "_research_inventory_lane_diag", {}) or {}
            payload.setdefault("research_active_open_books", int(_lane_diag.get("active_nonflat_inventory", 0) or 0))
            payload.setdefault("research_total_open_books", int(_lane_diag.get("actual_nonflat_inventory", 0) or 0))
            payload.setdefault("research_total_abs_base", float(_lane_diag.get("total_abs_base_inventory", 0.0) or 0.0))
            payload.setdefault("research_max_active_open_books", int(getattr(self, "research_max_active_open_books", 6) or 6))
            payload.setdefault("research_max_total_open_books", int(getattr(self, "research_max_total_open_books", 12) or 12))
            payload.setdefault("research_max_total_abs_base", float(getattr(self, "research_max_total_abs_base", 3.0) or 3.0))
            ...
            payload.setdefault("research_contract_guard_version", self.RESEARCH_CONTRACT_GUARD_VERSION)
            payload.setdefault("research_contract_rejects", getattr(self, "_research_contract_rejects", 0))
            payload.setdefault("research_contract_guard_skips", getattr(self, "_research_contract_guard_skips", 0))
            payload.setdefault("research_contract_guard_reprices", getattr(self, "_research_contract_guard_reprices", 0))
            payload.setdefault("research_contract_guard_accept_clears", getattr(self, "_research_contract_guard_accept_clears", 0))
            payload.setdefault("research_contract_guard_lifecycle_clears", getattr(self, "_research_contract_guard_lifecycle_clears", 0))
            payload.setdefault("research_contract_guard_hard_expiry_clears", getattr(self, "_research_contract_guard_hard_expiry_clears", 0))
            payload.setdefault("research_contract_guard_hard_lifetime_ticks", CONTRACT_GUARD_HARD_LIFETIME_TICKS)
            payload.setdefault("research_contract_guard_active", len(getattr(self, "_research_contract_reject_state", {})))
    ...
    def _console_allowed(self, r: dict[str, Any]) -> bool:
        ...
        if bool(getattr(self, "research_compact_console", True)):
            ...
            if typ in {
                ...
                "RUN_SUMMARY", "STALE_RESCUE", "RESCUE_TAKER", "BOUNDED_LOSS_ESCAPE", "PARK_POSITION",
                "PARK_PROTECTED", "PARK_LIVENESS", "UNPARK_POSITION",
                "PROTECTED_REFRESH_BLOCKED", "PROTECTED_REFRESH_SUBMIT",
                "PARK_REFRESH_SUBMIT", "PARK_CANCEL", "PARK_CANCEL_BLOCKED",
            }:
                return True
            ...
            if typ == "ENTRY_SIZE":
                ...
                return trigger in {
                    "ONE_AWAY_EXACT_MIN", "TWO_AWAY_EXACT_MIN",
                    "OPEN_BOOK_CAP", "ACTIVE_OPEN_BOOK_CAP",
                    "TOTAL_OPEN_BOOK_CAP", "TOTAL_ABS_BASE_CAP",
                } or (allow and sampled)
            ...
            if typ in {
                ...
                "ADVERSE", "ORDER", "RESCUE_ARMED", "RESCUE_MAKER",
                "RESCUE_BLOCKED", "PARK_REFRESH", "PARKED_HOLD",
                    "PARK_PROTECTED", "PARK_LIVENESS", "PROTECTED_REFRESH_BLOCKED",
                    "PROTECTED_REFRESH_SUBMIT", "PARK_REFRESH_SUBMIT", "PARK_CANCEL", "PARK_CANCEL_BLOCKED",
            }:
                return sampled
            return sampled

        if typ in {"ERROR", "RUN_SUMMARY", "RESEARCH_CONFIG", "DEBUG_CONFIG", "POSITION", "POSITION_GUARD", "ACTIONABLE_FILL", "REGIME", "SCORE_REGIME", "STALE_RESCUE", "RESCUE_ARMED", "RESCUE_MAKER", "RESCUE_TAKER", "BOUNDED_LOSS_ESCAPE", "RESCUE_BLOCKED", "PARK_POSITION", "PARK_PROTECTED", "PARK_LIVENESS", "UNPARK_POSITION", "PARK_REFRESH", "PARKED_HOLD", "PROTECTED_REFRESH_BLOCKED", "PROTECTED_REFRESH_SUBMIT", "PARK_REFRESH_SUBMIT", "PARK_CANCEL", "PARK_CANCEL_BLOCKED", "REALIZATION", "EXIT_URGENCY", "LADDER", "TAKER_DECISION", "EXIT_QTY", "INVENTORY_STATE", "ENTRY_SIZE", "QUOTE", "EXEC_PROB", "FILL", "MARKOUT", "FILL_CAL", "RANK", "KAPPA", "SCHED", "LANES", "CANCEL_DECISION", "TTL", "RESPOND_TIMING", "VOLUME_CAP", "HYBRID", "HYBRID_SUMMARY", "SESSION_TRANSITION"}:
            return True
        if typ == "ORDER_LIFECYCLE":
            phase = str(r.get("phase", "")).upper()
