...
grep -q 'RESEARCH_POLICY_VERSION = "inventory_state_decoupling_v4_12_18"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not inventory_state_decoupling_v4_12_18" >&2
  exit 1
}
...
echo "[Strategy1_Research] version=inventory_state_decoupling_v4_12_18"
echo "[Strategy1_Research] log_dir=$RESEARCH_DIR"

if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "V4.12.18 launcher preflight-only PASS"
  exit 0
fi
