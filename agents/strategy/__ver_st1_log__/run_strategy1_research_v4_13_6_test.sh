...
if KAPPA_PRODUCTIVITY_VERSION != "simplified_kappa_productivity_v4_13_6":
    raise SystemExit("ERROR: stale research_kappa_productivity.py for V4.13.6")
...
print("V4.13.6 density scheduler + V4.13.5 exit authority + V4.13.4 authoritative lanes OK")
PYV413PRODUCTIVITY
...
grep -q 'RESEARCH_POLICY_VERSION = "simplified_kappa_productivity_v4_13_6"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not simplified_kappa_productivity_v4_13_6" >&2
  exit 1
}
...
echo "[Strategy1_Research] version=simplified_kappa_productivity_v4_13_6"
echo "[Strategy1_Research] log_dir=$RESEARCH_DIR"

if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "V4.13.6 launcher preflight-only PASS"
  exit 0
fi
