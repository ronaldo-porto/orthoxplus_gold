...
class Strategy1_Research(Strategy1_Debug):
    RESEARCH_POLICY_VERSION = "inventory_state_decoupling_v4_12_18"
    ...
    def _research_execution_lane_budgets(self):
        if bool(getattr(self, "research_kappa_flywheel_enabled", True)):
            ...
            # Candidate count remains the hard global cap. These reserves only
            # express the breadth->density transition inside that cap. Risk work
            # keeps three reserved slots in every phase and may still spill first.
    ...
    def _research_fast_screen(self, state) -> ScreenResult:
        ...
        # V4.12.17 Kappa flywheel: qualification is the beginning, not the end.
        # Maintain a bounded quality core whose positive/low-downside books keep
        # producing minimum-size realized observations while breadth continues.
        fly_phase = flywheel_phase(len(kappa_eligible_ids))
        density_target = phase_density_target(fly_phase)
        core_limit = phase_core_limit(fly_phase)
        qualified_pool = [
            row for row in lane_rows
            if bool(getattr(row, "kappa_eligible", False))
        ...
        qualified_pool.sort(
            key=lambda row: (
                -float(getattr(row, "pnl_confidence_mult", 0.70) or 0.70),
                -float(getattr(row, "flywheel_priority", 0.0) or 0.0),
                int(getattr(row, "rolling_negative_count", 0) or 0),
                -int(getattr(row, "rolling_positive_count", 0) or 0),
                int(row.book_id),
            )
        )
        core_ids = {int(row.book_id) for row in qualified_pool[:core_limit]}
        lane_rows = [
            replace(
                row,
                density_due=bool(
                    ...
                    and (
                        int(getattr(row, "rolling_observation_count", 0) or 0) < density_target
                        or bool(getattr(row, "needs_refresh", False))
                    )
                ),
            )
            for row in lane_rows
        ]
        self._research_flywheel_phase = str(fly_phase)
        ...
        # Keep screen and sequential execution admission on the same phase budget
        # in the very tick where breadth crosses 41 or 80 books.
        self._research_lane_budgets = self._research_execution_lane_budgets()
