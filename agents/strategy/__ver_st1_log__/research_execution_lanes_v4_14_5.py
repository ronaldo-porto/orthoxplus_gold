...
COMPLETION_ECONOMICS_VERSION = "completion_density_v4_13_6"
...
class LaneBook:
    ...
    # V4.13.6: cached deep Score-EV can pre-screen completion capacity without
    # weakening the final NEGATIVE_EV hard gate. Unknown is fail-open so books
    # can still be rediscovered; known non-positive EV is kept out of the scarce
    # KAPPA_COMPLETION lane until the bounded cache expires.
    ...
    # V4.14.5 TOTAL_SCORE_FRONTIER.  When total_score_phase is non-empty these
    # fields are the sole score-scheduling authority for flat books.  Legacy
    # density/core flags above remain only for backward-compatible tests/logs.
...
def density_priority_budgets(
    books: Iterable[LaneBook],
    budgets: LaneBudgets | None,
    *,
    enabled: bool = True,
    min_candidates: int = 1,
    aggressive_coverage: bool = True,
) -> LaneBudgets | None:
    """Shift BOOTSTRAP breadth capacity toward useful Kappa density.

    V4.13.5 showed a large ONE_AWAY/TWO_AWAY backlog while BOOTSTRAP still
    reserved more COVERAGE than COMPLETION. When at least one flat, feasible,
    non-negative-known/unknown completion candidate exists, preserve all
    REALIZATION capacity plus one COVERAGE exploration slot and give the rest
    of acquisition capacity to KAPPA_COMPLETION.
    """
    if budgets is None:
        return None
    rows = list(books or [])
    if enabled:
        actionable = [
            row for row in rows
            if not bool(getattr(row, "has_inventory", False))
            ...
            and (
                ...
                or bool(getattr(row, "core_candidate", False))
                or bool(getattr(row, "recycling_candidate", False))
                or bool(getattr(row, "core_probe_candidate", False))
                or bool(getattr(row, "density_due", False))
            )
        ]
        if len(actionable) >= max(1, int(min_candidates or 1)):
            acquisition = max(0, int(budgets.coverage_slots) + int(budgets.completion_slots))
            coverage = 1 if acquisition > 0 and aggressive_coverage else 0
            return normalize_lane_budgets(
                coverage_slots=coverage,
                completion_slots=max(0, acquisition - coverage),
                realization_slots=budgets.realization_slots,
                shared_overflow_slots=budgets.shared_overflow_slots,
            )
    if aggressive_coverage:
        return budgets
    return normalize_lane_budgets(
        coverage_slots=0,
...
def apply_breadth_rotation_gate(
    ...
    *,
    ...
    for row in rows:
        ...
        score_due = (
            bool(getattr(row, "total_score_due", False))
            if str(getattr(row, "total_score_phase", "") or "")
            else bool(getattr(row, "density_due", False))
        )
        block = bool(
            qualified
            and not score_due
    ...
    *,
    ...
    productive = [
        ...
        and (
            max(0, int(getattr(row, "observations_remaining", 0) or 0)) in {1, 2}
            or (
                not str(getattr(row, "total_score_phase", "") or "")
                and bool(getattr(row, "density_due", False))
            )
        )
    ]
    productive_count = len(productive)
...
def classify_execution_lane(book: LaneBook) -> str:
    ...
    V4.13.6 makes the *known* deep trading-EV result part of cheap lane
    admission.  This is only a capacity pre-filter: unknown EV remains fail-open
    and final Score-EV still owns the hard NEGATIVE_EV decision at execution.
    """
    if book.has_inventory or book.is_dust:
        return LANE_REALIZATION
    remaining = max(0, int(book.observations_remaining or 0))
    completion_economics_ok = bool(book.economics_ok) and bool(
        getattr(book, "completion_ev_ok", True)
    )
    # V4.14.5: live rows carry total_score_phase and therefore have exactly one
    # score authority.  Do not re-derive completion from old refresh, CORE,
    # RECYCLING, density flags, or observation count. Critical expiry refresh is
    # already converted into total_score_due by research_total_score_frontier.
    if str(getattr(book, "total_score_phase", "") or ""):
        if bool(getattr(book, "total_score_due", False)) and completion_economics_ok:
            return LANE_COMPLETION
        return LANE_COVERAGE

    # Legacy fallback for old unit tests / archived callers that do not provide
    # V4.14.5 total-score annotations.
    if bool(book.needs_refresh) and completion_economics_ok:
        return LANE_COMPLETION
    ...
    ) and completion_economics_ok:
        return LANE_COMPLETION
    if remaining in {1, 2} and completion_economics_ok:
        return LANE_COMPLETION
    return LANE_COVERAGE


def completion_sort_key(book: LaneBook) -> tuple:
    """Order completion work without allowing legacy score authorities to conflict.

    V4.14.5 live rows use ``total_score_value`` as the sole score-ranking input.
    Rows without ``total_score_phase`` retain the exact V4.14.4 ordering for
    archived tests/tools and historical replays.
    """
    if str(getattr(book, "total_score_phase", "") or ""):
        remaining = max(0, int(book.observations_remaining or 0))
        deadline = max(0.0, min(1.0, _finite(getattr(book, "deadline_urgency", 0.0))))
        critical = bool(getattr(book, "deadline_critical", False))
        ev_known = bool(getattr(book, "completion_ev_known", False))
        ev_ok = bool(getattr(book, "completion_ev_ok", True))
        ...
        score_value = _finite(getattr(book, "total_score_value", 0.0))
        ...
        return (
            ...
            efficiency_rank,
            -deadline,
            0 if bool(getattr(book, "score_pnl_ready", True)) else 1,
            -_finite(getattr(book, "recent_realized_pnl", 0.0)),
    ...
    # Legacy V4.14.4 ordering: Kappa completion + density by urgency *and*
    # productivity. A productive qualified book may continue cycling, while an
    # order-hungry book is demoted even when it is ONE_AWAY. Critical rolling
    # deadlines remain absolute completion priority. Reached only by archived
    # callers/replays that do not annotate total_score_phase.
    remaining = max(0, int(book.observations_remaining or 0))
    deadline = max(
    ...
    # V4.13.6 density hierarchy after economic feasibility:
    #   ONE_AWAY -> productive CORE/RECYCLING -> CORE_PROBE -> TWO_AWAY -> refresh/density.
    # A known-positive candidate always outranks an unknown candidate; known
    # non-positive candidates normally never reach this pool because lane
    # classification moves them to COVERAGE.
    ev_known = bool(getattr(book, "completion_ev_known", False))
    ev_ok = bool(getattr(book, "completion_ev_ok", True))
    ev_rank = 0 if (ev_known and ev_ok) else (1 if not ev_known else 2)
    if remaining == 1:
    ...
    # Known-positive deep EV earns the explicit density hierarchy. For unknown
    # EV, retain the older productivity-efficiency protection so an INEFFICIENT
    # ONE_AWAY cannot crowd out a proven productive CORE merely on proximity.
    known_positive = bool(ev_known and ev_ok)
    primary_rank = type_rank if known_positive else efficiency_rank
    secondary_rank = efficiency_rank if known_positive else type_rank
    ...
    return (
        ...
        0 if critical else ev_rank,
        completion_cost,
        0 if critical else primary_rank,
        0 if critical else secondary_rank,
        -deadline,
        -productivity,
        score_pnl_rank,
        -recent_pnl,
...
def coverage_sort_key(book: LaneBook) -> tuple:
    ...
    productivity = _finite(getattr(book, "kappa_productivity_score", 0.0))
    total_score_rank = -_finite(getattr(book, "total_score_value", 0.0)) if str(
        getattr(book, "total_score_phase", "") or ""
    ) else 0.0
    return (
        efficiency_rank,
        total_score_rank,
...
def select_lane_candidates(
    ...
    *,
    ...
    for lane in (LANE_REALIZATION, LANE_COMPLETION, LANE_COVERAGE):
        ...
        if lane == LANE_COMPLETION and take > 0 and not any(
            str(getattr(row, "total_score_phase", "") or "") for row in pools[lane]
        ):
            # Legacy V4.13.6 density/flywheel balance (disabled for V4.14.5 live rows):
            #   1) best *known-positive* ONE_AWAY conversion,
            #   2) one productive CORE/RECYCLING bridge,
            #   3) one CORE_PROBE bootstrap when capacity remains,
            #   4) the normal economically-ranked completion pool.
            # This preserves the V4.13.1 recycling bridge without letting it
            # preempt the highest-value ONE_AWAY conversion discovered by deep EV.
            positive_one_away = next(
                (
                    row for row in pools[lane]
                    if max(0, int(getattr(row, "observations_remaining", 0) or 0)) == 1
                    and bool(getattr(row, "completion_ev_known", False))
                    and bool(getattr(row, "completion_ev_ok", True))
                ),
                None,
            )
            positive_flywheel = next(
                (
                    row for row in pools[lane]
                    if (
                        bool(getattr(row, "core_candidate", False))
                        or bool(getattr(row, "recycling_candidate", False))
                    )
                    and bool(getattr(row, "completion_ev_known", False))
                    and bool(getattr(row, "completion_ev_ok", True))
            ...
            flywheel = next(
                (
                    row for row in pools[lane]
                    if bool(getattr(row, "core_candidate", False))
                    or bool(getattr(row, "recycling_candidate", False))
                ),
                None,
            )
            specials = []
            # Known-positive flywheel work earns priority over an unknown probe;
            # otherwise retain the V4.13.2 CORE_PROBE bootstrap guarantee before
            # an unknown recycling bridge.
            for row in (positive_one_away, positive_flywheel, probe, flywheel):
                if row is not None and int(row.book_id) not in {int(x.book_id) for x in specials}:
                    specials.append(row)
            if specials:
                special_ids = {int(row.book_id) for row in specials}
                chosen_rows = specials[:take] + [
                    row for row in chosen_rows if int(row.book_id) not in special_ids
