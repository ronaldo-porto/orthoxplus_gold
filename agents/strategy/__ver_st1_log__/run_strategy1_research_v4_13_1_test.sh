...
# V4.13.1 Kappa Productivity Scheduler Correction deployment contract.
...
from research_kappa_productivity import (
    ...
    kappa_state, priority_for_state, scheduler_phase, TIER_INEFFICIENT,
)

if KAPPA_PRODUCTIVITY_VERSION != "simplified_kappa_productivity_v4_13_1":
    raise SystemExit("ERROR: stale research_kappa_productivity.py for V4.13.1")
if scheduler_phase(40) != "BOOTSTRAP" or scheduler_phase(41) != "BALANCED" or scheduler_phase(80) != "DENSITY":
    raise SystemExit("ERROR: V4.13.1 breadth/density phase thresholds failed")
...
if not book115.core_candidate or kappa_state(observations=3, core=True) != STATE_CORE:
    raise SystemExit("ERROR: V4.13.1 productive CORE recycling failed")
if book98.execution_tier != TIER_INEFFICIENT or book98.core_candidate:
    raise SystemExit("ERROR: V4.13.1 inefficient-book demotion failed")
if priority_for_state(book115, phase="DENSITY") <= priority_for_state(book98, phase="DENSITY"):
    raise SystemExit("ERROR: V4.13.1 Kappa-productivity ranking failed")
...
if book92.execution_tier != TIER_INEFFICIENT:
    raise SystemExit("ERROR: V4.13.1 zero-RT order-sink demotion failed")
...
if not bridge.recycling_candidate or bridge.core_candidate:
    raise SystemExit("ERROR: V4.13.1 CORE bootstrap bridge failed")
print("V4.13.1 Kappa productivity scheduler correction API OK")
PYV413PRODUCTIVITY
...
grep -q 'RESEARCH_POLICY_VERSION = "simplified_kappa_productivity_v4_13_1"' "$AGENT_PATH/Strategy1_Research.py" || {
  echo "ERROR: Strategy1_Research.py is not simplified_kappa_productivity_v4_13_1" >&2
  exit 1
}
...
research_enable_fast_candidate_screen=1 research_candidate_count=10 research_cohort_size=8 research_cohort_exploration_slots=1 research_max_open_books=6 research_max_active_open_books=6 research_max_total_open_books=12 research_max_parked_open_books=6 research_max_total_abs_base=3.0 research_kappa_conversion_pressure_enabled=1 research_kappa_conversion_reserve_slots=3 research_kappa_exploration_slots=1 research_kappa_flywheel_enabled=1 research_kappa_productivity_enabled=1 research_persistent_maker_enabled=1 research_hysteresis_min_price_ticks=3 research_post_only_safety_ticks=2 \
...
echo "[Strategy1_Research] version=simplified_kappa_productivity_v4_13_1"
echo "[Strategy1_Research] log_dir=$RESEARCH_DIR"

if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  echo "V4.13.1 launcher preflight-only PASS"
  exit 0
fi
