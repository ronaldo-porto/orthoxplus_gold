...
KAPPA_PRODUCTIVITY_VERSION = "wide_kappa_productivity_v4_14_2"
...
def wide_kappa_density_due(
    *,
    observations: int,
    raw_kappa: float | None,
    kappa_eligible: bool,
    execution_tier: str = TIER_UNKNOWN,
    min_density_observations: int = 6,
    preferred_density_observations: int = 8,
    raw_kappa_target: float = 0.35,
) -> bool:
    """Return whether a qualified book still needs *quality density* work.

    Three non-zero observations only make a book validator-eligible. V4.14.2
    keeps economically viable books cycling through six observations, then lets
    a positive-but-weak raw-Kappa book continue only to a small preferred cap.
    Negative-Kappa and known-inefficient books are never force-densified by this
    helper; they must re-earn capacity through the normal economics/productivity
    path instead of consuming breadth budget.
    """
    if not bool(kappa_eligible):
        return False
    if str(execution_tier or TIER_UNKNOWN).upper() == TIER_INEFFICIENT:
        return False
    obs = max(0, int(observations or 0))
    min_obs = max(3, int(min_density_observations or 6))
    preferred = max(min_obs, int(preferred_density_observations or 8))
    if obs < min_obs:
        return True
    if obs >= preferred:
        return False
    if raw_kappa is None:
        # Unknown Kappa gets the minimum density floor, not unlimited extension.
        return False
    kappa = _finite(raw_kappa, 0.0)
    target = max(1e-9, _finite(raw_kappa_target, 0.35))
    return 0.0 <= kappa < target
