...
def adverse_selection_risk(
    ...
    *,
    ...
    """Penalize expected adverse markout. OFI is optional and never static imbalance."""
    try:
...
OFI_FAST_ALPHA = 0.45
FROZEN_MARKOUT_MIN_SAMPLES = 4
FROZEN_MARKOUT_PRIOR_STRENGTH = 8.0
FROZEN_HYSTERESIS_MIN_PRICE_TICKS = 2.0
FROZEN_HYSTERESIS_EV_THRESHOLD = 0.06


def _touch_finite(value: Any) -> float | None:
    try:
        number = float(value)
    except (TypeError, ValueError):
        return None
...
def extract_touch(book: Any) -> BookTouch | None:
    ...
    if not bids or not asks:
        return None
    bid_px = _touch_finite(getattr(bids[0], "price", None))
    ask_px = _touch_finite(getattr(asks[0], "price", None))
    if bid_px is None or ask_px is None or ask_px <= bid_px:
        return None
    if not hasattr(bids[0], "quantity") or not hasattr(asks[0], "quantity"):
        return None
    bid_qty = _touch_finite(getattr(bids[0], "quantity", None))
    ask_qty = _touch_finite(getattr(asks[0], "quantity", None))
...
def expected_markout_bps(
    ...
    *,
    min_samples: int = FROZEN_MARKOUT_MIN_SAMPLES,
    prior_strength: float = FROZEN_MARKOUT_PRIOR_STRENGTH,
) -> float:
    """Horizon-weighted ExpectedMarkout. Sparse books shrink toward 0."""
...
def inventory_factor(
    ...
    *,
    ...
    return _clip(room * age_scale, 0.0, 1.0)
...
def volume_headroom_factor(volume_cap_headroom: float) -> float:
    return _clip(0.15 + 0.85 * _clip(volume_cap_headroom, 0.0, 1.0), 0.0, 1.0)
...
def expected_exit_capacity(
    *,
    ...
    if exit_rate is None:
        ...
        rate_norm = _clip(_finite(exit_rate) / max(1e-9, float(exit_rate_ref)), 0.0, 1.0)
    slow = 0.25 + 0.75 * rate_norm
    liq = liquidity_factor(volatility)
    tox_pen = 1.0 - 0.50 * _clip(toxicity, 0.0, 1.0)
...
class BaseStrategy(FinanceSimulationAgent):
    ...
    BASE_CHAMPION_PARENT = 'base_v4_1_1_maker_guard'
    ...
    def _bsimpl_2_Strategy1_Debug_handle(self, state: MarketSimulationStateUpdate) -> FinanceAgentResponse:
        ...
        if self._should_emit_tick(self._tick):
            self._emit(
                ...
                screen_all_books_ms=round(float((getattr(self, '_screen_timing', {}) or {}).get('screen_ms', 0.0) or 0.0), 4),
                full_predict_ms=round(float((getattr(self, '_screen_timing', {}) or {}).get('full_predict_ms', self._debug_stage_ms.get('predict_all_books_ms', 0.0)) or 0.0), 4),
                screen_fallback=int((getattr(self, '_screen_timing', {}) or {}).get('screen_fallback', 0) or 0),
                candidate_count=int((getattr(self, '_screen_timing', {}) or {}).get('candidate_count', 0) or 0),
                cache_hits=int((getattr(self, '_screen_timing', {}) or {}).get('cache_hits', 0) or 0),
                cache_misses=int((getattr(self, '_screen_timing', {}) or {}).get('cache_misses', 0) or 0),
    ...
    def _bsimpl_3_Strategy1_Research_initialize(self) -> None:
        ...
        self.score_ev_uncovered_weight = max(0.0, float(getattr(cfg, 'score_ev_uncovered_weight', 0.04)))
        self.score_ev_stale_weight = max(0.0, float(getattr(cfg, 'score_ev_stale_weight', 0.03)))
        self.score_ev_stale_ms = max(0.0, float(getattr(cfg, 'score_ev_stale_ms', 30000.0)))
        self.score_ev_adverse_weight = max(0.0, float(getattr(cfg, 'score_ev_adverse_weight', 0.05)))
        self.research_enable_realization = self._as_bool(getattr(cfg, 'research_enable_realization', True))
        self.research_realize_age_ticks = max(1, int(getattr(cfg, 'research_realize_age_ticks', 8)))
        self.research_profit_realize_bps = float(getattr(cfg, 'research_profit_realize_bps', 2.0))
        self.research_toxic_realize_bps = max(0.0, float(getattr(cfg, 'research_toxic_realize_bps', 10.0)))
        self._score_ev_last: dict[int, Any] = {}
        self._realization_last: dict[int, Any] = {}
        self._last_realization_ts: dict[int, int] = {}
        self._sim_start_ts: int | None = None
        self._last_sim_ts: int | None = None
        self.quote_hysteresis_enabled = self._as_bool(getattr(cfg, 'quote_hysteresis_enabled', True))
        self.hysteresis_min_price_ticks = max(0.25, float(getattr(cfg, 'hysteresis_min_price_ticks', FROZEN_HYSTERESIS_MIN_PRICE_TICKS)))
        self.hysteresis_ev_threshold = max(0.0, float(getattr(cfg, 'hysteresis_ev_threshold', FROZEN_HYSTERESIS_EV_THRESHOLD)))
        self.adaptive_ttl_enabled = self._as_bool(getattr(cfg, 'adaptive_ttl_enabled', True))
        self.entry_size_enabled = self._as_bool(getattr(cfg, 'entry_size_enabled', True))
        self.fast_candidate_screen_enabled = self._as_bool(getattr(cfg, 'fast_candidate_screen_enabled', True))
        self.candidate_count = clamp_candidate_count(getattr(cfg, 'candidate_count', DEFAULT_CANDIDATE_COUNT))
        self.screen_p95_target_ms = max(1.0, float(getattr(cfg, 'screen_p95_target_ms', P95_TARGET_MS)))
        self._feature_cache = FeatureCache()
        self._last_screen: ScreenResult | None = None
        self._screen_timing: dict[str, Any] = {}
        self._response_ms_samples: list[float] = []
        ...
        self._score_regime_debounce = DebounceState('COVERAGE', 'COVERAGE', 0)
        self._market_regime = 'NORMAL'
        self._score_regime = 'COVERAGE'
        ...
        self._execution_quotes = QuoteLifecycleStore(max_live=1024, max_pending_markouts=256)
        self._execution_hazard = FillHazardModel()
        self._execution_last: dict[int, dict[str, Any]] = {}
        self._execution_fills_classified = 0
        self._ofi = OfiTracker()
        self._ofi_last: dict[int, OfiSnapshot] = {}
        self._markout_horizons: dict[int, dict[int, dict[str, float]]] = {}
        self._markout_by_book: dict[int, dict[str, float]] = {}
        self._entry_size_last: dict[int, Any] = {}
        self._volume_headroom_last = 1.0
    ...
    def _regime_v2_snapshot(self, profile_list, predictions, selection) -> dict[str, Any]:
        ...
        denom = max(n, 1)
        obs_universe = self._observation_universe(profile_list)
        buckets = scheduler_bucket_counts(obs_universe, target)
        activity_deficit = self._activity_deficit_ratio(obs_universe, target, n)
        rt_velocity = self._round_trip_velocity()
        return {
            'book_count': n,
    ...
    def _observation_universe(self, profiles=None) -> dict[int, int]:
        counts = {
        ...
        for profile in list(profiles or []):
            try:
                ...
                continue
            counts.setdefault(bid, int(counts.get(bid, 0) or 0))
        return counts

    def _simulation_time_s(self) -> float:
        start_ts = getattr(self, '_sim_start_ts', None)
        now_ts = getattr(self, '_last_sim_ts', None)
    ...
    def _round_trip_velocity(self) -> float:
        return round_trip_velocity(
            int(getattr(self, '_research_round_trip_closes', 0) or 0),
            self._simulation_time_s(),
        )

    def _activity_deficit_ratio(
        self,
        observation_counts: dict[int, int],
        required: int,
        book_count: int,
    ) -> float:
        last_map = getattr(self, '_last_realization_ts', {}) or {}
        now = getattr(self, '_last_sim_ts', None)
    ...
    def _volume_cap_headroom(self, state) -> float:
        try:
            remaining = float(self._volume_cap_remaining(state))
            total = float(self._volume_cap_quote(state))
        except Exception:
            return 1.0
        if total <= 1e-12:
            self._volume_headroom_last = 1.0
            return 1.0
        head = max(0.0, min(1.0, remaining / total))
        self._volume_headroom_last = head
        return head

    def _ofi_snapshot(self, book_id: int) -> OfiSnapshot:
        return (getattr(self, '_ofi_last', {}) or {}).get(int(book_id), UNSUPPORTED)

    def _ofi_flow(self, book_id: int) -> float | None:
        snap = self._ofi_snapshot(book_id)
        if not snap.supported:
            return None
        if snap.ofi_fast is not None:
            return float(snap.ofi_fast)
        if snap.ofi_normalized is not None:
            return float(snap.ofi_normalized)
        return None

    def _ofi_against(self, book_id: int, inventory_sign: float = 0.0) -> float:
        return ofi_against_position(self._ofi_flow(int(book_id)), inventory_sign)

    def _update_ofi_from_state(self, state) -> None:
        tracker = getattr(self, '_ofi', None)
        if tracker is None:
            return
        ...
        for book_id, book in items:
            try:
                snap = tracker.update(int(book_id), extract_touch(book))
                self._ofi_last[int(book_id)] = snap
            except (TypeError, ValueError, AttributeError):
                continue

    def _horizon_expected_markout(self, book_id: int) -> float | None:
        horizons = (getattr(self, '_markout_horizons', {}) or {}).get(int(book_id))
        if not horizons:
            return None
        if all(
    ...
    def _markout_snapshot(self, book_id: int) -> tuple[float | None, int]:
        horizons = (getattr(self, '_markout_horizons', {}) or {}).get(int(book_id))
        if horizons:
            n = sum(
        ...
        row = (getattr(self, '_markout_by_book', {}) or {}).get(int(book_id), {})
        n = int(row.get('n', 0) or 0)
        if n <= 0:
            return None, 0
        return float(row.get('sum', 0.0)) / n, n

    def _evaluate_markouts(self, state) -> None:
        store = getattr(self, '_execution_quotes', None)
        if store is None or not getattr(store, 'pending', None):
            return
        ...
        for row in store.evaluate(now_ts=now, mids=mids):
            if row.status != 'OK' or row.markout_bps is None:
                continue
            stats = self._markout_by_book.setdefault(int(row.book), {'n': 0.0, 'sum': 0.0})
            stats['n'] = float(stats.get('n', 0.0)) + 1.0
            stats['sum'] = float(stats.get('sum', 0.0)) + float(row.markout_bps)
            horizons = self._markout_horizons.setdefault(int(row.book), {})
    ...
    def _book_exit_rate(self, book_id: int | None, inventory) -> float | None:
        if book_id is None:
            return None
        samples = int(
            (getattr(self, '_research_round_trip_samples_by_book', {}) or {}).get(int(book_id), 0) or 0
        )
        sim_t = self._simulation_time_s()
    ...
    def _entry_toxicity(self, book_id: int | None, profile) -> float:
        toxic = 0.0
        if book_id is not None:
            ...
            markout_mean, markout_n = self._markout_snapshot(int(book_id))
    ...
    def _entry_size_decision(self, *, base_size: float, profile, inventory):
        ...
        if ev is not None:
            ...
            mean, n = self._markout_snapshot(int(book_id))
        ...
        decision = allowed_entry_size(
            ...
            volatility=vol,
            toxicity=self._entry_toxicity(book_id, profile),
            expected_markout=markout,
            ofi_against=ofi_against,
            volume_cap_headroom=float(getattr(self, '_volume_headroom_last', 1.0) or 1.0),
            exit_rate=self._book_exit_rate(book_id, inventory),
            recent_drawdown=drawdown,
            hard_max_entry=float(getattr(self, 'mm_base_size', base_size) or base_size),
        )
        as_risk = 0.0 if ev is None else float(getattr(ev, 'adverse_selection_risk', 0.0) or 0.0)
        if ev is None:
            as_risk = composite_adverse_selection_risk(
                expected_markout_bps=markout, ofi_against=ofi_against,
            )
        if entry_adverse_blocked(expected_markout_bps=markout, adverse_selection_risk=as_risk):
            return EntrySizeDecision(
                entry_size=0.0,
                expected_exit_capacity=decision.expected_exit_capacity,
                inventory_after_full_fill=abs(float(getattr(inventory, 'net_base', 0.0) or 0.0)),
                inventory_risk_after_fill=decision.inventory_risk_after_fill,
                inventory_factor=decision.inventory_factor,
                liquidity_factor=decision.liquidity_factor,
                exit_capacity_factor=decision.exit_capacity_factor,
                volume_headroom_factor=decision.volume_headroom_factor,
                risk_factor=decision.risk_factor,
                uncapped_size=decision.uncapped_size,
                hard_cap=decision.hard_cap,
                trigger='ADVERSE_BLOCK',
            )
        return decision

    def _abs_inventory(self, book_id: int) -> float:
        try:
    ...
    def _compute_cheap_tob(self, book) -> tuple[float | None, float | None, int]:
        bids = getattr(book, 'bids', None) or []
        asks = getattr(book, 'asks', None) or []
        if not bids or not asks:
            return None, None, 0
    ...
    def _cheap_tob(self, book, *, book_id: int | None = None) -> tuple[float | None, float | None, int]:
        ...
        spread_bps, imb, ntrade = self._compute_cheap_tob(book)
        if cache is not None and book_id is not None:
            cache.store_touch(
                int(book_id), fingerprint,
                spread_bps=spread_bps, imbalance=imb, trade_events=ntrade,
            )
        return spread_bps, imb, ntrade

    def _keep_cheap_state(self, book_id, book, timestamp) -> None:
        cache = getattr(self, '_feature_cache', None)
        fingerprint = book_touch_fingerprint(book)
        ...
        try:
            ...
            pass
        try:
            snap = self._ofi.update(int(book_id), extract_touch(book))
            self._ofi_last[int(book_id)] = snap
        except Exception:
            pass

    def _fast_screen(self, state) -> ScreenResult:
        ...
        for book_id, book in (getattr(state, 'books', None) or {}).items():
            ...
            remaining = self._observations_remaining(bid)
            mem = mems.get(bid)
            if mem is None:
                mem = mems.get(book_id)
            spread_bps, imb, ntrade = self._cheap_tob(book, book_id=bid)
            live = False
        ...
        result = select_fast_candidates(rows, self.candidate_count)
        self._last_screen = result
        return result

    def _full_predict_fallback(self, state):
        started = time.perf_counter()
        predictions = self._bsimpl_0_DetailedTemplateAgent__predict_all_books(state)
        elapsed = (time.perf_counter() - started) * 1000.0
        timing = getattr(self, '_screen_timing', None)
        if timing is None:
            timing = {}
            self._screen_timing = timing
        timing['full_predict_ms'] = elapsed
        timing['screen_ms'] = float(timing.get('screen_ms', 0.0) or 0.0)
        timing['candidate_count'] = len(predictions)
        timing['screen_fallback'] = 1
        cache = getattr(self, '_feature_cache', None)
        if cache is not None:
            timing['cache_hits'] = int(cache.hits)
            timing['cache_misses'] = int(cache.misses)
        if getattr(self, '_debug_stage_ms', None) is not None:
            self._debug_stage_ms['predict_all_books_ms'] = elapsed
        return predictions

    def _bsimpl_3_Strategy1_Research__predict_all_books(self, state):
        books = getattr(state, 'books', None) or {}
        if not books:
            self._last_predictions = {}
            return {}
        if not getattr(self, 'fast_candidate_screen_enabled', True):
            return self._full_predict_fallback(state)
        started = time.perf_counter()
        try:
            screen = self._fast_screen(state)
        except Exception:
            self._screen_timing['screen_ms'] = (time.perf_counter() - started) * 1000.0
            return self._full_predict_fallback(state)
        self._screen_timing['screen_ms'] = (time.perf_counter() - started) * 1000.0
        ...
        for book_id, book in books.items():
            if int(book_id) in selected:
                ...
                self._keep_cheap_state(book_id, book, timestamp)
        self._last_predictions = predictions
        elapsed = (time.perf_counter() - started) * 1000.0
        self._screen_timing['full_predict_ms'] = elapsed
        self._screen_timing['candidate_count'] = len(screen.selected)
        self._screen_timing['forced_inventory_count'] = len(screen.forced_inventory)
        self._screen_timing['forced_kappa_count'] = len(screen.forced_kappa)
        self._screen_timing['screen_fallback'] = 0
        cache = getattr(self, '_feature_cache', None)
        if cache is not None:
            self._screen_timing['cache_hits'] = int(cache.hits)
            self._screen_timing['cache_misses'] = int(cache.misses)
        if getattr(self, '_debug_stage_ms', None) is not None:
            self._debug_stage_ms['predict_all_books_ms'] = elapsed
            self._debug_stage_ms['screen_all_books_ms'] = float(self._screen_timing.get('screen_ms', 0.0) or 0.0)
        return predictions

    def _bsimpl_3_Strategy1_Research_select_books_for_trading(self, state, predictions):
        selection = self._bsimpl_2_Strategy1_Debug_select_books_for_trading(state, predictions)
        if getattr(self, 'fast_candidate_screen_enabled', True):
            screen = getattr(self, '_last_screen', None)
            allowed = {int(bid) for bid in getattr(screen, 'selected', []) or []}
            if allowed:
                selection.alpha_books = [bid for bid in selection.alpha_books if int(bid) in allowed]
                selection.maintenance_books = [bid for bid in selection.maintenance_books if int(bid) in allowed]
        return selection
    ...
    def _score_ev_for_book(self, book_id: int, expected_alpha: float, mem):
        ...
        try:
            ...
            net = float(getattr(snap, 'net_qty', 0.0) or 0.0)
            inv_util = min(1.0, abs(net) / cap)
        ...
        last_real = (getattr(self, '_realization_last', {}) or {}).get(int(book_id))
        if last_real is not None:
            inventory_state = getattr(last_real, 'state', None)
        try:
            recent_pnl = float(getattr(mem, 'recent_pnl', 0.0) or 0.0)
        except (TypeError, ValueError, AttributeError):
            recent_pnl = None
        last_ts = (getattr(self, '_last_realization_ts', {}) or {}).get(int(book_id))
        now_ts = getattr(self, '_last_sim_ts', None)
        last_ms = None if last_ts is None else float(last_ts) / 1_000_000.0
        now_ms = None if now_ts is None else float(now_ts) / 1_000_000.0
        markout_mean, markout_n = self._markout_snapshot(int(book_id))
        expected_override = self._horizon_expected_markout(int(book_id))
        ofi_against = self._ofi_against(int(book_id), net)
        return compute_score_ev(
            book=int(book_id),
            side=side,
            ...
            uncovered_weight=float(self.score_ev_uncovered_weight),
            stale_weight=float(self.score_ev_stale_weight),
            stale_ms=float(self.score_ev_stale_ms),
            adverse_weight=float(self.score_ev_adverse_weight),
        )
    ...
    def _bsimpl_3_Strategy1_Research__global_book_rank(self, expected_alpha: float, mem) -> float:
        ...
        if getattr(self, 'debug_enabled', False) or getattr(self, 'research_enabled', False):
            self._emit(
                ...
                activity_deficit_value=breakdown.activity_deficit_value,
                adverse_selection_risk=breakdown.adverse_selection_risk,
    ...
    def _bsimpl_3_Strategy1_Research__inventory_urgency(self, inventory: InventorySnapshot, regime_params: RegimeParamSet, regime: MarketRegime, archetype: BookArchetype) -> float:
        book_id = getattr(inventory, '_research_book_id', None)
        if book_id is not None:
            last = (getattr(self, '_realization_last', {}) or {}).get(int(book_id))
            if last is not None:
                return float(getattr(last, 'exit_urgency', 0.0) or 0.0)
        if not getattr(self, 'research_enable_realization', True):
            return self._bsimpl_1_Strategy1__inventory_urgency(inventory, regime_params, regime, archetype)
        remaining = 0
        if book_id is not None:
            remaining = self._observations_remaining(int(book_id))
        return exit_urgency(
            ...
            volatility=0.0,
            ofi=self._ofi_flow(int(book_id)) if book_id is not None else None,
            imbalance=0.0,
    ...
    def _evaluate_realization(
        ...
        try:
            ...
            recent = None
        remaining = self._observations_remaining(book_id)
        ...
        fee = max(float(self.score_ev_fees_bps), 0.5) * 2.0
        ...
        return evaluate_realization(
            ...
            expected_markout=expected_markout,
            volatility=vol,
            ofi=self._ofi_flow(int(book_id)),
            imbalance=0.0,
            kappa_need=kappa_completion_need(remaining, unreal),
            observations_remaining=remaining,
            volume_cap_headroom=self._volume_cap_headroom(state),
    ...
    def _manage_realization(
        ...
        qty = self._round_order_size(
            abs(float(inventory.net_base)),
            getattr(state.config, 'volumeDecimals', 8),
        )
        if qty <= 0:
            return 0
        if self._try_close_loans(
            ...
            placed += 1
        decision = self._evaluate_realization(book_id, book, inventory, state, regime_params)
        self._realization_last[int(book_id)] = decision
        self._emit(
            'REALIZATION',
            force=True,
            tick=getattr(self, '_tick', None),
            ...
            inventory_age=decision.inventory_age,
            exit_urgency=decision.exit_urgency,
            state=decision.state,
            maker_exit_ev=decision.maker_exit_ev,
            taker_exit_cost=decision.taker_exit_cost,
        ...
        if decision.selected_action == ACTION_TAKER:
            ...
            if self._execute_aggressive_close(response, book_id, book, qty, long_pos):
                placed += 1
                self._inventory_reason[book_id] = 'SELECTIVE_TAKER'
            return placed
        n = self._place_maker_exit(
    ...
    def _bsimpl_3_Strategy1_Research__manage_inventory(self, response: FinanceAgentResponse, state: MarketSimulationStateUpdate, book_id: int, book: Book, inventory: InventorySnapshot, regime_params: RegimeParamSet, regime: MarketRegime, archetype: BookArchetype) -> int:
        ...
        return self._manage_realization(response, state, book_id, book, inventory, regime_params)
    ...
    def _bsimpl_3_Strategy1_Research_onTrade(self, event, validator: str | None=None) -> None:
        ...
        if abs(realized_delta) > 1e-12:
            ...
            if ts is None:
                ts = getattr(self, '_last_sim_ts', None)
            if ts is None:
                ts = getattr(self, '_tick', None)
            if ts is not None:
                try:
                    self._last_realization_ts[int(book_id)] = int(ts)
                except (TypeError, ValueError):
                    pass
    ...
    def _execution_on_fill(self, event, *, book_id: int, before: float, after: float) -> None:
        ...
        if record is not None:
            ...
            if is_maker:
                ...
                if fill_ts is not None and fill_price > 0.0:
    ...
    def _bsimpl_3_Strategy1_Research_handle(self, state: MarketSimulationStateUpdate) -> FinanceAgentResponse:
        try:
            ...
            if ts is not None:
                self._last_sim_ts = int(ts)
                if getattr(self, '_sim_start_ts', None) is None:
                    self._sim_start_ts = int(ts)
        except (TypeError, ValueError):
            pass
        self._execution_close_from_notices(state)
        try:
            self._update_ofi_from_state(state)
        except Exception:
            pass
        try:
            self._evaluate_markouts(state)
        except Exception:
            pass
    ...
    def _bsimpl_3_Strategy1_Research_dynamic_order_size(self, base_size: float, profile: BookProfile, regime_params: RegimeParamSet, inventory: InventorySnapshot, vol_dec: int, mid: float | None=None) -> float:
        ...
        entry = None
        if getattr(self, 'entry_size_enabled', True):
            try:
                entry = self._entry_size_decision(
                    base_size=float(base_size),
                    profile=profile,
                    inventory=inventory,
                )
        ...
        if entry is not None:
            book_id = getattr(profile, 'book_id', None)
            if book_id is not None:
                self._entry_size_last[int(book_id)] = entry
    ...
    def _hysteresis_hold_sides(
        ...
        try:
            ...
            imb = None if profile is None else getattr(profile, 'imbalance', None)
            ofi_snap = self._ofi_snapshot(int(book_id))
            new_ofi = None
            if ofi_snap.supported:
                new_ofi = ofi_snap.ofi_fast if ofi_snap.ofi_fast is not None else ofi_snap.ofi_normalized
    ...
    def _bsimpl_3_Strategy1_Research__place_skewed_quotes(self, response: FinanceAgentResponse, state: MarketSimulationStateUpdate, book_id: int, book, profile: BookProfile, prediction: DirectionForecast, inventory: InventorySnapshot, regime_params: RegimeParamSet, size: float, edge_bias: float, stats: dict | None=None) -> int:
        ...
        elif inventory.band == 'FLAT' and completion_samples <= 0 and self.research_kappa_completion_enabled:
            lane = LANE_COVERAGE
        else:
            lane = LANE_NORMAL
        if getattr(self, 'score_ev_enabled', True):
            ev = (getattr(self, '_score_ev_last', {}) or {}).get(int(book_id))
        ...
        self._quote_submit_snapshot[int(book_id)] = {
            ...
            'inventory_state': getattr(inventory, 'band', None),
    ...
    _inventory_urgency = _bsimpl_3_Strategy1_Research__inventory_urgency
    ...
    _predict_all_books = _bsimpl_3_Strategy1_Research__predict_all_books
    ...
    select_books_for_trading = _bsimpl_3_Strategy1_Research_select_books_for_trading
