...
class CohortCandidate:
    ...
    # V4.12.8 exact rolling-progress deadline.  Critical qualified refresh may
    # remain in the cohort; early warning-window refresh may not crowd out
    # incomplete work.
...
def _valid(c: CohortCandidate) -> bool:
    ...
    if not c.entry_feasible or not c.economics_ok:
        return False
    # Stable score-qualified books leave the acquisition cohort.  V4.12.8
    # admits refresh only at a critical rolling deadline; the broad warning
    # horizon is telemetry/ranking context, not permission to churn maintenance.
    if c.score_qualified and not (c.needs_refresh and c.deadline_critical):
        return False
    return True
...
def _priority(c: CohortCandidate) -> tuple:
    ...
    deadline = max(0.0, min(1.0, float(c.deadline_urgency or c.refresh_urgency or 0.0)))
    critical = bool(c.deadline_critical)
    refresh = bool(c.needs_refresh)
