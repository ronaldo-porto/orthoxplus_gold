...
def profitable_maker_exit_ttl_ms(
    *,
    baseline_ttl_ms: float,
    maker_net_bps: float | None,
    market_regime: str | None,
    enabled: bool = True,
    min_net_bps: float = 0.0,
    persistent_ttl_ms: float = 3000.0,
) -> tuple[float, bool]:
    """Extend only a profitable, non-toxic Maker exit beyond one publish cycle."""
    base = max(1.0, float(baseline_ttl_ms))
    if not bool(enabled):
        return base, False
    try:
        ...
        return base, False
    if not math.isfinite(net) or net <= float(min_net_bps):
        return base, False
    if str(market_regime or "NORMAL").upper() in {"TOXIC", "STRESSED"}:
        return base, False
    try:
        desired = max(base, float(persistent_ttl_ms))
    except (TypeError, ValueError):
        desired = base
    return min(desired, 5000.0), desired > base + 1e-9


def hold_existing_profitable_maker_exit(
    *,
    ...
    """Keep queue priority when an existing profitable exit is still materially valid."""
    if not bool(enabled) or existing_price is None or desired_price is None:
        return False
    try:
        net = float(maker_net_bps)
        ...
        return False
    if not math.isfinite(net) or net <= float(min_net_bps) or old <= 0 or new <= 0 or want <= 0:
        return False
    if have + 1e-12 < want * max(0.0, min(1.0, float(min_qty_fraction))):
        return False
    return abs(old - new) / tick < max(1.0, float(reprice_ticks))
