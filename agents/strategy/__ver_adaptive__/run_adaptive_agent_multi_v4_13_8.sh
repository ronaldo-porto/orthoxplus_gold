...
Options:
  ...
  --log                   enable detailed BaseStrategy V4.13.8 telemetry
...
if ! grep -q 'DEPLOY_POLICY_VERSION = "base_v4_13_8_champion"' "$AGENT_PATH/BaseStrategy.py"; then
  echo "ERROR: AdaptiveAgent V4.13.8 requires live BaseStrategy V4.13.8 champion." >&2
  echo 'Expected DEPLOY_POLICY_VERSION = "base_v4_13_8_champion" in BaseStrategy.py' >&2
  exit 1
fi
if ! grep -q 'BASE_CHAMPION_PARENT = "simplified_kappa_productivity_v4_13_8"' "$AGENT_PATH/BaseStrategy.py"; then
  echo "ERROR: AdaptiveAgent requires BaseStrategy promoted from frozen Research V4.13.8." >&2
  exit 1
fi
...
if ! grep -q 'ADAPTIVE_VERSION = "adaptive_v4_13_8_realtime"' "$AGENT_PATH/AdaptiveAgent.py"; then
  echo "ERROR: AdaptiveAgent.py is not adaptive_v4_13_8_realtime." >&2
  echo "Deploy the V4.13.8 realtime AdaptiveAgent.py before starting this launcher." >&2
  exit 1
fi
...
echo "[AdaptiveAgent] version=adaptive_v4_13_8_realtime"
echo "[AdaptiveAgent] base_policy=base_v4_13_8_champion"
...
if [[ "${ADAPTIVE_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  ...
  echo "AdaptiveAgent V4.13.8 realtime launcher preflight PASS"
  exit 0
fi
