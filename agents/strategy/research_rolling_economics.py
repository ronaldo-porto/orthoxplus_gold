...
"""Restart-safe rolling realized-PnL evidence for Research scoring/risk context."""
...
ROLLING_ECONOMICS_VERSION = "rolling_economics_v4_15_1"
...
class RollingBookEconomics:
    ...
    def loss_rate(self) -> float:
        return 0.0 if self.nonzero_count <= 0 else self.negative_count / float(self.nonzero_count)

    def as_log(self) -> dict[str, Any]:
        return {
            "rolling_economics_version": ROLLING_ECONOMICS_VERSION,
    ...
    for raw_book, raw_rows in raw.items():
        ...
        for item in raw_rows:
            if not isinstance(item, (list, tuple)) or len(item) < 2:
                continue
            try:
                ts, pnl = int(item[0]), float(item[1])
            except (TypeError, ValueError):
                continue
            if not math.isfinite(pnl) or abs(pnl) <= 1e-15:
                continue
            rows.append((ts, pnl))
        if rows:
...
def prune_realized_pnl_events(
    events: Mapping[int, list[tuple[int, float]]] | None, *, now: int | None, lookback_ns: int,
...
def note_realized_pnl_event(
    events: Mapping[int, list[tuple[int, float]]] | None, *, book_id: int, timestamp: int,
    realized_pnl: float, now: int | None, lookback_ns: int,
) -> dict[int, list[tuple[int, float]]]:
    ...
    out.setdefault(int(book_id), []).append((max(0, int(timestamp)), pnl))
    return prune_realized_pnl_events(out, now=now, lookback_ns=lookback_ns)


def rolling_book_economics(
    events: Mapping[int, list[tuple[int, float]]] | None, book_id: int, *, now: int | None, lookback_ns: int,
) -> RollingBookEconomics:
    rows = list(prune_realized_pnl_events(events, now=now, lookback_ns=lookback_ns).get(int(book_id), ()) or ())
    ...
    return RollingBookEconomics(
        book=int(book_id),
        nonzero_count=n,
        positive_count=sum(1 for pnl in pnls if pnl > 0.0),
        negative_count=sum(1 for pnl in pnls if pnl < 0.0),
        realized_sum=total,
        realized_mean=(total / n if n else 0.0),
