...
class Strategy5(Strategy3):
    ...
    def _hjb_spread_pressure(
        ...
        if getattr(self, "_mining_phase", "ACTIVE") == "CALIBRATE":
    ...
    def _place_skewed_quotes(
        ...
        if phase in ("OBSERVE", "RESUME"):
            return 0
        if phase == "COOLDOWN" and (
            not self.hjb_cooldown_quote_enabled or inventory.band == "FLAT"
        ):
            return 0
        if phase == "CALIBRATE" and not self._phase_allows_new_risk(
            book_id,
            profile,
            inventory,
        ...
        quote_size = size
        if phase == "CALIBRATE":
            quote_size *= self.hjb_calibration_size_mult
        if is_weak:
            quote_size *= self.hjb_weak_book_size_mult
            if stats is not None:
