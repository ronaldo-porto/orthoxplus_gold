...
def test_early_run_is_coverage():
    state, trigger, _ = propose_score_regime(_early_coverage_stats(), "COVERAGE", INSTANT)
    assert state == "COVERAGE"
    assert trigger == "ZERO_OBS_HYSTERESIS"
    from_unknown, enter_trigger, _ = propose_score_regime(
        _early_coverage_stats(), "BALANCED", INSTANT,
    )
    assert from_unknown == "COVERAGE"
    assert enter_trigger == "ZERO_OBS_COVERAGE"


def test_mature_coverage_is_completion():
    state, trigger, _ = propose_score_regime(
        _mature_completion_stats(), "COVERAGE", INSTANT,
    )
    assert state == "COMPLETION"
    assert trigger == "NEAR_QUALIFICATION"


def test_mature_qualification_is_balanced():
    state, trigger, _ = propose_score_regime(
        _mature_balanced_stats(), "COMPLETION", INSTANT,
    )
    assert state == "BALANCED"
    assert trigger == "QUALIFIED_COVERAGE"


def test_score_regime_hysteresis():
    # coverage_ratio=0.30 sits between exit (0.25) and enter (0.40).
    mid = _liquid_stats(
        books_0_obs=6,
        books_1_remaining=8,
        books_2_remaining=4,
    ...
    # eligible_ratio=0.16 sits between exit (0.14) and enter (0.22).
    balanced_mid = _liquid_stats(
        books_0_obs=2,
        books_1_remaining=4,
        books_2_remaining=3,
        books_eligible=3,
    ...
    stay_balanced, bal_trig, _ = propose_score_regime(balanced_mid, "BALANCED", INSTANT)
    assert stay_balanced == "BALANCED"
    assert bal_trig == "QUALIFIED_HYSTERESIS"
    from_completion_mid, _, _ = propose_score_regime(balanced_mid, "COMPLETION", INSTANT)
    assert from_completion_mid == "COMPLETION"
...
def test_inactive_tier_does_not_set_score_regime():
    state, _, _ = propose_score_regime(
        _mature_balanced_stats(inactive=12, inactive_frac=0.60, pending_kappa_frac=0.40),
        "BALANCED",
        INSTANT,
    )
    assert state == "BALANCED"
...
def test_score_regime_ignores_market_volatility_and_spread():
    scoring = dict(
        books_0_obs=16,
        books_1_remaining=2,
        books_2_remaining=1,
    ...
    calm, _, _ = propose_score_regime(
        _liquid_stats(**scoring, spread_med=1.0, vol_med=0.0001, stressed_ratio=0.02),
        "COVERAGE",
        INSTANT,
    )
    wild, _, _ = propose_score_regime(
        _liquid_stats(**scoring, spread_med=20.0, vol_med=0.05, stressed_ratio=0.80),
        "COVERAGE",
        INSTANT,
    )
    assert calm == "COVERAGE"
    assert wild == "COVERAGE"
    assert score_regime_metrics(_liquid_stats(**scoring, spread_med=20.0, vol_med=0.05))[
        "coverage_ratio"
    ] == 0.8
...
def test_score_regime_debounce_requires_consecutive_ticks():
    thr = RegimeV2Thresholds(debounce_ticks=3)
    state = DebounceState("COVERAGE", "COVERAGE", 0)
    completion = _mature_completion_stats()

    first = classify_regime_v2(completion, score_state=state, thresholds=thr)
    assert first.score_regime == "COVERAGE"
    assert first.score_debounce.hold == 1

    second = classify_regime_v2(
        completion, score_state=first.score_debounce, thresholds=thr,
    )
    assert second.score_regime == "COVERAGE"
    third = classify_regime_v2(
        completion, score_state=second.score_debounce, thresholds=thr,
    )
    assert third.score_regime == "COMPLETION"
    assert third.score_debounce.hold == 0
