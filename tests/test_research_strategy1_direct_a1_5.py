...
def test_direct_candidate_version():
    assert 'SIMPLE_POLICY_VERSION = "strategy1_direct_v4_16_2_a1_5"' in SRC
    assert 'RESEARCH_POLICY_VERSION = SIMPLE_POLICY_VERSION' in SRC
    assert DIRECT_ECONOMICS_VERSION == 'direct_economics_v4_16_2_a1_5'
    assert DIRECT_EXECUTION_CONTROLLER_VERSION == 'direct_execution_controller_v4_16_2_a1_5'
    assert DIRECT_QUALITY_VERSION == 'direct_maker_quality_v4_16_2_a1_5'
...
def test_a14_moderate_directional_taker_is_rejected_after_calibration_haircut():
    ...
    # positive under the 8 bps alpha scale. A1.5 maps full score to 4 bps and
...
def test_a15_directional_taker_entry_is_disabled_even_when_counterfactual_ev_is_strong():
    d = choose_direct_execution(
        maker_lifecycle_ev=0.010,
        directional_score=1.0,
        crossing_bps=0.20,
...
def test_a15_preserves_base_final_validator_but_uses_direct_fast_screen():
...
def test_a15_fastpath_prioritizes_incomplete_books_and_bounds_qualified_share():
    rows = [
...
def test_a15_fastpath_completion_priority_beats_qualified_book():
...
def test_a15_net_realized_shortfall_catches_fee_loss_hidden_by_positive_gross():
    s = MakerLifecycleStats()
...
def test_a15_cubic_downside_penalizes_large_tail_more_than_equal_mean_small_losses():
...
def test_a15_pre_submit_freshness_budget_is_authoritative():
...
def test_a15_selected_only_profile_build_avoids_full_universe_profile_ranking():
...
def test_a15_fastpath_version_and_bounded_candidate_count():
    assert DIRECT_FASTPATH_VERSION == 'direct_fastpath_v4_16_2_a1_5'
