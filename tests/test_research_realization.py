...
from research_hybrid import REASON_REJECT_LADDER, REASON_REJECT_TRANSITION
from research_exit_urgency import EXIT_URGENCY_COMPONENTS
...
def test_low_inventory_is_maker_first():
    ...
    for name in EXIT_URGENCY_COMPONENTS:
        assert name in decision.as_log()
...
def test_taker_rejected_when_cost_exceeds_risk():
    ...
    assert classify_exit_action(decision.exit_urgency) != ACTION_TAKER
    assert decision.selected_action != ACTION_TAKER
    assert decision.selected_action == ACTION_AGGRESSIVE
    assert decision.trigger in {
        "TAKER_REJECTED_COST",
        "TAKER_REJECTED_LADDER",
        "TAKER_REJECTED_ECONOMICS",
        "TAKER_REJECTED_NET_FLOOR",
        "MAKER_LADDER",
        "MAKER_EXIT_EV",
    }
...
def test_emergency_reduction_works():
    ...
    assert hard.trigger == "TAKER_CATASTROPHIC"

    quarantined = evaluate_realization(
        **_low_inventory(
            inventory_size=1.10,
            inventory_ratio=0.98,
            inventory_age=12.0,
    ...
    assert quarantined.selected_action != ACTION_TAKER
    assert quarantined.taker_allowed is False
    assert quarantined.hybrid_reason == REASON_REJECT_TRANSITION
    assert quarantined.trigger == REASON_REJECT_TRANSITION
...
def test_exit_urgency_is_continuous_unit_interval():
    for age in (0, 5, 20, 80):
        ...
        assert classify_exit_action(score) in {*MAKER_ACTIONS, ACTION_TAKER}


def test_hybrid_lock_profit_stays_on_maker_rung_until_eligible():
    decision = evaluate_realization(
        **_low_inventory(
            inventory_size=0.20,
    ...
    assert decision.selected_action != ACTION_TAKER
    assert decision.selected_action in MAKER_ACTIONS
    assert decision.taker_qty_frac == 0.0


def test_hybrid_lock_profit_takes_when_ladder_eligible():
    decision = evaluate_realization(
        **_low_inventory(
            inventory_size=0.20,
            inventory_ratio=0.18,
    ...
    assert decision.taker_eligible is True
    if decision.taker_exit_cost + 1e-12 >= decision.taker_risk:
        assert decision.selected_action != ACTION_TAKER
    else:
        assert decision.selected_action == ACTION_TAKER
        assert decision.trigger == "TAKER_EXIT_EV"
        assert 0.0 < decision.taker_qty_frac < 1.0


def test_hybrid_does_not_open_or_dump_when_capped():
    decision = evaluate_realization(
        **_low_inventory(
            inventory_size=0.20,
            inventory_ratio=0.18,
