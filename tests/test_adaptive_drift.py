...
"""Phase 3 Step 3.4: persistent fast-vs-slow Adaptive drift detection."""
...
from adaptive_ev import apply_drift_defensive_floors
...
CFG = DriftConfig(
    ...
    vol_abs=0.0008,
    vol_rel=0.25,
    inventory_age_abs=3.0,
    inventory_age_rel=0.25,
    hold_requests=20,
...
def test_volatility_and_inventory_age_channels():
    tracker = DriftTracker(CFG)
    _feed(tracker, 40, 0, volatility=0.002, inventory_age=6.0)
    _v1, req = _feed(tracker, 10, 40, volatility=0.006, inventory_age=18.0)
    v2, _req = _feed(tracker, 10, req, volatility=0.006, inventory_age=18.0)
    assert v2[0].trigger_drift is True
    hits = {name for name, sig in v2[0].signals.items() if sig.deteriorated}
    assert "volatility" in hits
    assert "inventory_age" in hits


def test_realized_pnl_alias_feeds_maker_pnl_channel():
    tracker = DriftTracker(CFG)
    _feed(tracker, 40, 0, realized_pnl=0.05)
    _v1, req = _feed(tracker, 10, 40, realized_pnl=-0.04)
    v2, _req = _feed(tracker, 10, req, realized_pnl=-0.04)
    assert v2[0].trigger_drift is True
    hits = {name for name, sig in v2[0].signals.items() if sig.deteriorated}
    assert "maker_pnl" in hits


def test_drift_floors_widen_cut_size_and_raise_exit():
    spread, size, exit_scale = apply_drift_defensive_floors(
        spread_scale=1.0,
        size_scale=1.0,
    ...
    assert spread >= 1.08 - 1e-12
    assert spread <= 1.18 + 1e-12
    assert size <= 0.65 + 1e-12
    assert exit_scale >= 1.12 - 1e-12
    assert exit_scale <= 1.20 + 1e-12


def test_drift_floors_cannot_tighten_or_exceed_base_size():
    spread, size, exit_scale = apply_drift_defensive_floors(
        spread_scale=0.90,
        size_scale=1.20,
        exit_urgency_scale=0.80,
        min_widen=0.08,
        max_widen=0.18,
        max_size=0.65,
        min_exit_boost=0.12,
