...
def test_phase1_trading_ev_formula_is_unchanged():
    ...
    assert abs(
        result.final_score
        - (result.trading_ev + result.completion_value + result.activity_deficit_value
           - result.dust_cost - result.inventory_cost - result.latency_cost
           - result.adverse_selection_risk)
    ) < 1e-12
...
def test_scheduler_bucket_counts():
    ...
    assert counts["books_eligible"] == 3
