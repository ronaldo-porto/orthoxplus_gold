...
#
# Multiple miners on one VPS:
#   ./run_miner.sh -i sn79-m1 -w wallet1 -h hotkey1 -a 8091 ...
#   ./run_miner.sh -i sn79-m2 -w wallet2 -h hotkey2 -a 8092 ...
#   ./run_miner.sh -i sn79-m3 -w wallet3 -h hotkey3 -a 8093 ...
#
# Each miner MUST have a unique hotkey and Axon port. The public IP and netuid
# may be shared. Unique PM2 names prevent later launches from replacing earlier
# miners.
...
# PM2/tmux process identity for multi-miner VPS deployments.
# Set with either:
#   PM2_NAME=sn79-m1 ./run_miner.sh ...
# or:
#   ./run_miner.sh ... -i sn79-m1
#
# If omitted, the name is derived from the final hotkey after CLI parsing.
PM2_NAME="${PM2_NAME:-}"
_EXTERNAL_PM2_NAME="$PM2_NAME"
...
# A process name supplied by the caller is per-miner runtime configuration and
# must win over a shared repository .env.
[ -n "$_EXTERNAL_PM2_NAME" ] && PM2_NAME="$_EXTERNAL_PM2_NAME"

while getopts e:p:w:h:u:a:g:n:m:t:l:i:G flag; do
    case "${flag}" in
        e) ENDPOINT=${OPTARG};;
...
# Resolve process identity after CLI parsing so the hotkey-specific default is
# correct. Only the PM2/tmux label is sanitized; wallet/hotkey values are not.
if [ -z "$PM2_NAME" ]; then
    PM2_NAME="miner-${HOTKEY_NAME}"
fi
PM2_NAME=$(printf '%s' "$PM2_NAME" | tr -cs 'A-Za-z0-9_.-' '_')
PM2_NAME="${PM2_NAME%_}"
[ -n "$PM2_NAME" ] || PM2_NAME="miner-${HOTKEY_NAME}"
TMUX_NAME="tmux-${PM2_NAME}"
...
# ── Launch miner ───────────────────────────────────────────────────────────────
# Restart only this miner.  Never delete a generic "miner" process, because that
# would stop another UID on a multi-miner VPS.
pm2 delete "$PM2_NAME" 2>/dev/null || true

# After this miner's old process is gone, refuse to start if ANOTHER local
# process already owns the requested Axon port.
if command -v ss >/dev/null 2>&1; then
    if ss -ltnH 2>/dev/null | awk '{print $4}' | grep -Eq "(^|:)$AXON_PORT$"; then
        echo "ERROR: AXON_PORT $AXON_PORT is already in use by another process."
        echo "Choose a unique port for each miner, e.g. 8091, 8092, 8093."
        exit 1
    fi
fi
...
" "$AGENT_PARAMS $GENTRX_PARAMS")
    ...
    tmux new-session -d -s "$TMUX_NAME" -n 'miner' 'htop -F miner.py'
    tmux set-option -t "$TMUX_NAME" mouse on
    tmux split-window -v -p 50 -t "$TMUX_NAME:miner.0" "pm2 logs '$PM2_NAME'"
    tmux split-window -h -p 50 -t "$TMUX_NAME:miner.1" "echo 'Waiting for training log...'; tail -F '$TRAIN_LOG' 2>/dev/null"
    tmux select-pane -t "$TMUX_NAME:miner.0"
