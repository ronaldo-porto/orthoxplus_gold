...
grep -q 'SIMPLE_POLICY_VERSION = "strategy1_direct_v4_16_2_a1_3"' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
...
if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  ...
  PYTHONPATH="$AGENT_PATH:$SCRIPT_DIR${PYTHONPATH:+:$PYTHONPATH}" \
    python -m pytest -q \
      tests/test_research_strategy1_direct_a1_3.py \
  ...
  echo "Strategy1 direct V4.16.2 A1.3 preflight PASS"
  exit 0
fi

echo "[Strategy1_Research_Simple] version=strategy1_direct_v4_16_2_a1_3"
