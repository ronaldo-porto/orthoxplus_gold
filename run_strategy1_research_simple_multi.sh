...
grep -q 'SIMPLE_POLICY_VERSION = "strategy1_direct_v4_16_2_a1_8"' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
...
# A1.8: A1.7.5 parameters remain frozen; exit realization changes only the Direct profitable-exit TTL.
...
if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  ...
  PYTHONPATH="$AGENT_PATH:$SCRIPT_DIR${PYTHONPATH:+:$PYTHONPATH}" \
    python -m pytest -q \
      ...
      tests/test_research_strategy1_direct_a1_8.py \
  ...
  echo "Strategy1 direct V4.16.2 A1.8 preflight PASS"
  exit 0
fi

echo "[Strategy1_Research_Simple] version=strategy1_direct_v4_16_2_a1_8"
