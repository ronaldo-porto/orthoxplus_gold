...
grep -q 'SIMPLE_POLICY_VERSION = "strategy1_direct_v4_16_2_a1_7_4"' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
...
# A1.7.4: only parameters that remain material to the direct candidate are pinned here.
...
if [[ "${RESEARCH_PREFLIGHT_ONLY:-0}" == "1" ]]; then
  ...
  echo "Strategy1 direct V4.16.2 A1.7.4 preflight PASS"
  exit 0
fi

echo "[Strategy1_Research_Simple] version=strategy1_direct_v4_16_2_a1_7_4"
