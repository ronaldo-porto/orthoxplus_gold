...
if [[ "$V626_BUILD" == "1" ]]; then
  ...
  grep -qF 'caps = self._v627_caps(v626_open_book_caps(raw))' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
  ...
  # The loss budget's VALUE belongs to v6.2.7 (which retires it); here only its presence is checked,
  # so a misspelled key still fails the gate.
  [[ "$PARAMS" == *"research_v626_loss_budget="* ]] || {
    echo "ERROR: v6.2.6 build without research_v626_loss_budget in PARAMS." >&2
    exit 1
  }
...
if [[ "$V627_BUILD" == "1" ]]; then
  # v6.2.7: rule A retired, rule B corrective, the caps on one band.
  grep -qF 'from research_v627_maker_ceiling import (' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.7 module is not imported." >&2
    exit 1
  }
  grep -qF 'getattr(self, "research_v626_loss_budget", False)' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.7 does not retire the loss budget by default." >&2
    exit 1
  }
  grep -qF 'out = v627_balance_gate_sides(' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.7 quotes are not gated on the book balance." >&2
    exit 1
  }
  grep -qF 'caps = self._v627_caps(v626_open_book_caps(v62_universe_caps(n, lot)))' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.7 startup caps are not reconciled to the band." >&2
    exit 1
  }
  grep -qF 'raw = v62_universe_caps(n, lot if self._v627_band_caps_on() else V625_BAND_CLIPS * lot)' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.7 pacing caps would apply the band twice." >&2
    exit 1
  }
  grep -qF 'out["research_a195_max_seed_abs_base"] = per_book_total' "$AGENT_PATH/research_v627_maker_ceiling.py" || {
    echo "ERROR: v6.2.7 seed bound is not one clip per book." >&2
    exit 1
  }
  [[ "$PARAMS" == *"research_v626_loss_budget=0"* ]] || {
    echo "ERROR: v6.2.7 build without research_v626_loss_budget=0 in PARAMS (rule A is retired)." >&2
    exit 1
  }
  # The balance gate's VALUE belongs to v6.2.8 (S4 supersedes and retires it); presence only here.
  [[ "$PARAMS" == *"research_v627_balance_gate="* ]] || {
    echo "ERROR: v6.2.7 build without research_v627_balance_gate in PARAMS." >&2
    exit 1
  }
...
if [[ "$V628_BUILD" == "1" ]]; then
  # v6.2.8: own-touch exits, band-relative urgency, fee-viable entries, both sides never frozen.
  grep -qF 'from research_v628_touch_exit import (' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.8 module is not imported." >&2
    exit 1
  }
  grep -qF 'with self._v628_rung_cap_scope():
                result = super()._research_apply_unified_exit(legacy, **kwargs)' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.8 unified exit is not priced through the capped rung." >&2
    exit 1
  }
  grep -qF '# v6.2.8 S1: the maker exit is priced through the capped rung.' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.8 maker exit placement is not priced through the capped rung." >&2
    exit 1
  }
  grep -qF 'return super()._research_parked_touch_exit(book_id, book, inventory)' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.8 parked-touch exit is not priced through the capped rung." >&2
    exit 1
  }
  grep -qF 'self._v628_util_book = int(book_id)' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.8 exit urgency does not measure inventory against the band." >&2
    exit 1
  }
  grep -qF 'if not self._v628_book_viable(book_id, facts):' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.8 entries are not gated on fee viability." >&2
    exit 1
  }
  grep -qF 'out = v628_skewed_sides(' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.8 quotes are not skewed across both sides." >&2
    exit 1
  }
  # S3 is retired before launch: it filters on net-of-fee PnL, but the making and skill legs are
  # both fee-blind, and it admitted 5 of 57 testnet books.  Its code stays, switched off.
  for key in research_v627_balance_gate=0 research_v628_rung_cap=1 research_v628_band_inventory=1 \
             research_v628_fee_viable=0 research_v628_skew_sides=1; do
    [[ "$PARAMS" == *"$key"* ]] || { echo "ERROR: v6.2.8 build without $key in PARAMS." >&2; exit 1; }
  done
...
if [[ "$V6210_BUILD" == "1" ]]; then
  # v6.2.10: quotes one tick inside the others' best price -- entries (P1) and own-touch maker exits (P2).
  grep -qF 'from research_v6210_touch_improve import (' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.10 module is not imported." >&2
    exit 1
  }
  grep -qF 'bid_px, ask_px = self._v6210_entry_prices(state, int(book_id), book, sides, bid_px, ask_px)' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.10 entry quotes are not priced inside the others' touch." >&2
    exit 1
  }
  grep -qF 'priced = v6210_improved_price_fn(' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.10 maker exits are not priced inside the others' touch." >&2
    exit 1
  }
  [[ "$(grep -cF 'self._v6210_exit_book = (' "$AGENT_PATH/Strategy1_Research_Simple.py")" == "3" ]] || {
    echo "ERROR: v6.2.10 does not bind the priced book in all three frozen exit producers." >&2
    exit 1
  }
  grep -qF 'own_ids = {row.order_id for row in ledger.live_orders(int(book_id))}' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.10 does not skip our own orders when reading the touch (it would walk the price)." >&2
    exit 1
  }
  for key in research_v6210_improve_entries=1 research_v6210_improve_exits=1; do
    [[ "$PARAMS" == *"$key"* ]] || { echo "ERROR: v6.2.10 build without $key in PARAMS." >&2; exit 1; }
  done
...
if [[ "$V6212_BUILD" == "1" ]]; then
  # v6.2.12: a held book defers its pace step instead of discarding the sample, so a book that is never
  # flat for a whole sampling interval can still step once it is flat (v6.2.11.1 ran 128 of 128 at the
  # minimum clip).  The held book still quotes one minimum order and still never steps while held.
  grep -qF 'from research_v6212_pace_defer import (' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.12 module is not imported." >&2
    exit 1
  }
  grep -qF 'pace.sampled_ns, pace.volume = v6212_sample_after_hold(' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.12 held branch does not go through the deferred sample." >&2
    exit 1
  }
  grep -qF 'self._v6212_count("pace_deferred")' "$AGENT_PATH/Strategy1_Research_Simple.py" || {
    echo "ERROR: v6.2.12 deferral is not counted." >&2
    exit 1
  }
  [[ "$V6211_BUILD" == "1" ]] || { echo "ERROR: v6.2.12 needs the v6.2.11 R2 hold on the pace step." >&2; exit 1; }
